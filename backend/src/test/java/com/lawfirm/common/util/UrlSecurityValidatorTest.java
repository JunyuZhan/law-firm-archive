...
class UrlSecurityValidatorTest {
  ...
  void validateUrl_shouldAllowHttp() {
    assertThatCode(() -> validator.validateUrl("http://www.baidu.com")).doesNotThrowAnyException();
  }

  @Test
  @DisplayName("HTTPS 协议应该通过验证")
  void validateUrl_shouldAllowHttps() {
    assertThatCode(() -> validator.validateUrl("https://www.baidu.com")).doesNotThrowAnyException();
  }
  ...
  void validateUrl_shouldAllowPublicIp() {
    assertThatCode(() -> validator.validateUrl("http://8.8.8.8")).doesNotThrowAnyException();
  }
