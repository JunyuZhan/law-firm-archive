...
class FieldChangeUtilsTest {
  ...
  static class ParentModel {
    ...
    public ParentModel(String parentField) {
  ...
  void compare_shouldOnlyCompareSpecifiedFields() {
    ...
    List<FieldChangeUtils.FieldChange> changes =
        FieldChangeUtils.compare(oldObj, newObj, fieldNames);
  ...
  void formatChanges_shouldFormatChanges() {
    ...
    assertThat(result).contains("name: [张三] -> [李四]").contains("age: [30] -> [35]").contains("; ");
  }
  ...
  void formatChanges_shouldNotHaveDelimiterForSingleChange() {
    List<FieldChangeUtils.FieldChange> changes =
        List.of(new FieldChangeUtils.FieldChange("name", "张三", "李四"));
