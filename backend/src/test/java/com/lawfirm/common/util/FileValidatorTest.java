...
class FileValidatorTest {
    ...
    class ExtensionTests {
        ...
        void validate_ExeFile_ShouldFail() {
            // Given
            MockMultipartFile file = new MockMultipartFile(
                    "file", "malware.exe", "application/octet-stream", new byte[10]);
            ...
            assertTrue(result.getErrorMessage().contains("禁止") || 
                       result.getErrorMessage().contains("可执行"));
        }
        ...
        void validate_JarFile_ShouldFail() {
            // Given
            MockMultipartFile file = new MockMultipartFile(
                    "file", "app.jar", "application/java-archive", new byte[10]);
        ...
        void validate_UnsupportedExtension_ShouldFail() {
            // Given
            MockMultipartFile file = new MockMultipartFile(
                    "file", "data.xyz", "application/octet-stream", new byte[10]);
    ...
    class SecurityTests {
        ...
        void validate_DisguisedExeAsJpg_ShouldFail() {
            ...
            MockMultipartFile file = new MockMultipartFile(
                    "file", "image.jpg", "image/jpeg", exeContent);
