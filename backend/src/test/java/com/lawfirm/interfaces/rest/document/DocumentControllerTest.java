...
/** DocumentController 单元测试 测试文档预览和编辑相关接口 */
...
class DocumentControllerTest {
  ...
  @Mock private DocAccessLogService accessLogService;
  ...
  class GetPreviewConfigTests {
    ...
    void shouldReturnPreviewConfigForSupportedFile() throws Exception {
      ...
      String proxyUrl =
          "http://backend:8080/api/document/"
              + TEST_DOCUMENT_ID
              + "/file-proxy?token=test&expires=123456";
      ...
      // When & Then
      mockMvc
          .perform(get("/document/{id}/preview", TEST_DOCUMENT_ID))
  ...
  class GetEditConfigTests {
    ...
    void shouldReturnEditConfigForSupportedFile() throws Exception {
      ...
      when(onlyOfficeService.generateEditConfig(
              anyString(),
              eq("test.docx"),
              contains("doc_" + TEST_DOCUMENT_ID),
              eq(TEST_USER_ID),
              eq(TEST_USERNAME),
              anyString()))
      ...
      // When & Then
      mockMvc
          .perform(get("/document/{id}/edit", TEST_DOCUMENT_ID))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.code").value(200))
          .andExpect(jsonPath("$.data.supported").value(true))
          .andExpect(jsonPath("$.data.documentId").value(TEST_DOCUMENT_ID));
  ...
  class OnlyOfficeCallbackTests {
    ...
    void shouldHandleCallbackWithStatus2() throws Exception {
      // Given
      Map<String, Object> payload = Map.of("status", 2, "url", "http://onlyoffice/download/url");
      doNothing().when(documentAppService).saveFromOnlyOffice(eq(TEST_DOCUMENT_ID), anyString());

      // When & Then
      mockMvc
          .perform(
              post("/document/{id}/callback", TEST_DOCUMENT_ID)
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(objectMapper.writeValueAsString(payload)))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.error").value(0));

      verify(documentAppService)
          .saveFromOnlyOffice(eq(TEST_DOCUMENT_ID), eq("http://onlyoffice/download/url"));
    }
    ...
    void shouldHandleCallbackWithStatus6() throws Exception {
      // Given
      Map<String, Object> payload = Map.of("status", 6, "url", "http://onlyoffice/download/url");
      doNothing().when(documentAppService).saveFromOnlyOffice(eq(TEST_DOCUMENT_ID), anyString());
    ...
    void shouldReturnErrorOnCallbackException() throws Exception {
      ...
      doThrow(new RuntimeException("保存失败"))
          .when(documentAppService)
          .saveFromOnlyOffice(anyLong(), anyString());
  ...
  class CheckEditSupportTests {
    ...
    void shouldReturnTrueForEditableDocument() throws Exception {
      ...
      // When & Then
      mockMvc
          .perform(get("/document/{id}/edit-support", TEST_DOCUMENT_ID))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.code").value(200))
          .andExpect(jsonPath("$.data.documentId").value(TEST_DOCUMENT_ID))
  ...
  class GetPreviewUrlTests {
    ...
    void shouldGeneratePreviewUrl() throws Exception {
      ...
      // When & Then
      mockMvc
          .perform(get("/document/{id}/preview-url", TEST_DOCUMENT_ID))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.code").value(200))
          .andExpect(jsonPath("$.data.documentId").value(TEST_DOCUMENT_ID))
          .andExpect(jsonPath("$.data.fileName").value("test.docx"))
