 ...
 * 证据清单文档生成属性测试
 *
 * <p>Feature: evidence-management Property 10: 证据清单序号生成 Validates: Requirements 5.4
 *
 * <p>测试证据清单中的证据项应有连续的序号（1, 2, 3...）
 */
class EvidenceListDocumentPropertyTest {
  ...
  void wordDocumentShouldHaveConsecutiveSequenceNumbers(
    ...
    // 使用局部变量明确非空，避免静态分析警告
    final List<EvidenceDTO> nonNullEvidences = evidences;

    EvidenceListDTO list = createTestList();
    Matter matter = createTestMatter();

    byte[] document = generator.generateWordDocument(list, matter, nonNullEvidences);
    ...
    try (XWPFDocument doc = new XWPFDocument(new ByteArrayInputStream(document))) {
      ...
      // 第一行是表头，从第二行开始是数据
      assertThat(rows.size()).isEqualTo(nonNullEvidences.size() + 1);
  ...
  @Property(tries = 100)
  void evidenceCountShouldMatchTableRows(@ForAll("evidenceList") List<EvidenceDTO> evidences) {
    ...
    try (XWPFDocument doc = new XWPFDocument(new ByteArrayInputStream(document))) {
      ...
      if (!tables.isEmpty()) {
        ...
        assertThat(evidenceTable.getRows().size()).isEqualTo(nonNullEvidences.size() + 1);
      }
    } catch (Exception e) {
      // 解析失败时跳过验证
    }
  ...
  @Property(tries = 50)
  void singleEvidenceShouldHaveSequenceOne(@ForAll("singleEvidence") EvidenceDTO evidence) {
  ...
  @Property(tries = 100)
  void sequenceNumbersShouldStartFromOneWithNoGaps(@ForAll("evidenceListSize") int size) {

    Assume.that(size > 0 && size <= 50);

    List<EvidenceDTO> evidences =
        IntStream.rangeClosed(1, size).mapToObj(i -> createEvidence("证据" + i)).toList();
        ...
        .flatMap(
            size ->
                Arbitraries.strings()
                    ...
                    .list()
                    .ofSize(size)
                    .map(names -> names.stream().map(this::createEvidence).toList()));
  }

  @Provide
  Arbitrary<EvidenceDTO> singleEvidence() {
    return Arbitraries.strings().alpha().ofMinLength(1).ofMaxLength(50).map(this::createEvidence);
  }
