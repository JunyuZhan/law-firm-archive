...
/** ClientChangeHistoryAppService 单元测试 测试客户变更历史服务的核心功能 */
...
class ClientChangeHistoryAppServiceTest {
  ...
  @Mock private ClientChangeHistoryRepository changeHistoryRepository;

  @Mock private ClientChangeHistoryMapper changeHistoryMapper;

  @Mock private ClientRepository clientRepository;

  @InjectMocks private ClientChangeHistoryAppService clientChangeHistoryAppService;
  ...
  class CreateChangeHistoryTests {
    ...
    void createChangeHistory_shouldSuccess_forEnterprise() {
      ...
      Client client =
          Client.builder().id(TEST_CLIENT_ID).name("测试企业").clientType("ENTERPRISE").build();

      when(clientRepository.getByIdOrThrow(eq(TEST_CLIENT_ID), anyString())).thenReturn(client);
      when(changeHistoryRepository.save(any(ClientChangeHistory.class)))
          .thenAnswer(
              invocation -> {
  ...
  class QueryChangeHistoryTests {
    ...
    void getClientChangeHistories_shouldSuccess() {
      // Given
      Client client = Client.builder().id(TEST_CLIENT_ID).name("测试企业").build();

      ClientChangeHistory history1 =
      ...
      // When
      List<ClientChangeHistoryDTO> result =
          clientChangeHistoryAppService.getClientChangeHistories(TEST_CLIENT_ID);
    ...
    void getChangeHistoriesByType_shouldSuccess() {
      ...
      // When
      List<ClientChangeHistoryDTO> result =
          clientChangeHistoryAppService.getChangeHistoriesByType(TEST_CLIENT_ID, "NAME");
  ...
  class DeleteChangeHistoryTests {
    ...
    void deleteChangeHistory_shouldSuccess() {
      // Given
      ClientChangeHistory history =
          ClientChangeHistory.builder().id(TEST_HISTORY_ID).clientId(TEST_CLIENT_ID).build();

      when(changeHistoryRepository.getByIdOrThrow(eq(TEST_HISTORY_ID), anyString()))
          .thenReturn(history);
