...
/** ClientTagAppService 单元测试 测试客户标签管理服务的核心功能 */
...
class ClientTagAppServiceTest {
  ...
  @Mock private ClientTagRepository clientTagRepository;

  @Mock private ClientRepository clientRepository;

  @Mock private ClientTagRelationMapper clientTagRelationMapper;

  @InjectMocks private ClientTagAppService clientTagAppService;
  ...
  class CreateTagTests {
    ...
    void createTag_shouldSuccess() {
      ...
      when(clientTagRepository.save(any(ClientTag.class)))
          .thenAnswer(
              invocation -> {
    ...
    void createTag_shouldFail_whenNameExists() {
      ...
      BusinessException exception =
          assertThrows(BusinessException.class, () -> clientTagAppService.createTag(command));
  ...
  class UpdateTagTests {
    ...
    void updateTag_shouldSuccess() {
      ...
      ClientTag tag =
          ClientTag.builder().id(TEST_TAG_ID).tagName("原标签名").tagColor("#ff0000").build();
    ...
    void updateTag_shouldFail_whenNameExists() {
      ...
      ClientTag tag = ClientTag.builder().id(TEST_TAG_ID).tagName("原标签名").build();
      ...
      // When & Then
      BusinessException exception =
          assertThrows(BusinessException.class, () -> clientTagAppService.updateTag(command));
      assertThat(exception.getMessage()).contains("标签名称已存在");
    }
  }
  ...
  class DeleteTagTests {
    ...
    void deleteTag_shouldSuccess() {
      // Given
      ClientTag tag = ClientTag.builder().id(TEST_TAG_ID).tagName("测试标签").build();
  ...
  class QueryTagTests {
    ...
    void listTags_shouldSuccess() {
      // Given
      ClientTag tag1 = ClientTag.builder().id(TEST_TAG_ID).tagName("标签1").build();

      ClientTag tag2 = ClientTag.builder().id(200L).tagName("标签2").build();

      @SuppressWarnings("unchecked")
      com.baomidou.mybatisplus.core.conditions.Wrapper<ClientTag> wrapper1 =
          any(com.baomidou.mybatisplus.core.conditions.Wrapper.class);
      when(clientTagRepository.list(wrapper1)).thenReturn(List.of(tag1, tag2));
    ...
    void getClientTags_shouldSuccess() {
      // Given
      ClientTag tag = ClientTag.builder().id(TEST_TAG_ID).tagName("客户标签").build();

      when(clientTagRelationMapper.selectTagIdsByClientId(TEST_CLIENT_ID))
          .thenReturn(List.of(TEST_TAG_ID));
      @SuppressWarnings("unchecked")
      com.baomidou.mybatisplus.core.conditions.Wrapper<ClientTag> wrapper2 =
          any(com.baomidou.mybatisplus.core.conditions.Wrapper.class);
      when(clientTagRepository.list(wrapper2)).thenReturn(Collections.singletonList(tag));
  ...
  class AssignTagsTests {
    ...
    void assignTags_shouldSuccess() {
      ...
      ClientTag tag1 = ClientTag.builder().id(TEST_TAG_ID).build();

      ClientTag tag2 = ClientTag.builder().id(200L).build();
