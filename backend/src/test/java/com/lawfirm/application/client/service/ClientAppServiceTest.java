...
class ClientAppServiceTest {
    ...
    class CreateClientTests {
        ...
        void createClient_Enterprise_Success() {
            ...
            when(clientRepository.count(any(LambdaQueryWrapper.class))).thenReturn(0L);
        ...
        void createClient_DuplicateName() {
            ...
            when(clientRepository.count(any(LambdaQueryWrapper.class))).thenReturn(1L);

            // When & Then
            BusinessException exception = assertThrows(BusinessException.class,
                    () -> clientAppService.createClient(command));
    ...
    class UpdateClientTests {
        ...
        void updateClient_DuplicateName() {
            ...
            // When & Then
            BusinessException exception = assertThrows(BusinessException.class,
                    () -> clientAppService.updateClient(command));
            assertEquals("客户名称已存在", exception.getMessage());
        }
    }
    // ==================== 删除客户测试 ====================
    ...
    class ListClientsTests {
        ...
        void listClients_AllPermission() {
            ...
            when(clientMapper.selectClientPage(any(), any(), any(), any(), any(), any())).thenReturn(page);
            when(userRepository.listByIds(any())).thenReturn(Arrays.asList(
                    createUser(2L, "案源人"),
                    createUser(3L, "负责律师")
            ));
