...
/** ClientRelatedCompanyAppService 单元测试 测试客户关联企业服务的核心功能 */
...
class ClientRelatedCompanyAppServiceTest {
  ...
  @Mock private ClientRelatedCompanyRepository relatedCompanyRepository;

  @Mock private ClientRepository clientRepository;

  @InjectMocks private ClientRelatedCompanyAppService clientRelatedCompanyAppService;
  ...
  class CreateRelatedCompanyTests {
    ...
    void createRelatedCompany_shouldSuccess_forEnterprise() {
      ...
      Client client = Client.builder().id(TEST_CLIENT_ID).clientType("ENTERPRISE").build();

      when(clientRepository.getByIdOrThrow(eq(TEST_CLIENT_ID), anyString())).thenReturn(client);
      when(relatedCompanyRepository.save(any(ClientRelatedCompany.class)))
          .thenAnswer(
              invocation -> {
  ...
  class UpdateRelatedCompanyTests {
    ...
    void updateRelatedCompany_shouldSuccess() {
      ...
      when(relatedCompanyRepository.getByIdOrThrow(eq(TEST_COMPANY_ID), anyString()))
          .thenReturn(company);
      lenient()
          .when(relatedCompanyRepository.updateById(any(ClientRelatedCompany.class)))
          .thenReturn(true);
  ...
  class DeleteRelatedCompanyTests {
    ...
    void deleteRelatedCompany_shouldSuccess() {
      // Given
      ClientRelatedCompany company =
          ClientRelatedCompany.builder().id(TEST_COMPANY_ID).relatedCompanyName("关联公司A").build();
  ...
  class QueryRelatedCompanyTests {
    ...
    void getRelatedCompaniesByClientId_shouldSuccess() {
      // Given
      Client client = Client.builder().id(TEST_CLIENT_ID).build();

      ClientRelatedCompany company1 =
          ClientRelatedCompany.builder()
              .id(TEST_COMPANY_ID)
              .clientId(TEST_CLIENT_ID)
      ...
      // When
      List<ClientRelatedCompanyDTO> result =
          clientRelatedCompanyAppService.getRelatedCompaniesByClientId(TEST_CLIENT_ID);
