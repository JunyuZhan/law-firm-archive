...
/**
 * ClientContactRecordAppService 单元测试
 * 测试客户联系记录服务的核心功能
 */
...
class ClientContactRecordAppServiceTest {
    ...
    private static final Long TEST_CONTACT_ID = 300L;
    private static final Long TEST_USER_ID = 400L;

    @Mock
    private ClientContactRecordRepository contactRecordRepository;
    ...
    @InjectMocks
    private ClientContactRecordAppService clientContactRecordAppService;
    ...
    class CreateContactRecordTests {
        ...
        void createContactRecord_shouldSuccess() {
            ...
            when(contactRecordRepository.save(any(ClientContactRecord.class))).thenAnswer(invocation -> {
    ...
    class UpdateContactRecordTests {
        ...
        void updateContactRecord_shouldSuccess() {
            ...
            when(contactRecordRepository.getByIdOrThrow(eq(TEST_RECORD_ID), anyString())).thenReturn(record);
            lenient().when(clientRepository.findById(TEST_CLIENT_ID)).thenReturn(client);
            lenient().when(userRepository.findById(TEST_USER_ID)).thenReturn(user);
            lenient().when(contactRecordRepository.updateById(any(ClientContactRecord.class))).thenReturn(true);
    ...
    class QueryContactRecordTests {
        ...
        void listContactRecords_shouldSuccess() {
            ...
            when(contactRecordRepository.findByClientId(any(Page.class), eq(TEST_CLIENT_ID))).thenReturn(page);
        ...
        void getContactRecordsByClientId_shouldSuccess() {
            ...
            // When
            List<ClientContactRecordDTO> result = clientContactRecordAppService.getContactRecordsByClientId(TEST_CLIENT_ID);
        ...
        void getFollowUpRecords_shouldSuccess() {
            ...
            // When
            List<ClientContactRecordDTO> result = clientContactRecordAppService.getFollowUpRecords(followUpDate);
