...
/** ClientShareholderAppService 单元测试 测试客户股东信息服务的核心功能 */
...
class ClientShareholderAppServiceTest {
  ...
  @Mock private ClientShareholderRepository shareholderRepository;

  @Mock private ClientRepository clientRepository;

  @InjectMocks private ClientShareholderAppService clientShareholderAppService;
  ...
  class CreateShareholderTests {
    ...
    void createShareholder_shouldSuccess_forEnterprise() {
      ...
      when(shareholderRepository.save(any(ClientShareholder.class)))
          .thenAnswer(
              invocation -> {
  ...
  class UpdateShareholderTests {
    ...
    void updateShareholder_shouldSuccess() {
      ...
      when(shareholderRepository.getByIdOrThrow(eq(TEST_SHAREHOLDER_ID), anyString()))
          .thenReturn(shareholder);
      lenient()
          .when(shareholderRepository.updateById(any(ClientShareholder.class)))
          .thenReturn(true);
  ...
  class DeleteShareholderTests {
    ...
    void deleteShareholder_shouldSuccess() {
      // Given
      ClientShareholder shareholder =
          ClientShareholder.builder().id(TEST_SHAREHOLDER_ID).shareholderName("张三").build();
  ...
  class QueryShareholderTests {
    ...
    void getShareholdersByClientId_shouldSuccess() {
      ...
      ClientShareholder shareholder1 =
          ClientShareholder.builder()
              .id(TEST_SHAREHOLDER_ID)
              .clientId(TEST_CLIENT_ID)
              .shareholderName("张三")
      ...
      ClientShareholder shareholder2 =
          ClientShareholder.builder()
              .id(200L)
              .clientId(TEST_CLIENT_ID)
              .shareholderName("李四")
      ...
      // When
      List<ClientShareholderDTO> result =
          clientShareholderAppService.getShareholdersByClientId(TEST_CLIENT_ID);
