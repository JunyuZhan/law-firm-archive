...
/** ConflictCheckAppService 单元测试 测试利冲审查服务的核心功能 */
...
class ConflictCheckAppServiceTest {
  ...
  @Mock private ConflictCheckItemMapper conflictCheckItemMapper;
  ...
  @InjectMocks private ConflictCheckAppService conflictCheckAppService;
  ...
  class CreateConflictCheckTests {
    ...
    void createConflictCheck_shouldSuccess_whenNoConflict() {
      // Given
      Matter matter =
          Matter.builder().id(TEST_MATTER_ID).clientId(TEST_CLIENT_ID).name("案件1").build();
      ...
      when(conflictCheckRepository.save(any(ConflictCheck.class)))
          .thenAnswer(
              invocation -> {
      ...
      lenient().when(clientRepository.list(wrapper1)).thenReturn(Collections.emptyList());
      @SuppressWarnings("unchecked")
      LambdaQueryWrapper<Matter> wrapper2 = any(LambdaQueryWrapper.class);
      when(matterRepository.list(wrapper2)).thenReturn(Collections.emptyList());
      when(conflictCheckItemMapper.insert(any(ConflictCheckItem.class)))
          .thenAnswer(
              invocation -> {
    ...
    void createConflictCheck_shouldSuccess_whenHasConflict() {
      ...
      Client conflictingClient = Client.builder().id(400L).name("对方当事人").build();
      ...
      Matter conflictingMatter =
          Matter.builder().id(500L).name("案件2").opposingParty("对方当事人").build();
      @SuppressWarnings("unchecked")
      LambdaQueryWrapper<Matter> wrapper3 = any(LambdaQueryWrapper.class);
      when(matterRepository.list(wrapper3))
          .thenReturn(Collections.singletonList(conflictingMatter));
      @SuppressWarnings("unchecked")
      LambdaQueryWrapper<Client> wrapper4 = any(LambdaQueryWrapper.class);
      lenient()
          .when(clientRepository.list(wrapper4))
          .thenReturn(Collections.singletonList(conflictingClient));
      when(conflictCheckItemMapper.insert(any(ConflictCheckItem.class)))
          .thenAnswer(
              invocation -> {
                ConflictCheckItem item = invocation.getArgument(0);
  ...
  class QuickConflictCheckTests {
    ...
    void quickConflictCheck_shouldReturnNoConflict() {
      ...
      when(clientRepository.list(wrapper5)).thenReturn(Collections.emptyList());
    ...
    void quickConflictCheck_shouldReturnExactMatch() {
      ...
      when(clientRepository.list(wrapper6))
  ...
  class ApplyConflictCheckTests {
    ...
    void applyConflictCheck_shouldSuccess() {
      ...
      when(clientRepository.list(wrapper7)).thenReturn(Collections.emptyList());
      @SuppressWarnings("unchecked")
      LambdaQueryWrapper<Matter> wrapper8 = any(LambdaQueryWrapper.class);
      when(matterRepository.list(wrapper8)).thenReturn(Collections.emptyList());
      ...
      // When
      ConflictCheckDTO result =
          conflictCheckAppService.applyConflictCheck("客户A", "对方当事人", "案件1", "MANUAL", "备注");
  ...
  class ApprovalTests {
    ...
    void approve_shouldSuccess() {
      ...
      when(conflictCheckRepository.getByIdOrThrow(eq(TEST_CHECK_ID), anyString()))
          .thenReturn(check);
  ...
  class ExemptionTests {
    ...
    void applyExemption_shouldSuccess() {
      ...
      lenient()
          ...
          .thenReturn(1L);
      lenient()
          .when(conflictCheckItemMapper.selectByCheckId(TEST_CHECK_ID))
    ...
    void applyExemption_shouldFail_whenNoConflict() {
      ...
      BusinessException exception =
          assertThrows(
              BusinessException.class, () -> conflictCheckAppService.applyExemption(command));
  ...
  class QueryConflictCheckTests {
    ...
    void listConflictChecks_shouldSuccess() {
      ...
      when(conflictCheckRepository.page(any(Page.class), any(LambdaQueryWrapper.class)))
          .thenReturn(page);
    ...
    void getConflictCheckById_shouldSuccess() {
      // Given
      ConflictCheck check = ConflictCheck.builder().id(TEST_CHECK_ID).checkNo("CC001").build();

      ConflictCheckItem item =
          ConflictCheckItem.builder().id(1L).checkId(TEST_CHECK_ID).partyName("当事人").build();
