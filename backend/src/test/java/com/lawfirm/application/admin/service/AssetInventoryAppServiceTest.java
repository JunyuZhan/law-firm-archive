...
/** AssetInventoryAppService 单元测试 测试资产盘点服务的核心功能 */
...
class AssetInventoryAppServiceTest {
  ...
  @Mock private AssetInventoryRepository inventoryRepository;

  @Mock private AssetInventoryMapper inventoryMapper;

  @Mock private AssetInventoryDetailMapper detailMapper;

  @Mock private AssetRepository assetRepository;

  @InjectMocks private AssetInventoryAppService assetInventoryAppService;
  ...
  class CreateInventoryTests {
    ...
    void createInventory_shouldSuccess_forFullType() {
      ...
      Asset asset2 = Asset.builder().id(2L).assetNo("A002").name("打印机").status("IN_USE").build();
    ...
    void createInventory_shouldSuccess_forPartialType() {
      ...
      Asset asset1 =
          Asset.builder().id(TEST_ASSET_ID).assetNo("A001").name("电脑").status("IN_USE").build();
    ...
    void createInventory_shouldFail_whenPartialWithoutAssetIds() {
      ...
      // When & Then
      assertThrows(
          BusinessException.class, () -> assetInventoryAppService.createInventory(command));
    }
  ...
  class UpdateDetailTests {
    ...
    void updateInventoryDetail_shouldSuccess() {
      ...
      // When
      assetInventoryAppService.updateInventoryDetail(TEST_DETAIL_ID, "IN_USE", "主仓库", 1L, null);
    ...
    void updateInventoryDetail_shouldFail_whenInventoryCompleted() {
      // Given
      AssetInventoryDetail detail =
          AssetInventoryDetail.builder().id(TEST_DETAIL_ID).inventoryId(TEST_INVENTORY_ID).build();
    ...
    void updateInventoryDetail_shouldDetectStatusDiscrepancy() {
      ...
      // When
      assetInventoryAppService.updateInventoryDetail(TEST_DETAIL_ID, "DAMAGED", "主仓库", 1L, null);
    ...
    void updateInventoryDetail_shouldDetectLocationDiscrepancy() {
      ...
      // When
      assetInventoryAppService.updateInventoryDetail(TEST_DETAIL_ID, "IN_USE", "分仓库", 1L, null);
  ...
  class CompleteInventoryTests {
    ...
    void completeInventory_shouldSuccess_forCreator() {
      ...
      AssetInventoryDetail detail1 =
          AssetInventoryDetail.builder()
              .id(1L)
              .discrepancyType(AssetInventoryDetail.DISCREPANCY_NORMAL)
              .build();

      AssetInventoryDetail detail2 =
          AssetInventoryDetail.builder()
              .id(2L)
              .discrepancyType(AssetInventoryDetail.DISCREPANCY_NORMAL)
              .build();

      when(inventoryRepository.getByIdOrThrow(TEST_INVENTORY_ID, "盘点不存在")).thenReturn(inventory);
    ...
    void completeInventory_shouldSuccess_forAdmin() {
      ...
      when(inventoryRepository.getByIdOrThrow(TEST_INVENTORY_ID, "盘点不存在")).thenReturn(inventory);
      when(detailMapper.selectByInventoryId(TEST_INVENTORY_ID)).thenReturn(List.of(detail));
      when(inventoryRepository.updateById(any(AssetInventory.class))).thenReturn(true);
  ...
  class QueryInventoryTests {
    ...
    void getInventoryById_shouldSuccess() {
      ...
      Asset asset = Asset.builder().id(TEST_ASSET_ID).assetNo("A001").name("电脑").build();
    ...
    void listInventories_shouldFilterByStatus() {
      // Given
      when(inventoryMapper.selectPage(any(Page.class), any())).thenReturn(new Page<>(1, 10));
