...
/** MeetingRecordAppService 单元测试 测试会议记录服务的核心功能 */
...
class MeetingRecordAppServiceTest {
  ...
  @Mock private MeetingRecordRepository recordRepository;

  @Mock private MeetingRecordMapper recordMapper;
  ...
  @InjectMocks private MeetingRecordAppService meetingRecordAppService;
  ...
  class CreateRecordTests {
    ...
    void createRecord_shouldFail_whenRoomNotFound() {
      ...
      BusinessException exception =
          assertThrows(
              BusinessException.class, () -> meetingRecordAppService.createRecord(command));
  ...
  class QueryRecordTests {
      ...
      lenient()
          .when(
              objectMapper.readValue(
                  eq("[999]"), any(com.fasterxml.jackson.core.type.TypeReference.class)))
          .thenReturn(Collections.singletonList(OTHER_USER_ID));

      // When & Then
      BusinessException exception =
          assertThrows(
              BusinessException.class, () -> meetingRecordAppService.getRecordById(TEST_RECORD_ID));
    ...
    void getRecordById_shouldSuccess_whenAttendee() throws JsonProcessingException {
      ...
      lenient()
          .when(
              objectMapper.readValue(
                  eq("[1,999]"), any(com.fasterxml.jackson.core.type.TypeReference.class)))
          .thenReturn(Arrays.asList(TEST_USER_ID, OTHER_USER_ID));
    ...
    void getRecordById_shouldSuccess_whenAdmin() {
      // Given
      MeetingRecord record =
          MeetingRecord.builder().id(TEST_RECORD_ID).organizerId(OTHER_USER_ID).build();
  ...
  class QueryByRoomTests {
    ...
    void getRecordsByRoom_shouldSuccess_whenAdmin() {
      // Given
      MeetingRecord record =
          MeetingRecord.builder().id(TEST_RECORD_ID).roomId(TEST_ROOM_ID).title("会议1").build();

      securityUtilsMock.when(() -> SecurityUtils.hasRole("ADMIN")).thenReturn(true);

      when(recordMapper.selectByRoomId(TEST_ROOM_ID)).thenReturn(Collections.singletonList(record));
  ...
  class QueryByDateRangeTests {
    ...
    void getRecordsByDateRange_shouldSuccess_whenAdmin() {
      ...
      MeetingRecord record =
          MeetingRecord.builder().id(TEST_RECORD_ID).meetingDate(LocalDate.now()).build();
