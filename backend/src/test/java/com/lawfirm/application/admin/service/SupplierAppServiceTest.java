...
/** SupplierAppService 单元测试 测试供应商管理服务的核心功能 */
...
class SupplierAppServiceTest {
  ...
  @InjectMocks private SupplierAppService supplierAppService;
  ...
  class QuerySupplierTests {
    ...
    void listSuppliers_shouldSuccess() {
      ...
      PageResult<SupplierDTO> result =
          supplierAppService.listSuppliers(query, null, null, null, null);
    ...
    void getSupplierById_shouldFail_whenNotFound() {
      ...
      BusinessException exception =
          assertThrows(
              BusinessException.class, () -> supplierAppService.getSupplierById(TEST_SUPPLIER_ID));
  ...
  class DeleteSupplierTests {
    ...
    void deleteSupplier_shouldFail_whenHasPurchaseRecords() {
      // Given
      Supplier supplier = Supplier.builder().id(TEST_SUPPLIER_ID).name("测试供应商").build();
      ...
      BusinessException exception =
          assertThrows(
              BusinessException.class, () -> supplierAppService.deleteSupplier(TEST_SUPPLIER_ID));
  ...
  class SupplierStatisticsTests {
    ...
    void getSupplierStatistics_shouldSuccess() {
      // Given
      when(supplierRepository.countByStatus())
          .thenReturn(
              List.of(
                  Map.of("status", "ACTIVE", "count", 80L),
                  Map.of("status", "INACTIVE", "count", 20L)));
      when(supplierRepository.countByRating())
          .thenReturn(
              List.of(
