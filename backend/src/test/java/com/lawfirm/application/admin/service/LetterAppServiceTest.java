...
/** LetterAppService 单元测试 测试出函管理服务的核心功能 */
...
class LetterAppServiceTest {
  ...
  @Mock private LetterTemplateRepository templateRepository;

  @Mock private LetterApplicationRepository applicationRepository;

  @Mock private LetterApplicationMapper applicationMapper;

  @Mock private MatterRepository matterRepository;

  @Mock private MatterClientRepository matterClientRepository;
  ...
  @Mock private com.lawfirm.application.admin.util.LetterTemplateFormatter letterTemplateFormatter;

  @InjectMocks private LetterAppService letterAppService;
  ...
  class TemplateManagementTests {
    ...
    void createTemplate_shouldSuccess() {
      // Given
      when(templateRepository.save(any(LetterTemplate.class)))
          .thenAnswer(
              invocation -> {
      ...
      // When
      LetterTemplateDTO result =
          letterAppService.createTemplate("律师函模板", "LAWYER_LETTER", "内容", "描述");
    ...
    void updateTemplate_shouldSuccess() {
      // Given
      LetterTemplate template = LetterTemplate.builder().id(TEST_TEMPLATE_ID).name("原名称").build();
      ...
      // When
      LetterTemplateDTO result =
          letterAppService.updateTemplate(TEST_TEMPLATE_ID, "新名称", null, null, null);
    ...
    void listActiveTemplates_shouldSuccess() {
      ...
      com.baomidou.mybatisplus.core.conditions.Wrapper<
              com.lawfirm.domain.admin.entity.LetterTemplate>
          wrapper = any(com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper.class);
      when(templateRepository.list(wrapper)).thenReturn(Collections.singletonList(template));
  ...
  class CreateApplicationTests {
    ...
    void createApplication_shouldSuccess() {
      ...
      when(applicationRepository.save(any(LetterApplication.class)))
          .thenAnswer(
              invocation -> {
    ...
    void createApplication_shouldFail_whenMatterNotActive() {
      ...
      LetterTemplate template = LetterTemplate.builder().id(TEST_TEMPLATE_ID).build();
      ...
      BusinessException exception =
          assertThrows(BusinessException.class, () -> letterAppService.createApplication(command));
  ...
  class ApproveApplicationTests {
    ...
    void approve_shouldFail_whenNotPending() {
      // Given
      LetterApplication application =
          LetterApplication.builder().id(TEST_APPLICATION_ID).status(LetterStatus.APPROVED).build();
      ...
      BusinessException exception =
          assertThrows(
              BusinessException.class, () -> letterAppService.approve(TEST_APPLICATION_ID, ""));
  ...
  class QueryApplicationTests {
    ...
    void listByMatter_shouldSuccess() {
      // Given
      LetterApplication application =
          LetterApplication.builder().id(TEST_APPLICATION_ID).matterId(TEST_MATTER_ID).build();
