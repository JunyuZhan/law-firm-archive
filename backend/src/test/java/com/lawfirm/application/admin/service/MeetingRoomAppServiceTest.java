...
/** MeetingRoomAppService 单元测试 测试会议室管理服务的核心功能 */
...
class MeetingRoomAppServiceTest {
  ...
  @Mock private MeetingBookingRepository meetingBookingRepository;

  @Mock private MeetingBookingMapper meetingBookingMapper;

  @Mock private ObjectMapper objectMapper;

  @InjectMocks private MeetingRoomAppService meetingRoomAppService;
  ...
  class CreateRoomTests {
    ...
    void createRoom_shouldFail_whenCodeExists() {
      ...
      MeetingRoom existing = MeetingRoom.builder().id(999L).code("ROOM-EXISTS").build();

      when(meetingRoomMapper.selectByCode("ROOM-EXISTS")).thenReturn(existing);

      // When & Then
      BusinessException exception =
          assertThrows(BusinessException.class, () -> meetingRoomAppService.createRoom(command));
  ...
  class UpdateRoomTests {
    ...
    void updateRoom_shouldSuccess() {
      // Given
      MeetingRoom room =
          MeetingRoom.builder().id(TEST_ROOM_ID).name("原名称").code("ROOM-A").capacity(10).build();
    ...
    void updateRoom_shouldFail_whenCodeExists() {
      // Given
      MeetingRoom room = MeetingRoom.builder().id(TEST_ROOM_ID).code("ROOM-A").build();

      MeetingRoom existing = MeetingRoom.builder().id(999L).code("ROOM-B").build();
  ...
  class DeleteRoomTests {
    ...
    void deleteRoom_shouldSuccess() {
      // Given
      MeetingRoom room = MeetingRoom.builder().id(TEST_ROOM_ID).name("会议室A").enabled(true).build();

      when(meetingRoomRepository.getByIdOrThrow(eq(TEST_ROOM_ID), anyString())).thenReturn(room);
      when(meetingBookingMapper.countConflicting(eq(TEST_ROOM_ID), any(), any(), any()))
          .thenReturn(0);
      when(meetingRoomRepository.updateById(any(MeetingRoom.class))).thenReturn(true);
    ...
    void deleteRoom_shouldFail_whenHasBookings() {
      // Given
      MeetingRoom room = MeetingRoom.builder().id(TEST_ROOM_ID).name("会议室A").build();
      ...
      BusinessException exception =
          assertThrows(
              BusinessException.class, () -> meetingRoomAppService.deleteRoom(TEST_ROOM_ID));
  ...
  class BookingTests {
    ...
    void bookMeeting_shouldSuccess() {
      ...
      when(meetingBookingMapper.countConflicting(eq(TEST_ROOM_ID), any(), any(), any()))
          .thenReturn(0);
      when(meetingBookingRepository.save(any(MeetingBooking.class)))
          .thenAnswer(
              invocation -> {
    ...
    void bookMeeting_shouldFail_whenTimeConflict() {
      ...
      // When & Then
      BusinessException exception =
          assertThrows(BusinessException.class, () -> meetingRoomAppService.bookMeeting(command));
    ...
    void bookMeeting_shouldFail_whenEndBeforeStart() {
      ...
      MeetingRoom room = MeetingRoom.builder().id(TEST_ROOM_ID).enabled(true).build();
  ...
  class QueryBookingTests {
    ...
    void listBookings_shouldSuccess() {
      ...
      Page<MeetingBooking> pageParam = any(Page.class);
      when(meetingBookingMapper.selectBookingPage(pageParam, any(), any(), any(), any(), any()))
          .thenReturn(page);
