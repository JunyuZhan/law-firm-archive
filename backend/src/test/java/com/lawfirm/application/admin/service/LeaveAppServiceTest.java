...
/** LeaveAppService 单元测试 测试请假管理服务的核心功能 */
...
class LeaveAppServiceTest {
  ...
  @InjectMocks private LeaveAppService leaveAppService;
  ...
  class QueryApplicationTests {
    ...
    void listApplications_shouldSuccess() {
      ...
      LeaveType type = LeaveType.builder().id(TEST_LEAVE_TYPE_ID).name("年假").build();
      ...
      Page<LeaveApplication> pageParam = any(Page.class);
      when(leaveApplicationMapper.selectApplicationPage(
              pageParam, any(), any(), any(), any(), any()))
          .thenReturn(page);
  ...
  class ApplyLeaveTests {
    ...
    void applyLeave_shouldSuccess() {
      ...
      when(leaveTypeRepository.getByIdOrThrow(eq(TEST_LEAVE_TYPE_ID), anyString()))
          .thenReturn(leaveType);
      when(leaveApplicationMapper.countOverlapping(eq(TEST_USER_ID), any(), any())).thenReturn(0);
      when(leaveBalanceMapper.selectByUserTypeYear(
              eq(TEST_USER_ID), eq(TEST_LEAVE_TYPE_ID), anyInt()))
          .thenReturn(balance);
    ...
    void applyLeave_shouldFail_whenStartTimeAfterEndTime() {
      ...
      LeaveType leaveType = LeaveType.builder().id(TEST_LEAVE_TYPE_ID).enabled(true).build();
      ...
      BusinessException exception =
          assertThrows(BusinessException.class, () -> leaveAppService.applyLeave(command));
    ...
    void applyLeave_shouldFail_whenTimeOverlapping() {
      ...
      when(leaveTypeRepository.getByIdOrThrow(eq(TEST_LEAVE_TYPE_ID), anyString()))
          .thenReturn(leaveType);
      when(leaveApplicationMapper.countOverlapping(eq(TEST_USER_ID), any(), any()))
          .thenReturn(1); // 有重叠
  ...
  class ApproveLeaveTests {
    ...
    void approveLeave_shouldSuccess_whenApproved() {
      ...
      LeaveType leaveType =
          LeaveType.builder().id(TEST_LEAVE_TYPE_ID).annualLimit(BigDecimal.valueOf(10)).build();
      ...
      when(leaveApplicationRepository.getByIdOrThrow(eq(TEST_APPLICATION_ID), anyString()))
          .thenReturn(application);
      when(leaveTypeRepository.getById(TEST_LEAVE_TYPE_ID)).thenReturn(leaveType);
      when(leaveBalanceMapper.deductBalance(
              eq(TEST_USER_ID), eq(TEST_LEAVE_TYPE_ID), anyInt(), any()))
          .thenReturn(1);
    ...
    void approveLeave_shouldFail_whenAlreadyProcessed() {
      ...
      // When & Then
      BusinessException exception =
          assertThrows(BusinessException.class, () -> leaveAppService.approveLeave(command));
  ...
  class LeaveBalanceTests {
    ...
    void getUserBalance_shouldUseDefaults() {
      ...
      verify(leaveBalanceMapper)
          .selectByUserAndYear(eq(TEST_USER_ID), eq(LocalDate.now().getYear()));
    }
  }
  ...
  class InitBalanceTests {
    ...
    void initUserBalance_shouldSkipExisting() {
      ...
      LeaveBalance existingBalance = LeaveBalance.builder().leaveTypeId(TEST_LEAVE_TYPE_ID).build();
  ...
  class PendingApplicationsTests {
    ...
    void getPendingApplications_shouldSuccess() {
      ...
      when(leaveApplicationMapper.selectPendingApplications())
