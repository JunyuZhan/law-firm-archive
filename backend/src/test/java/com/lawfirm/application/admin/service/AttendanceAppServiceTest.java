...
/** AttendanceAppService 单元测试 测试考勤管理服务的核心功能 */
...
class AttendanceAppServiceTest {
  ...
  @InjectMocks private AttendanceAppService attendanceAppService;
  ...
  class QueryAttendanceTests {
    ...
    void listAttendance_shouldSuccess() {
      ...
      Page<Attendance> pageParam = any(Page.class);
      when(attendanceMapper.selectAttendancePage(pageParam, any(), any(), any(), any()))
          .thenReturn(page);
  ...
  class CheckInTests {
          ...
          .thenAnswer(
              invocation -> {
                ...
                if (att.getCheckInTime() != null
                    && att.getCheckInTime().toLocalTime().isAfter(LocalTime.of(9, 0))) {
    ...
    void checkIn_shouldFail_whenDuplicate() {
      ...
      BusinessException exception =
          assertThrows(BusinessException.class, () -> attendanceAppService.checkIn(command));
  ...
  class CheckOutTests {
    ...
    void checkOut_shouldFail_whenNotCheckedIn() {
      ...
      // When & Then
      BusinessException exception =
          assertThrows(BusinessException.class, () -> attendanceAppService.checkOut(command));
  ...
  class MonthlyStatisticsTests {
    ...
    void getMonthlyStatistics_shouldSuccess() {
      ...
      when(attendanceMapper.countMonthlyAttendance(
              eq(TEST_USER_ID), any(LocalDate.class), any(LocalDate.class)))
          .thenReturn(List.of(stat1, stat2, stat3, stat4));

      // When
      MonthlyAttendanceStatisticsDTO result =
          attendanceAppService.getMonthlyStatistics(TEST_USER_ID, year, month);
    ...
    void getMonthlyStatistics_shouldUseCurrentUser() {
      ...
      // When
      MonthlyAttendanceStatisticsDTO result =
          attendanceAppService.getMonthlyStatistics(null, year, month);
