...
/**
 * DevelopmentPlanAppService 单元测试
 * 测试发展规划服务的核心功能
 */
...
class DevelopmentPlanAppServiceTest {
    ...
    private static final Long TEST_LEVEL_ID = 300L;

    @Mock
    private DevelopmentPlanRepository planRepository;

    @Mock
    private DevelopmentPlanMapper planMapper;

    @Mock
    private DevelopmentMilestoneRepository milestoneRepository;
    ...
    @InjectMocks
    private DevelopmentPlanAppService developmentPlanAppService;
    ...
    class CreatePlanTests {
        ...
        void createPlan_shouldSuccess() throws JsonProcessingException {
            ...
            when(planRepository.findByEmployeeAndYear(eq(TEST_USER_ID), eq(2024))).thenReturn(Optional.empty());
            when(objectMapper.writeValueAsString(anyList())).thenReturn("[\"目标1\",\"目标2\"]");
            when(planRepository.save(any(DevelopmentPlan.class))).thenAnswer(invocation -> {
                DevelopmentPlan plan = invocation.getArgument(0);
                plan.setId(TEST_PLAN_ID);
                plan.setPlanNo("PLAN1234567890");
                return true;
            });
            when(planRepository.getByIdOrThrow(eq(TEST_PLAN_ID), anyString())).thenAnswer(invocation -> {
        ...
        void createPlan_shouldFail_whenYearExists() {
            ...
            BusinessException exception = assertThrows(BusinessException.class,
                    () -> developmentPlanAppService.createPlan(command));
    ...
    class QueryPlanTests {
        ...
        void listPlans_shouldSuccess() {
            ...
            when(planMapper.selectPlanPage(any(Page.class), any(), any(), any(), any())).thenReturn(page);

            // When
            PageResult<DevelopmentPlanDTO> result = developmentPlanAppService.listPlans(1, 10, null, null, null, null);
    ...
    class UpdateMilestoneStatusTests {
        ...
        void updateMilestoneStatus_shouldFail_whenNotFound() {
            ...
            // When & Then
            BusinessException exception = assertThrows(BusinessException.class,
                    () -> developmentPlanAppService.updateMilestoneStatus(TEST_MILESTONE_ID, "COMPLETED", ""));
