...
/** PerformanceAppService 单元测试 测试绩效考核服务的核心功能 */
...
class PerformanceAppServiceTest {
  ...
  @Mock private PerformanceTaskRepository taskRepository;

  @Mock private PerformanceIndicatorRepository indicatorRepository;

  @Mock private PerformanceEvaluationRepository evaluationRepository;

  @Mock private PerformanceScoreRepository scoreRepository;

  @Mock private UserRepository userRepository;

  @InjectMocks private PerformanceAppService performanceAppService;
  ...
  class TaskManagementTests {
    ...
    void createTask_shouldSuccess() {
      ...
      when(taskRepository.save(any(PerformanceTask.class)))
          .thenAnswer(
              invocation -> {
    ...
    void startTask_shouldSuccess() {
      ...
      PerformanceTask task =
          PerformanceTask.builder().id(TEST_TASK_ID).name("考核任务").status("DRAFT").build();
    ...
    void startTask_shouldFail_whenNotDraft() {
      // Given
      PerformanceTask task =
          PerformanceTask.builder().id(TEST_TASK_ID).status("IN_PROGRESS").build();
      ...
      BusinessException exception =
          assertThrows(
              BusinessException.class, () -> performanceAppService.startTask(TEST_TASK_ID));
    ...
    void completeTask_shouldFail_whenCompleted() {
      // Given
      PerformanceTask task = PerformanceTask.builder().id(TEST_TASK_ID).status("COMPLETED").build();
      ...
      BusinessException exception =
          assertThrows(
              BusinessException.class, () -> performanceAppService.completeTask(TEST_TASK_ID));
  ...
  class IndicatorManagementTests {
    ...
    void createIndicator_shouldSuccess() {
      ...
      when(indicatorRepository.save(any(PerformanceIndicator.class)))
          .thenAnswer(
              invocation -> {
    ...
    void updateIndicator_shouldSuccess() {
      ...
      // When
      PerformanceIndicatorDTO result =
  ...
  class EvaluationTests {
    ...
    void submitEvaluation_shouldFail_whenAlreadyCompleted() {
      ...
      PerformanceEvaluation existing =
          PerformanceEvaluation.builder().id(TEST_EVALUATION_ID).status("COMPLETED").build();

      securityUtilsMock.when(() -> SecurityUtils.getUserId()).thenReturn(TEST_USER_ID);
      securityUtilsMock.when(() -> SecurityUtils.getCurrentUserId()).thenReturn(TEST_USER_ID);
      when(userRepository.getById(TEST_USER_ID)).thenReturn(new User());
      when(evaluationRepository.findByTaskEmployeeAndType(any(), any(), any()))
          .thenReturn(existing);

      // When & Then
      BusinessException exception =
          assertThrows(
              BusinessException.class, () -> performanceAppService.submitEvaluation(command));
  ...
  class QueryTests {
    ...
    void listTasks_shouldSuccess() {
      ...
      Page<PerformanceTask> pageParam = any(Page.class);
      when(taskRepository.findPage(pageParam, any(), any(), any())).thenReturn(page);

      // When
      PageResult<PerformanceTaskDTO> result =
          performanceAppService.listTasks(query, null, null, null);
