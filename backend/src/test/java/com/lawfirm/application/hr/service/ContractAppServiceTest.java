...
class ContractAppServiceTest {
    ...
    class CreateContractTests {
        ...
        void createContract_shouldSuccess() {
            ...
            when(employeeRepository.getByIdOrThrow(eq(TEST_EMPLOYEE_ID), anyString())).thenReturn(employee);
        ...
        void createContract_shouldFail_whenStartAfterEnd() {
            ...
            BusinessException exception = assertThrows(BusinessException.class,
                    () -> contractAppService.createContract(command));
    ...
    class QueryContractTests {
        ...
        void listContracts_shouldSuccess() {
            ...
            when(contractMapper.selectContractPage(any(Page.class), any(), any(), any())).thenReturn(page);
        ...
        void listContracts_shouldFilter_whenNotAdmin() {
            ...
            when(contractMapper.selectContractPage(any(Page.class), eq(TEST_EMPLOYEE_ID), any(), any())).thenReturn(page);
            ...
            // Then
            assertThat(result).isNotNull();
            verify(contractMapper).selectContractPage(any(Page.class), eq(TEST_EMPLOYEE_ID), any(), any());
        }
        ...
        void getContractById_shouldSuccess() {
            ...
            when(contractRepository.getByIdOrThrow(eq(TEST_CONTRACT_ID), anyString())).thenReturn(contract);
        ...
        void getContractById_shouldFail_whenNotOwner() {
            ...
            // When & Then
            BusinessException exception = assertThrows(BusinessException.class,
                    () -> contractAppService.getContractById(TEST_CONTRACT_ID));
    ...
    class RenewContractTests {
        ...
        void renewContract_shouldSuccess() {
            ...
            when(contractRepository.getByIdOrThrow(eq(TEST_CONTRACT_ID), anyString())).thenReturn(oldContract);
            when(contractRepository.findByContractNo(anyString())).thenReturn(Optional.empty());
            when(contractRepository.save(any(Contract.class))).thenAnswer(invocation -> {
            ...
            // When
            ContractDTO result = contractAppService.renewContract(TEST_CONTRACT_ID, LocalDate.now().plusDays(1), LocalDate.now().plusYears(3));
