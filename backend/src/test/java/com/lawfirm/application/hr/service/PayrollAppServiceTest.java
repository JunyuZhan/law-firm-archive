...
/** PayrollAppService 单元测试 测试薪资管理服务的核心功能 */
...
class PayrollAppServiceTest {
  ...
  @Mock private PayrollSheetRepository payrollSheetRepository;

  @Mock private PayrollSheetMapper payrollSheetMapper;

  @Mock private PayrollItemRepository payrollItemRepository;

  @Mock private NotificationAppService notificationAppService;

  @Mock private PayrollIncomeRepository payrollIncomeRepository;
  ...
  @InjectMocks private PayrollAppService payrollAppService;
  ...
  class CreatePayrollSheetTests {
    ...
    void createPayrollSheet_shouldSuccess() {
      ...
      when(payrollSheetRepository.save(any(PayrollSheet.class)))
          .thenAnswer(
              invocation -> {
      ...
      @SuppressWarnings("unchecked")
      com.baomidou.mybatisplus.extension.conditions.query.LambdaQueryChainWrapper<Employee>
          queryWrapper =
              mock(
      ...
      when(payrollSheetRepository.getByIdOrThrow(eq(TEST_SHEET_ID), anyString()))
          .thenAnswer(
              invocation -> {
    ...
    void createPayrollSheet_shouldFail_whenExists() {
      ...
      PayrollSheet existing =
          PayrollSheet.builder().id(999L).payrollYear(2024).payrollMonth(1).build();
      ...
      BusinessException exception =
          assertThrows(
              BusinessException.class, () -> payrollAppService.createPayrollSheet(command));
  ...
  class QueryPayrollSheetTests {
    ...
    void listPayrollSheets_shouldSuccess() {
      ...
      securityUtilsMock
          .when(() -> SecurityUtils.hasAnyRole("ADMIN", "DIRECTOR", "FINANCE"))
          .thenReturn(true);
      ...
      @SuppressWarnings("unchecked")
      Page<PayrollSheet> pageParam = any(Page.class);
      when(payrollSheetMapper.selectPayrollSheetPage(pageParam, any(), any(), any(), any()))
          .thenReturn(page);
    ...
    void getPayrollSheetById_shouldSuccess() {
      // Given
      PayrollSheet sheet =
          PayrollSheet.builder().id(TEST_SHEET_ID).payrollNo("GZ202401001").build();
      ...
      when(payrollItemRepository.findByPayrollSheetId(TEST_SHEET_ID))
          .thenReturn(Collections.emptyList());
      // Mock PayrollItemService.toItemDTO for empty list (won't be called, but safe)
      lenient().when(payrollItemService.toItemDTO(any())).thenReturn(null);
  ...
  class ConfirmPayrollTests {
    ...
    void confirmPayroll_shouldSuccess() {
      ...
      // Then
      verify(payrollItemService).confirmPayrollItem(command);
    }
    ...
    void confirmPayroll_shouldFail_whenNotOwner() {
      ...
      doThrow(new BusinessException("只能确认自己的工资"))
          .when(payrollItemService)
          .confirmPayrollItem(any(ConfirmPayrollCommand.class));

      // When & Then
      BusinessException exception =
          assertThrows(
              BusinessException.class, () -> payrollAppService.confirmPayrollItem(command));
  ...
  class ApprovePayrollTests {
    ...
    void approvePayroll_shouldFail_whenNotPending() {
      ...
      PayrollSheet sheet =
          PayrollSheet.builder().id(TEST_SHEET_ID).status(PayrollStatus.APPROVED).build();
      ...
      BusinessException exception =
          assertThrows(
              BusinessException.class, () -> payrollAppService.approvePayrollSheet(command));
  ...
  class IssuePayrollTests {
    ...
    void issuePayroll_shouldFail_whenAlreadyIssued() {
      ...
      // When & Then
      BusinessException exception =
          assertThrows(BusinessException.class, () -> payrollAppService.issuePayroll(command));
    ...
    void issuePayroll_shouldFail_whenNotApproved() {
      ...
      PayrollSheet sheet =
          PayrollSheet.builder().id(TEST_SHEET_ID).status(PayrollStatus.PENDING_APPROVAL).build();
