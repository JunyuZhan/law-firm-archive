...
/**
 * EmployeeAppService 单元测试
 * 测试HR模块员工档案管理核心业务逻辑：员工档案创建、查询、更新、删除等
 * 
 * 状态：进行中（另一个工程师在同时工作，请注意协调）
 */
...
class EmployeeAppServiceTest {
    ...
    @Mock
    private ContractRepository hrContractRepository;
    ...
    void setUp() {
        ...
        payrollQueryWrapper = mock(LambdaQueryChainWrapper.class);
    ...
    class ListEmployeesTests {
        ...
        void listEmployees_Success() {
            ...
            when(employeeMapper.selectEmployeePage(
                    any(Page.class), any(), any(), any(), any(), any()
            )).thenReturn(page);
            
            when(userRepository.findById(eq(100L))).thenReturn(createTestUser());
            ...
            verify(employeeMapper).selectEmployeePage(
                    any(Page.class), any(), any(), any(), any(), any()
            );
        }
    ...
    class GetEmployeeByIdTests {
        ...
        void getEmployeeById_NotFound() {
            ...
            BusinessException exception = assertThrows(BusinessException.class,
                    () -> employeeAppService.getEmployeeById(employeeId));
    ...
    class GetEmployeeByUserIdTests {
        ...
        void getEmployeeByUserId_Success() {
            ...
            when(employeeRepository.findByUserId(eq(userId)))
        ...
        void getEmployeeByUserId_NotFound() {
            ...
            // When & Then
            BusinessException exception = assertThrows(BusinessException.class,
                    () -> employeeAppService.getEmployeeByUserId(userId));
            assertEquals("员工档案不存在", exception.getMessage());
        }
    }
    ...
    class CreateEmployeeTests {
        ...
        void createEmployee_Success() {
            ...
            User user = createTestUser();
            when(userRepository.getByIdOrThrow(eq(100L), anyString()))
                    .thenReturn(user);
            when(userRepository.findById(eq(100L))).thenReturn(user);
            when(employeeRepository.findByUserId(eq(100L)))
                    .thenReturn(Optional.empty());
            when(employeeRepository.findByEmployeeNo(anyString()))
                    .thenReturn(Optional.empty());
        ...
        void createEmployee_UserNotFound() {
            ...
            // When & Then
            BusinessException exception = assertThrows(BusinessException.class,
                    () -> employeeAppService.createEmployee(command));
    ...
    class UpdateEmployeeTests {
        ...
        void updateEmployee_Success() {
            ...
            when(employeeRepository.findByEmployeeNo(anyString()))
                    .thenReturn(Optional.empty());
            when(userRepository.findById(eq(100L))).thenReturn(user);
            when(employeeRepository.updateById(any(Employee.class)))
                    .thenReturn(true);
    ...
    class DeleteEmployeeTests {
        ...
        void deleteEmployee_HasPayrollItems() {
            ...
            when(employeeRepository.getByIdOrThrow(eq(employeeId), anyString()))
                    .thenReturn(employee);
            when(payrollItemRepository.lambdaQuery()
                    .eq(PayrollItem::getEmployeeId, employeeId)
                    .eq(PayrollItem::getDeleted, false)
                    .list()).thenReturn(Collections.singletonList(payrollItem));
