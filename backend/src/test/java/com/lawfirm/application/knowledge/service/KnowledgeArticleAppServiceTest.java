...
/** KnowledgeArticleAppService 单元测试 测试知识文章管理服务的核心功能 */
...
class KnowledgeArticleAppServiceTest {
  ...
  @Mock private KnowledgeArticleMapper articleMapper;

  @Mock private ArticleCollectionMapper articleCollectionMapper;

  @InjectMocks private KnowledgeArticleAppService articleAppService;
  ...
  class QueryArticleTests {
    ...
    void listArticles_shouldSuccess() {
      ...
      Page<KnowledgeArticle> pageParam1 = any(Page.class);
      when(articleMapper.selectArticlePage(pageParam1, any(), any(), any(), any()))
          .thenReturn(page);
  ...
  class DeleteArticleTests {
    ...
    void deleteArticle_shouldFail_whenNotOwner() {
      ...
      BusinessException exception =
          assertThrows(
              BusinessException.class, () -> articleAppService.deleteArticle(TEST_ARTICLE_ID));
  ...
  class PublishArticleTests {
    ...
    void publishArticle_shouldFail_whenNotOwner() {
      ...
      BusinessException exception =
          assertThrows(
              BusinessException.class, () -> articleAppService.publishArticle(TEST_ARTICLE_ID));
  ...
  class LikeArticleTests {
    ...
    void likeArticle_shouldSuccess() {
      // Given
      KnowledgeArticle article =
          KnowledgeArticle.builder().id(TEST_ARTICLE_ID).likeCount(10).build();
  ...
  class MyArticlesTests {
    ...
    void getMyArticles_shouldSuccess() {
      ...
      @SuppressWarnings("unchecked")
      Page<KnowledgeArticle> pageParam2 = any(Page.class);
      when(articleMapper.selectArticlePage(pageParam2, eq(TEST_USER_ID), any(), any(), any()))
          .thenReturn(page);
  ...
  class CollectArticleTests {
    ...
    void collectArticle_shouldFail_whenAlreadyCollected() {
      ...
      BusinessException exception =
          assertThrows(
              BusinessException.class, () -> articleAppService.collectArticle(TEST_ARTICLE_ID));
  ...
  class GetCollectedArticlesTests {
    ...
    void getMyCollectedArticles_shouldSuccess() {
      // Given
      KnowledgeArticle article =
          KnowledgeArticle.builder().id(TEST_ARTICLE_ID).title("收藏的文章").build();
