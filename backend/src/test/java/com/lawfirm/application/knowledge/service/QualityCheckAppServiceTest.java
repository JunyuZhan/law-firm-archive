...
class QualityCheckAppServiceTest {
    ...
    @Mock
    private QualityCheckRepository checkRepository;

    @Mock
    private QualityCheckMapper checkMapper;

    @Mock
    private QualityCheckDetailRepository detailRepository;

    @Mock
    private QualityCheckDetailMapper detailMapper;
    ...
    @InjectMocks
    private QualityCheckAppService checkAppService;
    ...
    class CreateCheckTests {
        ...
        void createCheck_shouldSuccess() {
            ...
            CreateQualityCheckCommand.CheckDetailCommand detailCmd = new CreateQualityCheckCommand.CheckDetailCommand();
            ...
            when(checkRepository.save(any(QualityCheck.class))).thenAnswer(invocation -> {
                QualityCheck check = invocation.getArgument(0);
                check.setId(TEST_CHECK_ID);
                return true;
            });
            when(detailRepository.save(any(QualityCheckDetail.class))).thenAnswer(invocation -> {
        ...
        void createCheck_shouldCalculateQualified_whenAllPassAndScoreMet() {
            ...
            when(checkRepository.updateById(any(QualityCheck.class))).thenAnswer(invocation -> {
                QualityCheck check = invocation.getArgument(0);
                // 验证合格计算
