...
/** AiBillingAppService 单元测试 测试AI账单服务的核心功能 */
...
class AiBillingAppServiceTest {
  ...
  @Mock private AiMonthlyBillRepository billRepository;
  ...
  @InjectMocks private AiBillingAppService aiBillingAppService;
  ...
  void setUp() {
    ...
    when(sysConfigAppService.getConfigValue(anyString())).thenReturn("100");
    when(departmentRepository.getById(anyLong())).thenReturn(null);
  }
  ...
  class QueryBillTests {
    ...
    void getUserBill_shouldFail_forOtherUser() {
      ...
      // When & Then
      assertThrows(
          BusinessException.class, () -> aiBillingAppService.getUserBill(otherUserId, 2024, 1));
    }
    ...
    void getUserBill_shouldSuccess_forAdmin() {
      ...
      AiMonthlyBill bill = AiMonthlyBill.builder().id(TEST_BILL_ID).userId(otherUserId).build();
  ...
  class GenerateBillTests {
    ...
    void generateMonthlyBills_shouldFail_forNonAdmin() {
      ...
      // When & Then
      assertThrows(
          BusinessException.class, () -> aiBillingAppService.generateMonthlyBills(2024, 1));
    }
  ...
  class BillOperationTests {
    ...
    void markDeducted_shouldFail_forNonAdmin() {
      ...
      // When & Then
      assertThrows(
          BusinessException.class, () -> aiBillingAppService.markDeducted(TEST_BILL_ID, "备注"));
    }
    ...
    void waiveBill_shouldSuccess_forDirector() {
      ...
      AiMonthlyBill bill =
          AiMonthlyBill.builder().id(TEST_BILL_ID).deductionStatus("PENDING").build();
    ...
    void waiveBill_shouldFail_forFinanceOnly() {
      ...
      // When & Then
      assertThrows(
          BusinessException.class, () -> aiBillingAppService.waiveBill(TEST_BILL_ID, "原因"));
    }
  }
  ...
  class StatusMappingTests {
    ...
    void deductionStatus_shouldMapDeducted() {
      // Given
      AiMonthlyBill bill =
          AiMonthlyBill.builder().id(TEST_BILL_ID).deductionStatus("DEDUCTED").build();
    ...
    void deductionStatus_shouldMapWaived() {
      // Given
      AiMonthlyBill bill =
          AiMonthlyBill.builder().id(TEST_BILL_ID).deductionStatus("WAIVED").build();
