...
class AiUsageAppServiceTest {
    ...
    @InjectMocks
    private AiUsageAppService aiUsageAppService;
    ...
    class QueryUsageTests {
        ...
        void getMyUsageSummary_shouldSuccess() {
            ...
            Map<String, Object> stats = Map.of(
                    "total_calls", 100,
                    "total_tokens", 50000L,
                    "prompt_tokens", 30000L,
                    "completion_tokens", 20000L,
                    "total_cost", new BigDecimal("10.00"),
                    "user_cost", new BigDecimal("5.00"),
                    "company_cost", new BigDecimal("5.00"),
                    "charge_ratio", 50);

            AiUserQuota quota = AiUserQuota.builder()
                    .userId(TEST_USER_ID)
        ...
        void getUsageSummary_shouldSuccess() {
            ...
            Map<String, Object> stats = Map.of(
                    "total_calls", 50,
                    "total_tokens", 25000L,
                    "prompt_tokens", 15000L,
                    "completion_tokens", 10000L,
                    "total_cost", new BigDecimal("5.00"),
                    "user_cost", new BigDecimal("2.50"),
                    "company_cost", new BigDecimal("2.50"),
                    "charge_ratio", 50);

            when(usageLogRepository.getSummary(anyLong(), any(), any())).thenReturn(stats);
            when(quotaRepository.findByUserId(TEST_USER_ID)).thenReturn(null);
            when(userRepository.findById(TEST_USER_ID)).thenReturn(
                    User.builder().id(TEST_USER_ID).realName("用户").build());
    ...
    class AdminQueryTests {
        ...
        void getAllUsersSummary_shouldFail_forNonAdmin() {
            ...
            assertThrows(BusinessException.class,
                    () -> aiUsageAppService.getAllUsersSummary(YearMonth.now()));
        }
        ...
        void getDepartmentSummary_shouldFail_forNonAdmin() {
            ...
            // When & Then
            assertThrows(BusinessException.class,
                    () -> aiUsageAppService.getDepartmentSummary(YearMonth.now()));
        }
    }
    ...
    class CalculationTests {
        ...
        void calculateAvgTokensPerCall_shouldCalculateCorrectly() {
            ...
            when(usageLogRepository.getSummary(anyLong(), any(), any())).thenReturn(stats);
            when(quotaRepository.findByUserId(TEST_USER_ID)).thenReturn(null);
            when(userRepository.findById(TEST_USER_ID)).thenReturn(User.builder().id(TEST_USER_ID).build());
