...
class ArchiveBorrowAppServiceTest {
    ...
    @Mock
    private ArchiveBorrowRepository borrowRepository;
    ...
    @Mock
    private ArchiveOperationLogRepository operationLogRepository;

    @InjectMocks
    private ArchiveBorrowAppService borrowAppService;
    ...
    class QueryBorrowTests {
        ...
        void listBorrows_shouldSuccess() {
            ...
            when(borrowRepository.page(any(Page.class), any(LambdaQueryWrapper.class)))
                    .thenReturn(page);
    ...
    class CreateBorrowTests {
        ...
        void createBorrow_shouldSuccess() {
            ...
            when(borrowRepository.count(any(LambdaQueryWrapper.class))).thenReturn(0L);
            when(borrowRepository.save(any(ArchiveBorrow.class))).thenAnswer(invocation -> {
        ...
        void createBorrow_shouldFail_whenArchiveNotStored() {
            ...
            BusinessException exception = assertThrows(BusinessException.class,
                    () -> borrowAppService.createBorrow(command));
        ...
        void createBorrow_shouldFail_whenHasUnreturnedBorrow() {
            ...
            Archive archive = Archive.builder()
                    .id(TEST_ARCHIVE_ID)
                    .status(ArchiveBorrowStatus.ARCHIVE_STORED)
                    .build();

            when(archiveRepository.getByIdOrThrow(eq(TEST_ARCHIVE_ID), anyString())).thenReturn(archive);
            when(borrowRepository.count(any(LambdaQueryWrapper.class))).thenReturn(1L); // 有未归还记录
