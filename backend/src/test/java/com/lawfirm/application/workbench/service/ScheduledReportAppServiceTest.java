...
/**
 * ScheduledReportAppService 单元测试
 * 测试定时报表服务的核心功能
 */
...
class ScheduledReportAppServiceTest {
    ...
    @Mock
    private ScheduledReportLogRepository logRepository;

    @Mock
    private ScheduledReportLogMapper logMapper;

    @Mock
    private ReportTemplateRepository templateRepository;

    @Mock
    private CustomReportAppService customReportAppService;
    ...
    @InjectMocks
    private ScheduledReportAppService scheduledReportAppService;
    ...
    class CreateScheduledReportTests {
        ...
        void createScheduledReport_shouldSuccess_forDaily() {
            ...
            when(scheduledReportRepository.save(any(ScheduledReport.class))).thenAnswer(invocation -> {
    ...
    class UpdateScheduledReportTests {
        ...
        void updateScheduledReport_shouldSuccess() {
            ...
            when(scheduledReportRepository.getByIdOrThrow(eq(TEST_TASK_ID), anyString())).thenReturn(task);
            when(templateRepository.getByIdOrThrow(eq(TEST_TEMPLATE_ID), anyString())).thenReturn(template);
            lenient().when(scheduledReportRepository.updateById(any(ScheduledReport.class))).thenReturn(true);
            lenient().when(templateRepository.findById(TEST_TEMPLATE_ID)).thenReturn(template);

            // When
            ScheduledReportDTO result = scheduledReportAppService.updateScheduledReport(TEST_TASK_ID, command);
    ...
    class QueryScheduledReportTests {
        ...
        void listScheduledReports_shouldSuccess() {
            ...
            when(scheduledReportMapper.selectScheduledReportPage(any(Page.class), any(), any(), any()))
                    .thenReturn(page);
    ...
    class QueryExecuteLogsTests {
        ...
        void listExecuteLogs_shouldSuccess() {
            // Given
            ScheduledReportLog log = ScheduledReportLog.builder()
                    .id(1L)
                    .taskId(TEST_TASK_ID)
                    .taskNo("TASK2024001")
                    .status("SUCCESS")
            ...
            when(logMapper.selectLogPage(any(Page.class), any(), any())).thenReturn(page);
