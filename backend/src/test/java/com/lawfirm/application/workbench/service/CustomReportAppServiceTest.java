...
class CustomReportAppServiceTest {
    ...
    class QueryTemplateTests {
        ...
        void listTemplates_shouldSuccess() {
            ...
            when(templateMapper.selectTemplatePage(any(Page.class), any(), any(), any(), eq(TEST_USER_ID)))
                    .thenReturn(page);

            // When
            PageResult<ReportTemplateDTO> result = customReportAppService.listTemplates(1, 10, null, null, null);
    ...
    class CreateTemplateTests {
        ...
        void createTemplate_shouldSuccess() {
            ...
            when(templateRepository.save(any(ReportTemplate.class))).thenAnswer(invocation -> {
        ...
        void createTemplate_shouldFail_whenNameExists() {
            ...
            BusinessException exception = assertThrows(BusinessException.class,
                    () -> customReportAppService.createTemplate(command));
    ...
    class GenerateReportTests {
        ...
        void generateReportByTemplate_shouldSuccess() {
            ...
            // When
            ReportDTO result = customReportAppService.generateReportByTemplate(TEST_TEMPLATE_ID, parameters, "EXCEL");
    ...
    class DataSourceTests {
        ...
        void getDataSources_shouldSuccess() {
            ...
            // 验证包含常见的数据源
            boolean hasMatterDataSource = result.stream()
                    .anyMatch(ds -> "MATTER".equals(ds.get("code")));
        ...
        void getDataSourceFields_shouldSuccess_forMatter() {
            ...
            assertThat(result.size()).isGreaterThan(0);
            // 验证包含常见字段
            boolean hasMatterNo = result.stream()
                    .anyMatch(f -> "matter_no".equals(f.get("field")));
