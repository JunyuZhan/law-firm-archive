...
class WorkbenchAppServiceTest {
    ...
    class GetWorkbenchDataTests {
        ...
        void getWorkbenchData_shouldReturnCompleteData() {
            ...
            lenient().when(matterMapper.countByUserIdAndStatus(TEST_USER_ID, "IN_PROGRESS")).thenReturn(10);
            lenient().when(matterMapper.countByUserIdAndStatus(TEST_USER_ID, "PENDING_APPROVAL")).thenReturn(2);
            lenient().when(matterMapper.countByUserIdAndStatus(TEST_USER_ID, "CLOSED")).thenReturn(5);
            lenient().when(timesheetMapper.sumHoursByUserIdAndDateRange(anyLong(), any(LocalDate.class), any(LocalDate.class)))
                    .thenReturn(BigDecimal.valueOf(160));
            lenient().when(timesheetMapper.sumBillableHoursByUserIdAndDateRange(anyLong(), any(LocalDate.class), any(LocalDate.class)))
                    .thenReturn(BigDecimal.valueOf(120));
            lenient().when(timesheetMapper.countByUserIdAndDateRange(anyLong(), any(LocalDate.class), any(LocalDate.class)))
                    .thenReturn(20);
            lenient().when(approvalRepository.findPendingByApproverId(TEST_USER_ID, 5)).thenReturn(Collections.emptyList());
            lenient().when(taskRepository.findPendingByAssigneeId(TEST_USER_ID, 5)).thenReturn(Collections.emptyList());
            lenient().when(scheduleRepository.findByUserAndDate(TEST_USER_ID, LocalDate.now())).thenReturn(Collections.emptyList());
            lenient().when(matterMapper.selectRecentByUserId(TEST_USER_ID, 5)).thenReturn(Collections.emptyList());
        ...
        void getWorkbenchData_shouldReturnDefaults_whenPartialFailure() {
            ...
            when(timesheetMapper.sumHoursByUserIdAndDateRange(anyLong(), any(), any()))
                    .thenReturn(BigDecimal.valueOf(160));
            when(approvalRepository.findPendingByApproverId(TEST_USER_ID, 5)).thenReturn(Collections.emptyList());
            when(taskRepository.findPendingByAssigneeId(TEST_USER_ID, 5)).thenReturn(Collections.emptyList());
            when(scheduleRepository.findByUserAndDate(TEST_USER_ID, LocalDate.now())).thenReturn(Collections.emptyList());
    ...
    class GetTimesheetSummaryTests {
        ...
        void getTimesheetSummary_shouldCalculateCorrectly() {
            ...
            when(timesheetMapper.sumHoursByUserIdAndDateRange(eq(TEST_USER_ID), eq(startOfMonth), eq(endOfMonth)))
                    .thenReturn(BigDecimal.valueOf(160));
            when(timesheetMapper.sumBillableHoursByUserIdAndDateRange(eq(TEST_USER_ID), eq(startOfMonth), eq(endOfMonth)))
                    .thenReturn(BigDecimal.valueOf(120));
            when(timesheetMapper.countByUserIdAndDateRange(eq(TEST_USER_ID), eq(startOfMonth), eq(endOfMonth)))
                    .thenReturn(20);
        ...
        void getTimesheetSummary_shouldReturnZero_whenNull() {
            ...
            when(timesheetMapper.sumBillableHoursByUserIdAndDateRange(anyLong(), any(), any())).thenReturn(null);
