...
/**
 * StateCompensationAppService 单元测试
 * 测试国家赔偿案件服务的核心功能
 */
...
class StateCompensationAppServiceTest {
    ...
    @Mock
    private MatterStateCompensationRepository stateCompensationRepository;
    ...
    @InjectMocks
    private StateCompensationAppService stateCompensationAppService;
    ...
    class CreateTests {
        ...
        void create_shouldSuccess_forAdminCompensation() {
            ...
            when(stateCompensationRepository.save(any(MatterStateCompensation.class))).thenAnswer(invocation -> {
        ...
        void create_shouldFail_whenInvalidCaseType() {
            ...
            BusinessException exception = assertThrows(BusinessException.class,
                    () -> stateCompensationAppService.create(command));
    ...
    class UpdateTests {
        ...
        void update_shouldSuccess() {
            ...
            when(stateCompensationRepository.getByIdOrThrow(eq(TEST_COMPENSATION_ID), anyString())).thenReturn(entity);
            when(matterRepository.getByIdOrThrow(eq(TEST_MATTER_ID), anyString())).thenReturn(matter);
            lenient().doNothing().when(matterAppService).validateMatterOwnership(TEST_MATTER_ID);
            lenient().when(stateCompensationRepository.updateById(any(MatterStateCompensation.class))).thenReturn(true);
