 ...
 *
 * <p>测试范围： - 创建项目（正常流程、异常情况） - 项目验证逻辑 - 业务规则验证
 *
 * @author junyuzhan
 * @since 2026-01-10
 */
...
class MatterAppServiceTest {
  ...
  @Mock private MatterParticipantMapper participantMapper;

  @Mock private UserRepository userRepository;

  @Mock private DepartmentRepository departmentRepository;

  @Mock private DeadlineAppService deadlineAppService;

  @InjectMocks private MatterAppService matterAppService;
  ...
  class CreateMatterTests {
    ...
    void createMatter_WithoutContract_ShouldThrowException() {
      ...
      BusinessException exception =
          assertThrows(BusinessException.class, () -> matterAppService.createMatter(command));
    ...
    void createMatter_ClientNotFound_ShouldThrowException() {
      ...
      // When & Then
      assertThrows(BusinessException.class, () -> matterAppService.createMatter(validCommand));
    }
    ...
    void createMatter_ContractNotApproved_ShouldThrowException() {
      // Given
      when(clientRepository.getByIdOrThrow(anyLong(), anyString())).thenReturn(mockClient);

      Contract pendingContract = new Contract();
      pendingContract.setId(100L);
      pendingContract.setStatus("PENDING"); // 未审批
      when(contractRepository.getByIdOrThrow(anyLong(), anyString())).thenReturn(pendingContract);
    ...
    void createMatter_ClientMismatch_ShouldThrowException() {
      ...
      when(contractRepository.getByIdOrThrow(anyLong(), anyString())).thenReturn(mismatchContract);
  ...
  class QueryMatterTests {
    ...
    void getMatterById_NotFound_ShouldThrowException() {
      ...
      // When & Then
      assertThrows(BusinessException.class, () -> matterAppService.getMatterById(999L));
    }
  ...
  class StatusChangeTests {
    ...
    void approveCloseMatter_NotFound_ShouldThrowException() {
      ...
      // When & Then
      assertThrows(
          BusinessException.class, () -> matterAppService.approveCloseMatter(999L, true, "同意结案"));
    }
  }
