...
/** TimerAppService 单元测试 测试计时器服务的核心功能 */
...
class TimerAppServiceTest {
  ...
  @Mock private TimesheetAppService timesheetAppService;

  @InjectMocks private TimerAppService timerAppService;
  ...
  class StartTimerTests {
    ...
    void startTimer_shouldSuccess() {
      ...
      Matter matter = Matter.builder().id(TEST_MATTER_ID).name("测试案件").build();
    ...
    void startTimer_shouldFail_whenAlreadyRunning() {
      ...
      TimerSession runningSession =
          TimerSession.builder().id(TEST_SESSION_ID).status("RUNNING").build();
      ...
      try (MockedStatic<SecurityUtils> mockedSecurityUtils = mockStatic(SecurityUtils.class)) {
        ...
        BusinessException exception =
            assertThrows(BusinessException.class, () -> timerAppService.startTimer(command));
  ...
  class PauseTimerTests {
    ...
    void pauseTimer_shouldFail_whenNoRunning() {
      ...
      try (MockedStatic<SecurityUtils> mockedSecurityUtils = mockStatic(SecurityUtils.class)) {
        ...
        // When & Then
        BusinessException exception =
            assertThrows(BusinessException.class, () -> timerAppService.pauseTimer());
  ...
  class ResumeTimerTests {
    ...
    void resumeTimer_shouldFail_whenNoPaused() {
      ...
      try (MockedStatic<SecurityUtils> mockedSecurityUtils = mockStatic(SecurityUtils.class)) {
        ...
        // When & Then
        BusinessException exception =
            assertThrows(BusinessException.class, () -> timerAppService.resumeTimer());
  ...
  class StopTimerTests {
    ...
    void stopTimer_shouldFail_whenNoActive() {
      ...
      try (MockedStatic<SecurityUtils> mockedSecurityUtils = mockStatic(SecurityUtils.class)) {
        ...
        // When & Then
        BusinessException exception =
            assertThrows(BusinessException.class, () -> timerAppService.stopTimer());
        assertThat(exception.getMessage()).contains("没有正在运行的计时器");
      }
    }
  }
