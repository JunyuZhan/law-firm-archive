...
/** ScheduleAppService 单元测试 测试日程管理服务的核心功能 */
...
class ScheduleAppServiceTest {
  ...
  @InjectMocks private ScheduleAppService scheduleAppService;
  ...
  class CreateScheduleTests {
    ...
    void createSchedule_shouldFail_whenEndTimeBeforeStartTime() {
      ...
      BusinessException exception =
          assertThrows(BusinessException.class, () -> scheduleAppService.createSchedule(command));
    ...
    void createSchedule_shouldSetDefaultAllDay() {
      ...
      verify(scheduleRepository)
          .save(argThat(schedule -> schedule.getAllDay() != null && !schedule.getAllDay()));
    }
  }
  ...
  class QueryScheduleTests {
    ...
    void listSchedules_shouldSuccess() {
      ...
      when(scheduleMapper.selectSchedulePage(
              ArgumentMatchers.<Page<Schedule>>any(), eq(TEST_USER_ID), any(), any(), any(), any()))
          .thenReturn(page);
    ...
    void getScheduleById_shouldSuccess() {
      // Given
      Schedule schedule = Schedule.builder().id(1L).title("开庭").userId(TEST_USER_ID).build();
    ...
    void getScheduleById_shouldFail_whenNotOwner() {
      ...
      // When & Then
      BusinessException exception =
          assertThrows(BusinessException.class, () -> scheduleAppService.getScheduleById(1L));
    ...
    void getSchedulesByDate_shouldSuccess() {
      ...
      Schedule schedule = Schedule.builder().id(1L).title("会议").userId(TEST_USER_ID).build();
    ...
    void getMyTodaySchedules_shouldSuccess() {
      // Given
      Schedule schedule = Schedule.builder().id(1L).title("今日会议").userId(TEST_USER_ID).build();
    ...
    void getMyUpcomingSchedules_shouldSuccess() {
      // Given
      Schedule schedule = Schedule.builder().id(1L).title("未来会议").userId(TEST_USER_ID).build();

      when(scheduleMapper.selectUpcomingSchedules(
              eq(TEST_USER_ID), any(LocalDateTime.class), eq(10)))
          .thenReturn(Collections.singletonList(schedule));
  ...
  class UpdateScheduleTests {
    ...
    void updateSchedule_shouldSuccess() {
      // Given
      Schedule schedule = Schedule.builder().id(1L).title("原标题").userId(TEST_USER_ID).build();
      ...
      // When
      ScheduleDTO result =
          scheduleAppService.updateSchedule(1L, "新标题", "新描述", "新地点", null, null, null);
    ...
    void updateSchedule_shouldFail_whenNotOwner() {
      // Given
      Schedule schedule = Schedule.builder().id(1L).userId(OTHER_USER_ID).build();
    ...
    void updateSchedule_shouldOnlyUpdateNonNullFields() {
      // Given
      Schedule schedule =
          Schedule.builder().id(1L).title("原标题").description("原描述").userId(TEST_USER_ID).build();
  ...
  class DeleteScheduleTests {
    ...
    void deleteSchedule_shouldSuccess() {
      // Given
      Schedule schedule = Schedule.builder().id(1L).title("待删除日程").userId(TEST_USER_ID).build();
    ...
    void deleteSchedule_shouldFail_whenNotOwner() {
      ...
      // When & Then
      BusinessException exception =
          assertThrows(BusinessException.class, () -> scheduleAppService.deleteSchedule(1L));
      assertThat(exception.getMessage()).contains("无权操作");
    }
  }
  ...
  class CancelScheduleTests {
    ...
    void cancelSchedule_shouldSuccess() {
      // Given
      Schedule schedule =
          Schedule.builder().id(1L).title("待取消日程").userId(TEST_USER_ID).status("ACTIVE").build();
    ...
    void cancelSchedule_shouldFail_whenNotOwner() {
      ...
      // When & Then
      BusinessException exception =
          assertThrows(BusinessException.class, () -> scheduleAppService.cancelSchedule(1L));
      assertThat(exception.getMessage()).contains("无权操作");
    }
  }
}
