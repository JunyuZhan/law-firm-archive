...
/**
 * ExpenseAppService 单元测试
 * 测试财务模块费用报销核心业务逻辑：费用申请、审批、支付、成本归集与分摊等
 */
...
class ExpenseAppServiceTest {
    ...
    @Mock
    private CostSplitRepository costSplitRepository;
    ...
    void setUp() {
        ...
        // 设置一些常用的 mock 返回值
        when(expenseRepository.getBaseMapper()).thenReturn(mock(com.lawfirm.infrastructure.persistence.mapper.ExpenseMapper.class));
        when(costAllocationRepository.getBaseMapper()).thenReturn(mock(com.lawfirm.infrastructure.persistence.mapper.CostAllocationMapper.class));
        when(costSplitRepository.getBaseMapper()).thenReturn(mock(com.lawfirm.infrastructure.persistence.mapper.CostSplitMapper.class));
    }
    ...
    class CreateExpenseTests {
        ...
        void createExpense_Success() throws Exception {
            ...
            // 模拟插入操作并设置expense的ID
            when(expenseRepository.getBaseMapper().insert(any(Expense.class))).thenAnswer(invocation -> {
            ...
            when(approvalService.createApproval(
                    anyString(), anyLong(), anyString(), anyString(), anyLong(), anyString(), anyString(), anyString()
            )).thenReturn(1L);
            ...
            verify(approvalService).createApproval(anyString(), anyLong(), anyString(), anyString(), anyLong(), anyString(), anyString(), anyString());
        }
        ...
        void createExpense_MatterNotFound() {
            ...
            BusinessException exception = assertThrows(BusinessException.class,
                    () -> expenseAppService.createExpense(command));
    ...
    class ApproveExpenseTests {
        ...
        void approveExpense_Success_Approve() {
            ...
            when(userRepository.findRoleCodesByUserId(eq(1L))).thenReturn(Arrays.asList("ADMIN", "FINANCE"));
        ...
        void approveExpense_NotPending() {
            ...
            // When & Then
            BusinessException exception = assertThrows(BusinessException.class,
                    () -> expenseAppService.approveExpense(command));
    ...
    class AllocateCostTests {
        ...
        void allocateCost_Success() {
            ...
            when(costAllocationRepository.getBaseMapper().insert(any(CostAllocation.class))).thenReturn(1);
            when(expenseRepository.getBaseMapper().updateById(any(Expense.class))).thenReturn(1);
