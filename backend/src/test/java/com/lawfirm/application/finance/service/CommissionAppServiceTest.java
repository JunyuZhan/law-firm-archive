...
/** CommissionAppService 单元测试 测试提成管理服务的核心功能 */
...
class CommissionAppServiceTest {
  ...
  @Mock private CommissionRuleRepository commissionRuleRepository;

  @Mock private CommissionRepository commissionRepository;

  @Mock private CommissionDetailMapper commissionDetailMapper;
  ...
  @Mock private ContractParticipantRepository contractParticipantRepository;

  @Mock private com.lawfirm.domain.client.repository.ClientRepository clientRepository;

  @Mock private com.lawfirm.domain.matter.repository.MatterRepository matterRepository;

  @InjectMocks private CommissionAppService commissionAppService;
  ...
  void setUp() {
    ...
    lenient()
        .when(userRepository.findRoleCodesByUserId(TEST_USER_ID))
  ...
  class CommissionRuleTests {
    ...
    void createCommissionRule_shouldFail_whenCodeExists() {
      ...
      when(commissionRuleRepository.findByRuleCode("RULE001"))
          .thenReturn(Optional.of(existingRule));

      // When & Then
      BusinessException exception =
          assertThrows(
              BusinessException.class, () -> commissionAppService.createCommissionRule(command));
    ...
    void updateCommissionRule_shouldSuccess() {
      // Given
      CommissionRule rule =
          CommissionRule.builder().id(1L).ruleCode("RULE001").ruleName("原名称").build();
    ...
    void deleteCommissionRule_shouldSuccess() {
      // Given
      CommissionRule rule = CommissionRule.builder().id(1L).isDefault(false).build();
    ...
    void deleteCommissionRule_shouldFail_whenDefault() {
      // Given
      CommissionRule rule = CommissionRule.builder().id(1L).isDefault(true).build();
      ...
      BusinessException exception =
          assertThrows(
              BusinessException.class, () -> commissionAppService.deleteCommissionRule(1L));
    ...
    void setDefaultRule_shouldSuccess() {
      // Given
      CommissionRule rule = CommissionRule.builder().id(1L).build();
    ...
    void toggleRule_shouldSuccess() {
      // Given
      CommissionRule rule = CommissionRule.builder().id(1L).active(true).isDefault(false).build();
    ...
    void toggleRule_shouldFail_whenDefaultAndActive() {
      // Given
      CommissionRule rule = CommissionRule.builder().id(1L).active(true).isDefault(true).build();

      when(commissionRuleRepository.findById(1L)).thenReturn(rule);

      // When & Then
      BusinessException exception =
          assertThrows(BusinessException.class, () -> commissionAppService.toggleRule(1L));
  ...
  class ManualCalculateCommissionTests {
    ...
    void manualCalculateCommission_shouldSuccess() {
      ...
      Fee fee = Fee.builder().id(100L).contractId(200L).matterId(300L).build();

      Contract contract = Contract.builder().id(200L).commissionRuleId(1L).build();
      ...
      when(contractParticipantRepository.getByIdOrThrow(eq(1L), anyString()))
      ...
      assertThat(result.get(0).getCommissionAmount())
          .isEqualByComparingTo(BigDecimal.valueOf(4000)); // 10000 * 40%
    ...
    void manualCalculateCommission_shouldFail_whenNoContract() {
      // Given
      Payment payment =
          Payment.builder().id(1L).feeId(100L).status(PaymentStatus.CONFIRMED).build();
    ...
    void manualCalculateCommission_shouldFail_whenAlreadyExists() {
      ...
      Fee fee = Fee.builder().id(100L).contractId(200L).build();

      Contract contract = Contract.builder().id(200L).build();

      Commission existingCommission = Commission.builder().id(1L).build();
      ...
      when(commissionRepository.findByPaymentId(1L))
          .thenReturn(Collections.singletonList(existingCommission));
  ...
  class QueryCommissionRuleTests {
    ...
    void getCommissionRule_shouldSuccess() {
      // Given
      CommissionRule rule =
          CommissionRule.builder().id(1L).ruleCode("RULE001").ruleName("标准规则").build();
    ...
    void getCommissionRule_shouldFail_whenNotFound() {
      ...
      // When & Then
      BusinessException exception =
          assertThrows(BusinessException.class, () -> commissionAppService.getCommissionRule(1L));
    ...
    void listCommissionRules_shouldSuccess() {
      // Given
      CommissionRule rule =
          CommissionRule.builder().id(1L).ruleCode("RULE001").active(true).build();
  ...
  class QueryCommissionTests {
    ...
    void listCommissions_shouldFilterByUserId_whenNotAdmin() {
      ...
      // When & Then
      BusinessException exception =
          assertThrows(BusinessException.class, () -> commissionAppService.listCommissions(query));
    ...
    void getCommission_shouldSuccess() {
      // Given
      Commission commission =
          Commission.builder().id(1L).commissionNo("COM2024001").paymentId(100L).build();
      ...
      lenient()
          .when(commissionBaseMapper.countByCommissionIdAndUserId(1L, TEST_USER_ID))
          .thenReturn(1);
    ...
    void getCommission_shouldFail_whenNoPermission() {
      ...
      Commission commission = Commission.builder().id(1L).build();
      ...
      // When & Then
      BusinessException exception =
          assertThrows(BusinessException.class, () -> commissionAppService.getCommission(1L));
      assertThat(exception.getMessage()).contains("无权查看");
    }
    ...
    void getUserTotalCommission_shouldSuccess() {
      ...
      when(commissionRepository.sumCommissionByUserId(TEST_USER_ID))
    ...
    void getUserTotalCommission_shouldFail_whenNotSelf() {
      ...
      BusinessException exception =
          assertThrows(
              BusinessException.class, () -> commissionAppService.getUserTotalCommission(999L));
      assertThat(exception.getMessage()).contains("无权查看");
    }
  }
  ...
  class CommissionSummaryTests {
    ...
    void getCommissionSummary_shouldSuccess() {
      // Given
      when(userRepository.findRoleCodesByUserId(TEST_USER_ID)).thenReturn(Arrays.asList("ADMIN"));
      when(commissionRepository.sumTotalCommission(anyString(), anyString()))
          .thenReturn(BigDecimal.valueOf(100000));
      when(commissionRepository.sumCommissionByStatus(
              eq(CommissionStatus.APPROVED), anyString(), anyString()))
          .thenReturn(BigDecimal.valueOf(80000));
      when(commissionRepository.sumCommissionByStatus(
              eq(CommissionStatus.PAID), anyString(), anyString()))
          .thenReturn(BigDecimal.valueOf(50000));
      when(commissionRepository.countCommissions(anyString(), anyString())).thenReturn(100L);
      when(commissionRepository.sumCommissionByUser(anyString(), anyString()))
      ...
      // Then
      assertThat(result).isNotNull();
      assertThat((BigDecimal) result.get("totalCommission"))
          .isEqualByComparingTo(BigDecimal.valueOf(100000));
      assertThat((BigDecimal) result.get("approvedCommission"))
          .isEqualByComparingTo(BigDecimal.valueOf(80000));
      assertThat((BigDecimal) result.get("paidCommission"))
  ...
  class ApproveCommissionTests {
    ...
    void approveCommission_shouldSuccess_whenApproved() {
      ...
      lenient()
          .when(userRepository.findRoleCodesByUserId(TEST_USER_ID))
          .thenReturn(Arrays.asList("DIRECTOR"));
    ...
    void approveCommission_shouldSuccess_whenRejected() {
      ...
      Commission commission =
          Commission.builder().id(1L).status(CommissionStatus.CALCULATED).build();
    ...
    void approveCommission_shouldFail_whenNotCalculated() {
      ...
      BusinessException exception =
          assertThrows(
              BusinessException.class, () -> commissionAppService.approveCommission(1L, true, ""));
  ...
  class IssueCommissionTests {
    ...
    void issueCommission_shouldFail_whenNotApproved() {
      ...
      // When & Then
      BusinessException exception =
          assertThrows(BusinessException.class, () -> commissionAppService.issueCommission(1L));
