...
/** InvoiceAppService 单元测试 测试财务模块发票管理核心业务逻辑：开票申请、税额计算、权限控制等 */
...
class InvoiceAppServiceTest {
  ...
  @InjectMocks private InvoiceAppService invoiceAppService;
  ...
  class IssueInvoiceTests {
    ...
    void issueInvoice_Success() {
      ...
      Invoice invoice = Invoice.builder().id(invoiceId).status("PENDING").build();
  ...
  class CancelInvoiceTests {
    ...
    void cancelInvoice_Success() {
      ...
      Invoice invoice =
          Invoice.builder().id(invoiceId).invoiceNo("INV2024010001").status("ISSUED").build();
    ...
    void cancelInvoice_NotIssued() {
      ...
      BusinessException exception =
          assertThrows(
              BusinessException.class, () -> invoiceAppService.cancelInvoice(invoiceId, "测试原因"));
  ...
  class GetInvoiceStatisticsTests {
    ...
    void getInvoiceStatistics_Success_Admin() {
      ...
      List<Map<String, Object>> byClient =
          Arrays.asList(
    ...
    void getInvoiceStatistics_NoPermission() {
      ...
      // When & Then
      BusinessException exception =
          assertThrows(BusinessException.class, () -> invoiceAppService.getInvoiceStatistics());
  ...
  class HelperMethodTests {
    ...
    void getInvoiceTypeName() {
      ...
      Invoice invoice = Invoice.builder().id(1L).invoiceType("SPECIAL").status("PENDING").build();
    ...
    void getUnknownTypeAndStatusName() {
      Invoice invoice =
          Invoice.builder().id(1L).invoiceType("UNKNOWN_TYPE").status("UNKNOWN_STATUS").build();
  ...
  class EdgeCaseTests {
    ...
    void nullValueHandling() {
      ...
      @SuppressWarnings("unchecked")
      Page<Invoice> pageParam3 = any(Page.class);
      @SuppressWarnings("unchecked")
      LambdaQueryWrapper<Invoice> wrapper3 = any(LambdaQueryWrapper.class);
      when(invoiceRepository.page(pageParam3, wrapper3)).thenReturn(page);
