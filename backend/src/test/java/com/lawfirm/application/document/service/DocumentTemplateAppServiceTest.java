...
/** DocumentTemplateAppService 单元测试 测试文档模板管理服务的核心功能 */
...
class DocumentTemplateAppServiceTest {
  ...
  @Mock private DocumentTemplateMapper templateMapper;

  @Mock private TemplateVariableService templateVariableService;

  @Mock private DocumentAppService documentAppService;

  @Mock private DocumentRepository documentRepository;

  @Mock private MinioService minioService;

  @InjectMocks private DocumentTemplateAppService documentTemplateAppService;
  ...
  class UpdateTemplateTests {
    ...
    void updateTemplate_shouldSuccess() {
      ...
      DocumentTemplate template =
          DocumentTemplate.builder().id(TEST_TEMPLATE_ID).name("原名称").status("ACTIVE").build();
      ...
      // When
      DocumentTemplateDTO result =
          documentTemplateAppService.updateTemplate(TEST_TEMPLATE_ID, command);
  ...
  class DeleteTemplateTests {
    ...
    void deleteTemplate_shouldSuccess() {
      // Given
      DocumentTemplate template =
          DocumentTemplate.builder().id(TEST_TEMPLATE_ID).name("模板1").build();
  ...
  class UseTemplateTests {
    ...
    void useTemplate_shouldSuccess() {
      // Given
      DocumentTemplate template =
          DocumentTemplate.builder().id(TEST_TEMPLATE_ID).name("模板1").status("ACTIVE").build();
    ...
    void useTemplate_shouldFail_whenInactive() {
      // Given
      DocumentTemplate template =
          DocumentTemplate.builder().id(TEST_TEMPLATE_ID).status("INACTIVE").build();
  ...
  class GenerateDocumentTests {
    ...
    void generateDocument_shouldSuccess() {
      ...
      try {
        doReturn("http://minio/documents/file.docx")
            .when(minioService)
            .uploadBytes(any(byte[].class), anyString(), anyString());
      } catch (Exception e) {
        // ignore
      }
      lenient().doNothing().when(templateRepository).incrementUseCount(TEST_TEMPLATE_ID);
    ...
    void generateDocument_shouldFail_whenInactive() {
      ...
      BusinessException exception =
          assertThrows(
              BusinessException.class, () -> documentTemplateAppService.generateDocument(command));
      assertThat(exception.getMessage()).contains("已停用");
    }
    ...
    void generateDocument_shouldFail_whenContentEmpty() {
      ...
      when(templateRepository.getByIdOrThrow(eq(TEST_TEMPLATE_ID), anyString()))
          .thenReturn(template);
      when(templateVariableService.collectVariables(TEST_MATTER_ID))
  ...
  class BatchGenerateDocumentsTests {
    ...
    void batchGenerateDocuments_shouldSuccess() {
      // Given
      DocumentTemplate template =
          DocumentTemplate.builder().id(TEST_TEMPLATE_ID).status("ACTIVE").content("模板内容").build();
      ...
      when(documentAppService.getDocumentById(anyLong()))
          .thenAnswer(
              invocation -> {
  ...
  class QueryTemplateTests {
    ...
    void listTemplates_shouldSuccess() {
      ...
      Page<DocumentTemplate> pageParam = any(Page.class);
      when(templateMapper.selectTemplatePage(pageParam, any(), any(), any(), any()))
          .thenReturn(page);
