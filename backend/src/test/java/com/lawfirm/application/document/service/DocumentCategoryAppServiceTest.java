...
/**
 * DocumentCategoryAppService 单元测试
 * 测试文档分类管理服务的核心功能
 */
...
class DocumentCategoryAppServiceTest {
    ...
    @InjectMocks
    private DocumentCategoryAppService documentCategoryAppService;
    ...
    class CreateCategoryTests {
        ...
        void createCategory_shouldSuccess() {
            ...
            when(categoryRepository.save(any(DocumentCategory.class))).thenAnswer(invocation -> {
        ...
        void createCategory_shouldSuccess_whenHasParent() {
            // Given
            DocumentCategory sibling = DocumentCategory.builder()
                    .id(300L)
                    .sortOrder(5)
                    .build();

            when(categoryRepository.findByParentId(TEST_PARENT_ID)).thenReturn(Collections.singletonList(sibling));
            when(categoryRepository.save(any(DocumentCategory.class))).thenAnswer(invocation -> {
                DocumentCategory category = invocation.getArgument(0);
                category.setId(TEST_CATEGORY_ID);
                return true;
            ...
            // When
            DocumentCategoryDTO result = documentCategoryAppService.createCategory("子分类", TEST_PARENT_ID, null);
    ...
    class UpdateCategoryTests {
        ...
        void updateCategory_shouldSuccess() {
            ...
            when(categoryRepository.getByIdOrThrow(eq(TEST_CATEGORY_ID), anyString())).thenReturn(category);
            lenient().when(categoryRepository.updateById(any(DocumentCategory.class))).thenReturn(true);

            // When
            DocumentCategoryDTO result = documentCategoryAppService.updateCategory(
                    TEST_CATEGORY_ID, "新名称", "新描述", 2);
    ...
    class DeleteCategoryTests {
        ...
        void deleteCategory_shouldFail_whenHasChildren() {
            ...
            DocumentCategory child = DocumentCategory.builder()
                    .id(300L)
                    .parentId(TEST_CATEGORY_ID)
                    .build();

            when(categoryRepository.getByIdOrThrow(eq(TEST_CATEGORY_ID), anyString())).thenReturn(category);
            when(categoryRepository.findByParentId(TEST_CATEGORY_ID)).thenReturn(Collections.singletonList(child));
    ...
    class QueryCategoryTests {
        ...
        void getCategoryTree_shouldSuccess() {
            // Given
            DocumentCategory root = DocumentCategory.builder()
                    .id(TEST_CATEGORY_ID)
                    .name("根分类")
                    .parentId(0L)
                    .build();
