...
/** SealAppService 单元测试 测试印章管理服务的核心功能 */
...
class SealAppServiceTest {
  ...
  @Mock private SealMapper sealMapper;

  @InjectMocks private SealAppService sealAppService;
  ...
  class CreateSealTests {
    ...
    void createSeal_shouldSuccess() {
      ...
      lenient()
          .when(sealRepository.save(any(Seal.class)))
          .thenAnswer(
              invocation -> {
  ...
  class UpdateSealTests {
    ...
    void updateSeal_shouldSuccess() {
      // Given
      Seal seal = Seal.builder().id(TEST_SEAL_ID).name("原名称").keeperId(TEST_KEEPER_ID).build();
  ...
  class ChangeSealStatusTests {
    ...
    void changeSealStatus_shouldSuccess() {
      // Given
      Seal seal = Seal.builder().id(TEST_SEAL_ID).name("公章").status("ACTIVE").build();
    ...
    void changeSealStatus_shouldFail_whenDestroyed() {
      // Given
      Seal seal = Seal.builder().id(TEST_SEAL_ID).status("DESTROYED").build();
  ...
  class DeleteSealTests {
    ...
    void deleteSeal_shouldSuccess() {
      // Given
      Seal seal = Seal.builder().id(TEST_SEAL_ID).name("公章").build();
    ...
    void deleteSeal_shouldFail_whenHasPendingApplications() {
      ...
      // When & Then
      BusinessException exception =
          assertThrows(BusinessException.class, () -> sealAppService.deleteSeal(TEST_SEAL_ID));
  ...
  class QuerySealTests {
    ...
    void listSeals_shouldSuccess() {
      ...
      Page<Seal> pageParam = any(Page.class);
      when(sealMapper.selectSealPage(pageParam, any(), any(), any(), any())).thenReturn(page);
