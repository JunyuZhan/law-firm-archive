...
class OperationLogAppServiceTest {
  ...
  @InjectMocks private OperationLogAppService operationLogAppService;
  ...
  class ListModulesAndTypesTests {
    ...
    void listModules_shouldSuccess() {
      ...
      com.baomidou.mybatisplus.extension.conditions.query.LambdaQueryChainWrapper<OperationLog>
          queryWrapper =
              mock(
      ...
      doReturn(queryWrapper)
          .when(queryWrapper)
          .select(any(com.baomidou.mybatisplus.core.toolkit.support.SFunction.class));
      when(queryWrapper.eq(any(), any())).thenReturn(queryWrapper);
      doReturn(queryWrapper)
          .when(queryWrapper)
          .groupBy(any(com.baomidou.mybatisplus.core.toolkit.support.SFunction.class));
  ...
  class StatisticsTests {
    ...
    void getStatistics_shouldSuccess() {
      ...
      when(operationLogRepository.count(ArgumentMatchers.<LambdaQueryWrapper<OperationLog>>any()))
          .thenAnswer(
              invocation -> {
                callCount[0]++;
                if (callCount[0] == 1) {
  ...
  class CleanHistoryLogsTests {
    ...
    void cleanHistoryLogs_shouldSuccess() {
      ...
      when(mapperMock.delete(ArgumentMatchers.<LambdaQueryWrapper<OperationLog>>any()))
          .thenReturn(10);
