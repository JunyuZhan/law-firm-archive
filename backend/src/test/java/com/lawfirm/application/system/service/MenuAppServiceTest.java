...
class MenuAppServiceTest {
  ...
  @InjectMocks private MenuAppService menuAppService;
  ...
  class QueryMenuTests {
    ...
    void getMenuTree_shouldSuccess() {
      // Given
      Menu menu1 = Menu.builder().id(1L).parentId(0L).name("菜单1").menuType("MENU").build();

      Menu menu2 = Menu.builder().id(2L).parentId(1L).name("子菜单1").menuType("MENU").build();
    ...
    void getUserMenuTree_shouldSuccess() {
      // Given
      Menu menu =
          Menu.builder().id(TEST_MENU_ID).parentId(0L).name("用户菜单").menuType("MENU").build();

      when(businessCacheService.getUserMenus(eq(TEST_USER_ID), any()))
          .thenAnswer(
              invocation -> {
  ...
  class UpdateMenuTests {
    ...
    void updateMenu_shouldFail_whenParentIsSelf() {
      // Given
      Menu menu = Menu.builder().id(TEST_MENU_ID).name("测试菜单").build();
      ...
      BusinessException exception =
          assertThrows(BusinessException.class, () -> menuAppService.updateMenu(command));
  ...
  class DeleteMenuTests {
    ...
    void deleteMenu_shouldFail_whenHasChildren() {
      ...
      // When & Then
      BusinessException exception =
          assertThrows(BusinessException.class, () -> menuAppService.deleteMenu(TEST_MENU_ID));
  ...
  class AssignRoleMenusTests {
    ...
    void assignRoleMenus_shouldFail_whenRoleIdNull() {
      ...
      BusinessException exception =
          assertThrows(
              BusinessException.class, () -> menuAppService.assignRoleMenus(null, List.of(1L)));
