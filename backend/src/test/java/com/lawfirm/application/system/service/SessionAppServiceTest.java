...
class SessionAppServiceTest {
    ...
    class CreateSessionTests {
        ...
        void createSession_shouldSuccess() {
            ...
            // When
            UserSession result = sessionAppService.createSession(
                    TEST_USER_ID, "testuser", TEST_TOKEN, "refresh-token",
                    "127.0.0.1", "Mozilla/5.0", "PC", "Chrome", "Windows", "北京"
            );
    ...
    class QuerySessionTests {
        ...
        void listSessions_shouldSuccess_whenAdmin() {
            ...
            when(sessionMapper.selectSessionPage(any(Page.class), any(UserSessionQueryDTO.class)))
                    .thenReturn(page);
            when(userRepository.listByIds(anyList())).thenReturn(List.of(
                    User.builder().id(TEST_USER_ID).realName("测试用户").build()
            ));
    ...
    class ValidateSessionTests {
        ...
        void validateSession_shouldSuccess() {
            ...
            when(sessionRepository.findByToken(TEST_TOKEN))
    ...
    class UpdateLastAccessTimeTests {
        ...
        void updateLastAccessTime_shouldSuccess() {
            // Given
            when(sessionRepository.updateLastAccessTimeByToken(anyString(), any(LocalDateTime.class))).thenReturn(1);

            // When
            sessionAppService.updateLastAccessTime(TEST_TOKEN);

            // Then
            verify(sessionRepository).updateLastAccessTimeByToken(eq(TEST_TOKEN), any(LocalDateTime.class));
        }
    }
}
