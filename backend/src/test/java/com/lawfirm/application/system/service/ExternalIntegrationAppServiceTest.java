...
class ExternalIntegrationAppServiceTest {
  ...
  @Mock private ExternalIntegrationRepository integrationRepository;

  @Mock private ExternalIntegrationMapper integrationMapper;

  @Mock private AesEncryptionService encryptionService;

  @InjectMocks private ExternalIntegrationAppService integrationAppService;
  ...
  class QueryIntegrationTests {
    ...
    void listIntegrations_shouldSuccess() {
      ...
      when(integrationMapper.selectPage(
              ArgumentMatchers.<Page<ExternalIntegration>>any(), any(), any(), any()))
          .thenReturn(page);
    ...
    void listAllIntegrations_shouldSuccess() {
      // Given
      ExternalIntegration integration =
          ExternalIntegration.builder().id(TEST_INTEGRATION_ID).integrationCode("AI-001").build();
    ...
    void listEnabledByType_shouldSuccess() {
      ...
      // When
      List<ExternalIntegrationDTO> result =
          integrationAppService.listEnabledByType(ExternalIntegration.TYPE_AI);
  ...
  class CreateIntegrationTests {
    ...
    void createIntegration_shouldFail_whenCodeExists() {
      ...
      BusinessException exception =
          assertThrows(
              BusinessException.class, () -> integrationAppService.createIntegration(command));
  ...
  class GetDecryptedKeysTests {
    ...
    void getDecryptedApiKey_shouldSuccess() {
      // Given
      ExternalIntegration integration =
          ExternalIntegration.builder().id(TEST_INTEGRATION_ID).apiKey("encrypted-key").build();
