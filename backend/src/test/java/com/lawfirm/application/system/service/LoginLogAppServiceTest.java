...
class LoginLogAppServiceTest {
  ...
  @InjectMocks private LoginLogAppService loginLogAppService;
  ...
  class QueryLoginLogTests {
    ...
    void listLoginLogs_shouldSuccess_whenAdmin() {
      ...
      when(loginLogRepository.page(
              ArgumentMatchers.<Page<LoginLog>>any(),
              ArgumentMatchers.<LambdaQueryWrapper<LoginLog>>any()))
          .thenReturn(page);
    ...
    void getLoginLog_shouldFail_whenNotOwnLog() {
      // Given
      LoginLog log =
          LoginLog.builder().id(TEST_LOG_ID).userId(OTHER_USER_ID).username("otheruser").build();
      ...
      BusinessException exception =
          assertThrows(BusinessException.class, () -> loginLogAppService.getLoginLog(TEST_LOG_ID));
