...
class EvidenceAppServiceTest {
    ...
    @Mock
    private EvidenceCrossExamMapper crossExamMapper;
    ...
    @InjectMocks
    private EvidenceAppService evidenceAppService;
    ...
    class QueryEvidenceTests {
        ...
        void listEvidence_shouldSuccess() {
            ...
            when(evidenceMapper.selectEvidencePage(any(Page.class), any(), any(), any(), any(), any(), any()))
                    .thenReturn(page);
    ...
    class CreateEvidenceTests {
        ...
        void createEvidence_shouldFail_whenMatterArchived() {
            ...
            BusinessException exception = assertThrows(BusinessException.class,
                    () -> evidenceAppService.createEvidence(command));
    ...
    class CrossExamTests {
        ...
        void addCrossExam_shouldSuccess() {
            ...
            doNothing().when(matterAppService).validateMatterOwnership(TEST_MATTER_ID);
            when(matterRepository.findById(TEST_MATTER_ID)).thenReturn(matter);
            when(crossExamMapper.selectByEvidenceIdAndParty(eq(TEST_EVIDENCE_ID), eq("原告"))).thenReturn(null);
        ...
        void addCrossExam_shouldFail_whenExists() {
            ...
            EvidenceCrossExam existing = EvidenceCrossExam.builder()
                    .id(1L)
                    .examParty("原告")
                    .build();
            ...
            doNothing().when(matterAppService).validateMatterOwnership(TEST_MATTER_ID);
            lenient().when(matterRepository.findById(TEST_MATTER_ID)).thenReturn(matter);
            when(crossExamMapper.selectByEvidenceIdAndParty(eq(TEST_EVIDENCE_ID), eq("原告"))).thenReturn(existing);

            // When & Then
            BusinessException exception = assertThrows(BusinessException.class,
                    () -> evidenceAppService.addCrossExam(command));
