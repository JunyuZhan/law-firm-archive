...
/** EvidenceListAppService 单元测试 测试证据清单管理服务的核心功能 */
...
class EvidenceListAppServiceTest {
  ...
  @Mock private EvidenceListRepository listRepository;

  @Mock private EvidenceListMapper listMapper;
  ...
  @Mock private EvidenceListDocumentGenerator documentGenerator;

  @Mock private MinioService minioService;

  @Mock private MatterAppService matterAppService;

  @InjectMocks private EvidenceListAppService evidenceListAppService;
  ...
  void setUp() throws Exception {
    ...
    java.lang.reflect.Field field =
        EvidenceListAppService.class.getDeclaredField("matterAppService");
  ...
  class CreateListTests {
    ...
    void createList_shouldFail_whenMatterArchived() {
      ...
      Matter matter = Matter.builder().id(TEST_MATTER_ID).status("ARCHIVED").build();
      ...
      BusinessException exception =
          assertThrows(BusinessException.class, () -> evidenceListAppService.createList(command));
  ...
  class QueryListTests {
    ...
    void listEvidenceLists_shouldSuccess() {
      ...
      Page<EvidenceList> pageParam = any(Page.class);
      when(listMapper.selectListPage(pageParam, eq(TEST_MATTER_ID), any())).thenReturn(page);
      lenient()
          .when(matterAppService.getAccessibleMatterIds(anyString(), anyLong(), anyLong()))
          .thenReturn(null);

      // When
      PageResult<EvidenceListDTO> result =
          evidenceListAppService.listEvidenceLists(TEST_MATTER_ID, null, 1, 10);
    ...
    void getListById_shouldSuccess() throws JsonProcessingException {
      ...
      Evidence evidence1 =
          Evidence.builder().id(TEST_EVIDENCE_ID_1).evidenceNo("EV001").name("证据1").build();
      Evidence evidence2 =
          Evidence.builder().id(TEST_EVIDENCE_ID_2).evidenceNo("EV002").name("证据2").build();
      ...
      lenient()
          .when(
              objectMapper.readValue(
                  eq("[300,301]"), any(com.fasterxml.jackson.core.type.TypeReference.class)))
    ...
    void getListById_shouldFail_whenNoPermission() {
      // Given
      EvidenceList list = EvidenceList.builder().id(TEST_LIST_ID).matterId(TEST_MATTER_ID).build();
      ...
      BusinessException exception =
          assertThrows(
              BusinessException.class, () -> evidenceListAppService.getListById(TEST_LIST_ID));
  ...
  class UpdateListTests {
    ...
    void updateList_shouldSuccess() throws JsonProcessingException {
      ...
      // When
      EvidenceListDTO result =
          evidenceListAppService.updateList(
  ...
  class GenerateFileTests {
    ...
    void generateListFile_shouldSuccess_whenWordFormat() throws Exception {
      ...
      Evidence evidence1 = Evidence.builder().id(TEST_EVIDENCE_ID_1).name("证据1").build();
      Evidence evidence2 = Evidence.builder().id(TEST_EVIDENCE_ID_2).name("证据2").build();

      Matter matter = Matter.builder().id(TEST_MATTER_ID).name("案件1").build();
      ...
      when(evidenceRepository.listByIds(anyList())).thenReturn(Arrays.asList(evidence1, evidence2));
      when(matterRepository.findById(TEST_MATTER_ID)).thenReturn(matter);
      when(documentGenerator.generateWordDocument(any(), any(), anyList()))
          .thenReturn(documentBytes);
      when(minioService.uploadFile(
              any(ByteArrayInputStream.class), anyString(), anyString(), anyString()))
      ...
      verify(minioService)
          .uploadFile(any(ByteArrayInputStream.class), anyString(), anyString(), anyString());
    }
    ...
    void generateListFile_shouldSuccess_whenPdfFormat() throws Exception {
      ...
      Evidence evidence = Evidence.builder().id(TEST_EVIDENCE_ID_1).name("证据1").build();
      ...
      lenient()
          .when(
              objectMapper.readValue(
                  eq("[300]"), any(com.fasterxml.jackson.core.type.TypeReference.class)))
      ...
      when(documentGenerator.generatePdfDocument(any(), any(), anyList()))
          .thenReturn(documentBytes);
      when(minioService.uploadFile(
              any(ByteArrayInputStream.class), anyString(), anyString(), anyString()))
    ...
    void generateListFile_shouldFail_whenNoEvidence() throws JsonProcessingException {
      ...
      lenient()
          ...
          .thenReturn(null);
      @SuppressWarnings("unchecked")
      com.fasterxml.jackson.core.type.TypeReference<List<Long>> typeRef4 =
          any(com.fasterxml.jackson.core.type.TypeReference.class);
      lenient()
          .when(objectMapper.readValue(anyString(), typeRef4))
  ...
  class QueryByMatterTests {
    ...
    void getListsByMatter_shouldSuccess() {
      // Given
      EvidenceList list =
          EvidenceList.builder().id(TEST_LIST_ID).matterId(TEST_MATTER_ID).name("清单1").build();

      when(listMapper.selectByMatterId(TEST_MATTER_ID)).thenReturn(Collections.singletonList(list));
      lenient()
          .when(matterAppService.getAccessibleMatterIds(anyString(), anyLong(), anyLong()))
          .thenReturn(null);
