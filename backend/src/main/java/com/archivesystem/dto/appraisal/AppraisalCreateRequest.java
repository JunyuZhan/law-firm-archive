...
public class AppraisalCreateRequest {
    ...
    @Schema(description = "鉴定类型：RETENTION(保管期限)、SECURITY(密级)、DESTRUCTION(销毁)", example = "RETENTION", requiredMode = Schema.RequiredMode.REQUIRED)
    ...
    @Schema(description = "鉴定原因", example = "该档案具有重要历史价值，建议延长保管期限", requiredMode = Schema.RequiredMode.REQUIRED)
