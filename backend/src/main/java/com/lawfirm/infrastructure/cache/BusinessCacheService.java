 ...
 * 业务缓存服务
 *
 * <p>封装业务层缓存操作，提供类型安全的缓存 API。 基于 CacheDegradationService 实现，支持 Redis + 本地缓存双层架构。
 *
 * @author junyuzhan
 * @since 2026-01-10
 */
...
public class BusinessCacheService {
   ...
   * @return 配置值
   */
  public String getConfig(final String key, final Supplier<String> dbLoader) {
   ...
   * @param key 配置键
   */
  public void evictConfig(final String key) {
    String cacheKey = PREFIX_CONFIG + key;
   ...
   * @return 菜单列表
   */
  public <T> T getUserMenus(final Long userId, final Supplier<T> dbLoader) {
   ...
   * @param userId 用户ID
   */
  public void evictUserMenus(final Long userId) {
    String cacheKey = PREFIX_MENU_USER + userId;
   ...
   * @return 部门信息
   */
  public <T> T getDepartment(final Long id, final Supplier<T> dbLoader) {
   ...
   * @return 部门树
   */
  public <T> T getDepartmentTree(final Supplier<T> dbLoader) {
    log.debug("获取部门树缓存");
   ...
   * @param id 部门ID
   */
  public void evictDepartment(final Long id) {
    String cacheKey = PREFIX_DEPT + id;
   ...
   * @return 字典项列表
   */
  public <T> T getDictItems(final String code, final Supplier<T> dbLoader) {
   ...
   * @param code 字典编码
   */
  public void evictDictItems(final String code) {
    String cacheKey = PREFIX_DICT_ITEMS + code;
  ...
  public CacheStats getCacheStats() {
    ...
    try {
      ...
      stats.setTotalCacheCount(
          stats.getConfigCacheCount()
              + stats.getMenuCacheCount()
              + stats.getDeptCacheCount()
   ...
   */
  private void evictByPattern(final String pattern) {
    try {
