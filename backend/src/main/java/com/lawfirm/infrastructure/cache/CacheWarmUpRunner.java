...
public class CacheWarmUpRunner implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        if (failCount > 0) {
            log.warn("========== 缓存预热完成（有失败）| 成功: {} | 失败: {} | 耗时: {}ms ==========", 
        ...
        for (Map.Entry<String, Object> entry : beans.entrySet()) {
            ...
            for (Method method : clazz.getDeclaredMethods()) {
                ...
                if (annotation != null) {
                    ...
                    task.setDescription(annotation.description().isEmpty() 
     ...
     * 同步执行预热任务
     */
    private void executeSync(WarmUpTask task) throws Exception {
     ...
     */
    @Async
    public void executeAsync(WarmUpTask task) {
        try {
            executeSync(task);
        } catch (Exception e) {
