 ...
 * 用户详情服务实现
 *
 * <p>性能优化：使用Redis缓存用户认证信息，避免每次请求都查询数据库
 */
@Slf4j
@Service
...
public class UserDetailsServiceImpl implements UserDetailsService {
   ...
   * 清除用户认证缓存.
   *
   * <p>在用户信息变更、权限变更、登出等场景调用
   *
   * @param username 用户名
   */
  public void clearUserAuthCache(final String username) {
    if (username == null) {
      return;
    }
    try {
   ...
   * @param userId 用户ID
   */
  public void clearUserAuthCacheByUserId(final Long userId) {
    if (userId == null) {
      return;
    }
    try {
