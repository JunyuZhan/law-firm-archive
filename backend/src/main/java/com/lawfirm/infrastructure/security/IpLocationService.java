 ...
 * IP 地理位置服务
 *
 * <p>使用 ip2region 离线数据库查询 IP 地址的地理位置 支持国内 IP 精确到城市级别
 *
 * @author system
 */
@Slf4j
...
public class IpLocationService {
  ...
  public void init() {
    ...
    try {
      ...
      try (InputStream inputStream = resource.getInputStream()) {
        ...
        log.info("IP地理位置服务初始化成功，数据库大小: {} KB", cBuff.length / BYTES_PER_KB);
      }
    } catch (Exception e) {
   ...
   * @return IP位置信息
   */
  private IpLocation parseRegion(final String region) {
   ...
   * @return 是否为同一地区
   */
  public boolean isSameRegion(final IpLocation loc1, final IpLocation loc2) {
