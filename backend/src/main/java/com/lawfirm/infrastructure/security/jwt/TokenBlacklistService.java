 ...
 * Token 黑名单服务
 *
 * <p>功能： 1. 管理已失效的 Token（登出、刷新后的旧 Token） 2. 支持 Token 轮换机制 3. 使用 Redis 存储，自动过期
 *
 * <p>安全说明： - 当用户登出时，将 access token 和 refresh token 加入黑名单 - 当刷新 token 时，将旧的 refresh token
 * 加入黑名单（防止重放攻击） - 黑名单条目会在 token 原本的过期时间后自动删除
 */
@Slf4j
@Service
...
public class TokenBlacklistService {
   ...
   */
  public void addToBlacklist(final String token, final long expireSeconds) {
  ...
  public void addToBlacklist(final String token) {
    addToBlacklist(token, DEFAULT_EXPIRE_SECONDS); // 24 小时
  }
   ...
   */
  public boolean markRefreshTokenUsed(final String refreshToken, final long expireSeconds) {
   ...
   * @return true 如果已被使用
   */
  public boolean isRefreshTokenUsed(final String refreshToken) {
    if (refreshToken == null || refreshToken.isEmpty()) {
      return false;
    }
   ...
   * @param userId 用户ID
   */
  public void invalidateUserTokens(final Long userId) {
   ...
   */
  public boolean isTokenInvalidatedByUser(final Long userId, final long issuedAt) {
    String key = "token:invalidate:user:" + userId;
