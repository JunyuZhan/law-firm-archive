...
public class PayrollAutoConfirmScheduler {
    ...
    public void autoConfirmPayrollItems() {
        ...
        // 查询所有待确认状态的工资表
        List<PayrollSheet> sheets = payrollSheetRepository.lambdaQuery()
        ...
        for (PayrollSheet sheet : sheets) {
            try {
                // 查询该工资表中所有待确认的工资明细
                List<PayrollItem> pendingItems = payrollItemRepository.lambdaQuery()
                ...
                long confirmedCount = allItems.stream()
                ...
                log.info("自动确认工资表: payrollNo={}, autoConfirmedCount={}", 
                        sheet.getPayrollNo(), pendingItems.size());
