...
public class PaddleOcrService implements OcrService {
  ...
  /** 身份证识别置信度阈值 */
  private static final double ID_CARD_CONFIDENCE_THRESHOLD = 0.95;
  ...
  public void init() {
    ...
    factory.setConnectTimeout(CONNECT_TIMEOUT); // 连接超时10秒
  ...
  public OcrResult recognizeText(final MultipartFile file) {
    try {
      ...
      return OcrResult.builder().success(false).errorMessage("识别失败: " + e.getMessage()).build();
    }
  }
   ...
   * @throws Exception 异常
   */
  private String callPaddleOcr(final MultipartFile file, final String endpoint) throws Exception {
    ...
    ResponseEntity<String> response =
        restTemplate.exchange(
            paddleOcrUrl + endpoint, HttpMethod.POST, requestEntity, String.class);
   ...
   * @throws Exception 异常
   */
  private String callPaddleOcrByUrl(final String imageUrl, final String endpoint) throws Exception {
    HttpHeaders headers = new HttpHeaders();
   ...
   * @throws Exception 异常
   */
  private OcrResult parseGeneralResult(final String jsonResult) throws Exception {
   ...
   * @throws Exception 异常
   */
  private OcrResult parseBankReceiptResult(final String jsonResult) throws Exception {
    JsonNode root = objectMapper.readTree(jsonResult);
    ...
    return OcrResult.builder()
        ...
        .type("BANK_RECEIPT")
        .data(resultData)
        .confidence(data.path("confidence").asDouble(HIGH_CONFIDENCE_THRESHOLD))
        .bankName(bankName)
        .amount(amount)
   ...
   * @throws Exception 异常
   */
  private OcrResult parseIdCardResult(final String jsonResult, final boolean isFront)
      throws Exception {
    JsonNode root = objectMapper.readTree(jsonResult);
    JsonNode data = root.path("result");
    ...
    if (isFront) {
      ...
      return OcrResult.builder()
          ...
          .rawText(rawText)
          .data(resultData)
          .confidence(data.path("confidence").asDouble(ID_CARD_CONFIDENCE_THRESHOLD))
          .name(name)
          .gender(gender)
          .ethnicity(ethnicity)
   ...
   * @throws Exception 异常
   */
  private OcrResult parseBusinessLicenseResult(final String jsonResult) throws Exception {
    JsonNode root = objectMapper.readTree(jsonResult);
    JsonNode data = root.path("result");
   ...
   * @throws Exception 异常
   */
  private OcrResult parseBusinessCardResult(final String jsonResult) throws Exception {
    JsonNode root = objectMapper.readTree(jsonResult);
    JsonNode data = root.path("result");
   ...
   * @throws Exception 异常
   */
  private OcrResult parseInvoiceResult(final String jsonResult) throws Exception {
    JsonNode root = objectMapper.readTree(jsonResult);
    JsonNode data = root.path("result");
