...
public interface GoOutRecordMapper extends BaseMapper<GoOutRecord> {
     ...
     * 查询用户的外出记录
     */
    @Select("SELECT * FROM go_out_record WHERE user_id = #{userId} AND deleted = false ORDER BY out_time DESC")
    List<GoOutRecord> selectByUserId(@Param("userId") Long userId);

    /**
     * 查询指定日期范围的外出记录
     */
    @Select("SELECT * FROM go_out_record " +
            "WHERE user_id = #{userId} " +
            "AND DATE(out_time) >= #{startDate} " +
            "AND DATE(out_time) <= #{endDate} " +
            "AND deleted = false " +
            "ORDER BY out_time DESC")
    List<GoOutRecord> selectByDateRange(@Param("userId") Long userId,
     ...
     * 查询当前外出的记录
     */
    @Select("SELECT * FROM go_out_record WHERE user_id = #{userId} AND status = 'OUT' AND deleted = false ORDER BY out_time DESC")
