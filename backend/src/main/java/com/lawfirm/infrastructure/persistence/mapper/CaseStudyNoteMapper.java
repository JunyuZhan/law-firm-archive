...
public interface CaseStudyNoteMapper extends BaseMapper<CaseStudyNote> {
   ...
   */
  @Select(
      "SELECT * FROM case_study_note WHERE case_id = #{caseId} AND user_id = #{userId} AND deleted = false")
   ...
   */
  @Select(
      "SELECT * FROM case_study_note WHERE case_id = #{caseId} AND deleted = false ORDER BY created_at DESC")
   ...
   */
  @Select(
      "SELECT * FROM case_study_note WHERE user_id = #{userId} AND deleted = false ORDER BY created_at DESC")
