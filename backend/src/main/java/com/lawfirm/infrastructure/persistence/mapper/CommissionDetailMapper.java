...
public interface CommissionDetailMapper extends BaseMapper<CommissionDetail> {
     ...
     * 根据提成ID查询明细
     */
    @Select("SELECT * FROM finance_commission_detail WHERE commission_id = #{commissionId} AND deleted = false")
    List<CommissionDetail> selectByCommissionId(@Param("commissionId") Long commissionId);

    /**
     * 根据用户ID查询明细
     */
    @Select("SELECT * FROM finance_commission_detail WHERE user_id = #{userId} AND deleted = false ORDER BY created_at DESC")
