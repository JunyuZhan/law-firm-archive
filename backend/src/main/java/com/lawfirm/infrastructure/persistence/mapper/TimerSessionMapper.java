...
public interface TimerSessionMapper extends BaseMapper<TimerSession> {
     ...
     * 查询用户当前正在运行的计时器会话
     */
    @Select("SELECT * FROM timer_session WHERE user_id = #{userId} AND status = 'RUNNING' ORDER BY start_time DESC LIMIT 1")
    TimerSession selectRunningByUserId(@Param("userId") Long userId);

    /**
     * 查询用户当前已暂停的计时器会话
     */
    @Select("SELECT * FROM timer_session WHERE user_id = #{userId} AND status = 'PAUSED' ORDER BY pause_time DESC LIMIT 1")
