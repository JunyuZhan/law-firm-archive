...
public interface ArchiveMapper extends BaseMapper<Archive> {
     ...
     * 查询档案的借阅记录
     */
    @Select("SELECT * FROM archive_borrow WHERE archive_id = #{archiveId} AND deleted = false ORDER BY created_at DESC")
    List<com.lawfirm.domain.archive.entity.ArchiveBorrow> selectBorrowsByArchiveId(@Param("archiveId") Long archiveId);

    /**
     * 按库位查询档案（M7-022）
     */
    @Select("SELECT * FROM archive WHERE location_id = #{locationId} AND deleted = false AND status = 'STORED' ORDER BY box_no, archive_no")
