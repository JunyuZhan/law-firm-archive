...
public interface LawRegulationMapper extends BaseMapper<LawRegulation> {
    ...
    @Select("<script>" +
            ...
            "<if test='keyword != null'> AND (title LIKE CONCAT('%',#{keyword},'%') OR keywords LIKE CONCAT('%',#{keyword},'%')) </if>" +
            "ORDER BY issue_date DESC, id DESC" +
            "</script>")
    IPage<LawRegulation> selectRegulationPage(Page<LawRegulation> page,
     ...
     * 减少收藏次数
     */
    @Update("UPDATE law_regulation SET collect_count = collect_count - 1 WHERE id = #{id} AND collect_count > 0")
