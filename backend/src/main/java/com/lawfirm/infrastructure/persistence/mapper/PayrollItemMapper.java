...
public interface PayrollItemMapper extends BaseMapper<PayrollItem> {
   ...
   */
  @Select(
      "SELECT * FROM hr_payroll_item WHERE payroll_sheet_id = #{payrollSheetId} "
          + "AND deleted = false ORDER BY employee_no")
   ...
   */
  @Select(
      "SELECT pi.* FROM hr_payroll_item pi "
          ...
          + "LIMIT 1")
  PayrollItem selectByEmployeeIdAndYearMonth(
   ...
   */
  @Select(
      "SELECT COUNT(*) FROM hr_payroll_item "
          + "WHERE payroll_sheet_id = #{payrollSheetId} "
          + "AND confirm_status = 'CONFIRMED' AND deleted = false")
