...
public interface TrainingMapper extends BaseMapper<Training> {
     ...
     * 查询可报名的培训
     */
    @Select("SELECT * FROM hr_training WHERE status = 'PUBLISHED' AND enroll_deadline >= CURRENT_DATE AND deleted = false ORDER BY start_time")
     ...
     * 减少报名人数
     */
    @Update("UPDATE hr_training SET current_participants = current_participants - 1 WHERE id = #{id} AND current_participants > 0")
