...
public interface DocumentMapper extends BaseMapper<Document> {
  ...
  @Select(
      "<script>"
          ...
          + "</script>")
  IPage<Document> selectDocumentPage(
      Page<Document> page,
   ...
   */
  @Select(
      "SELECT * FROM doc_document WHERE (id = #{docId} OR parent_doc_id = #{docId}) "
          + "AND deleted = false ORDER BY version DESC")
   ...
   */
  @Select(
      "SELECT COUNT(*) FROM doc_document WHERE category_id = #{categoryId} AND deleted = false AND is_latest = true")
   ...
   */
  @Select(
      "SELECT * FROM doc_document WHERE matter_id = #{matterId} "
   ...
   */
  @Update(
      "UPDATE matter_dossier_item SET document_count = "
          + "GREATEST(0, COALESCE(document_count, 0) + #{delta}) "
          + "WHERE id = #{dossierItemId}")
  void updateDossierItemDocCount(
      @Param("dossierItemId") Long dossierItemId, @Param("delta") int delta);
   ...
   */
  @Select(
      "SELECT * FROM doc_document WHERE dossier_item_id = #{dossierItemId} "
          + "AND deleted = false ORDER BY display_order ASC, created_at DESC")
