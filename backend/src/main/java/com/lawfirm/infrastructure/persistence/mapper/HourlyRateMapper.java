...
public interface HourlyRateMapper extends BaseMapper<HourlyRate> {
     ...
     * 获取用户当前有效费率
     */
    @Select("SELECT * FROM hourly_rate WHERE user_id = #{userId} AND status = 'ACTIVE' " +
     ...
     * 获取用户最新费率
     */
    @Select("SELECT * FROM hourly_rate WHERE user_id = #{userId} AND status = 'ACTIVE' AND deleted = false ORDER BY effective_date DESC LIMIT 1")
