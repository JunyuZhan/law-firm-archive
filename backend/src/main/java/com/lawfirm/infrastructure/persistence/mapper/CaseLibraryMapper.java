...
public interface CaseLibraryMapper extends BaseMapper<CaseLibrary> {
  ...
  @Select(
      "<script>"
          ...
          + "OR cause_of_action LIKE CONCAT('%',#{keyword},'%')) </if>"
          + "ORDER BY judge_date DESC, id DESC"
          + "</script>")
  IPage<CaseLibrary> selectCasePage(
      Page<CaseLibrary> page,
   ...
   */
  @Update(
      "UPDATE case_library SET collect_count = collect_count - 1 WHERE id = #{id} AND collect_count > 0")
