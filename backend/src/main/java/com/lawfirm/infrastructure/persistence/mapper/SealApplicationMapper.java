...
public interface SealApplicationMapper extends BaseMapper<SealApplication> {
  ...
  @Select(
      "<script>"
          ...
          + "</script>")
  IPage<SealApplication> selectApplicationPage(
      Page<SealApplication> page,
   ...
   */
  @Select(
      "SELECT * FROM seal_application WHERE status = 'PENDING' AND deleted = false ORDER BY created_at ASC")
   ...
   */
  @Select(
      "SELECT COUNT(*) FROM seal_application WHERE seal_id = #{sealId} AND status = 'USED' AND deleted = false")
  int countUsageBySealId(@Param("sealId") Long sealId);
   ...
   */
  @Select(
      "SELECT COUNT(*) FROM seal_application WHERE seal_id = #{sealId} "
          + "AND status IN ('PENDING', 'APPROVED') AND deleted = false")
