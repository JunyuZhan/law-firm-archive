...
public interface LeaveBalanceMapper extends BaseMapper<LeaveBalance> {
   ...
   */
  @Select(
      "SELECT * FROM leave_balance WHERE user_id = #{userId} AND year = #{year} AND deleted = false")
   ...
   */
  @Select(
      "SELECT * FROM leave_balance WHERE user_id = #{userId} "
          + "AND leave_type_id = #{leaveTypeId} AND year = #{year} "
          + "AND deleted = false")
  LeaveBalance selectByUserTypeYear(
  ...
  @Update(
      "UPDATE leave_balance SET used_days = used_days + #{days}, "
          + "remaining_days = remaining_days - #{days}, updated_at = NOW() "
          + "WHERE user_id = #{userId} AND leave_type_id = #{leaveTypeId} "
          + "AND year = #{year} AND remaining_days >= #{days}")
  int deductBalance(
      @Param("userId") Long userId,
      @Param("leaveTypeId") Long leaveTypeId,
      @Param("year") Integer year,
