...
public interface LetterApplicationMapper extends BaseMapper<LetterApplication> {
  ...
  @Select(
      "<script>"
          ...
          + "AND application_no LIKE CONCAT('%', #{applicationNo}, '%') </if>"
          + "<if test='status != null and status != \"\"'> AND status = #{status} </if>"
          + "<if test='startDate != null and startDate != \"\"'> "
          + "AND created_at >= CAST(#{startDate} AS TIMESTAMP) </if>"
          + "<if test='endDate != null and endDate != \"\"'> "
          + "AND created_at &lt;= CAST(#{endDate} AS TIMESTAMP) + INTERVAL '1 day' </if>"
   ...
   */
  @Select(
      "SELECT * FROM letter_application WHERE status IN ('APPROVED', 'PRINTED') "
          + "AND deleted = false ORDER BY approved_at ASC")
   ...
   */
  @Select(
      "SELECT * FROM letter_application WHERE matter_id = #{matterId} AND deleted = false ORDER BY created_at DESC")
  List<LetterApplication> selectByMatterId(@Param("matterId") Long matterId);

  /**
   * 查询某项目的出函申请最大序号（从application_no中提取） 用于生成新的申请编号，避免并发冲突.
   *
   * @param matterId 项目ID
   * @return 最大序号
   */
  @Select(
      "SELECT MAX(CAST(SUBSTRING(application_no FROM '[0-9]+$') AS INTEGER)) "
