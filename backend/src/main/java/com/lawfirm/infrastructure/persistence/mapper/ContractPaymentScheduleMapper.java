...
public interface ContractPaymentScheduleMapper extends BaseMapper<ContractPaymentSchedule> {
   ...
   */
  @Select(
      "SELECT * FROM contract_payment_schedule WHERE contract_id = #{contractId} "
          + "AND deleted = false ORDER BY planned_date")
   ...
   */
  @Select(
      "SELECT COALESCE(SUM(amount), 0) FROM contract_payment_schedule "
