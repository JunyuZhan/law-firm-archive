...
public interface ApprovalMapper extends BaseMapper<Approval> {
  ...
  @Select(
      "<script>"
          ...
          + "</script>")
  List<Approval> selectApprovalPage(
  ...
  @Select(
      "SELECT * FROM workbench_approval "
          ...
          + "ORDER BY created_at DESC")
  List<Approval> selectByBusiness(
   ...
   */
  @Select(
      "SELECT COUNT(*) FROM workbench_approval WHERE approver_id = #{approverId} "
   ...
   */
  @Select(
      "SELECT * FROM workbench_approval WHERE approver_id = #{approverId} "
          + "AND status = 'PENDING' AND deleted = false "
          + "ORDER BY created_at DESC LIMIT #{limit}")
  List<Approval> selectPendingByApproverId(
      @Param("approverId") Long approverId, @Param("limit") int limit);
  ...
  /**
   * 根据数据范围查询审批历史（已完成的审批）ALL: 查看所有已完成审批.
   *
   * @return 所有审批历史列表
   */
  @Select(
  ...
  /**
   * ✅ 修复问题556: 使用递归CTE一次性查询所有后代部门ID 避免递归Java调用导致的多次数据库查询.
   *
   * @param parentId 父部门ID
   * @return 所有后代部门ID列表
   */
