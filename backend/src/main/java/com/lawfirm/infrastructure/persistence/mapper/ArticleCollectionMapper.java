...
public interface ArticleCollectionMapper extends BaseMapper<ArticleCollection> {
     ...
     * 查询用户是否已收藏文章
     */
    @Select("SELECT COUNT(*) FROM article_collection WHERE user_id = #{userId} AND article_id = #{articleId} AND deleted = false")
    int countByUserAndArticle(@Param("userId") Long userId, @Param("articleId") Long articleId);

    /**
     * 查询用户收藏的所有文章ID
     */
    @Select("SELECT article_id FROM article_collection WHERE user_id = #{userId} AND deleted = false ORDER BY created_at DESC")
