...
public interface PrepaymentMapper extends BaseMapper<Prepayment> {
   ...
   */
  @Select(
      "SELECT * FROM finance_prepayment WHERE client_id = #{clientId} "
   ...
   */
  @Select(
      "SELECT * FROM finance_prepayment WHERE matter_id = #{matterId} AND deleted = false ORDER BY receipt_date DESC")
   ...
   */
  @Select(
      "SELECT * FROM finance_prepayment WHERE contract_id = #{contractId} "
          + "AND deleted = false ORDER BY receipt_date DESC")
  ...
  @Select(
      "<script>"
          ...
          + "<if test='prepaymentNo != null and prepaymentNo != \"\"'> "
          + "AND prepayment_no LIKE '%' || #{prepaymentNo} || '%' </if>"
          + "ORDER BY created_at DESC"
          + "</script>")
  IPage<Prepayment> selectPrepaymentPage(
      Page<Prepayment> page,
