...
public class WecomNotificationChannel {
     ...
     */
    @Async
    public void sendCourtReminder(Long receiverId, String matterTitle, String caseNo, 
                                  String courtName, String courtTime, String location) {
        try {
            ExternalIntegration config = getWecomConfig();
            if (config == null) {
                return;
     ...
     */
    @Async
    public void sendTaskDeadlineReminder(Long receiverId, String taskTitle, 
                                         String dueDate, int daysLeft) {
        try {
            ExternalIntegration config = getWecomConfig();
            if (config == null) {
                return;
            ...
            String icon = daysLeft <= 0 ? "🚨" : (daysLeft <= 1 ? "⚠️" : "📋");
            String urgency = daysLeft <= 0 ? "已逾期" : (daysLeft == 1 ? "明天到期" : daysLeft + "天后到期");
            ...
            markdown.append("> **状态**：<font color=\"").append(color).append("\">")
                    .append(urgency).append("</font>\n");
     ...
     */
    @Async
    public void sendApprovalNotification(Long receiverId, String approvalType, 
                                         String applicant, String summary) {
        try {
            ExternalIntegration config = getWecomConfig();
            if (config == null) {
                return;
     ...
     * 内部发送方法
     */
    private void sendNotificationInternal(ExternalIntegration config, Notification notification) {
        if (!shouldPush(config, notification.getType())) {
            return;
        }
     ...
     * 构建消息内容
     */
    private String buildMessageContent(Notification notification) {
