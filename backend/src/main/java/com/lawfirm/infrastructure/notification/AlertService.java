...
public class AlertService {
  ...
  /** 磁盘空间严重告警阈值 */
  private static final long DISK_CRITICAL_THRESHOLD = 90;
  ...
  public enum AlertLevel {
     ...
     * @param color 级别颜色
     */
    AlertLevel(final String name, final String color) {
      this.name = name;
      this.color = color;
    }
   ...
   */
  public void sendLoginFailureAlert(final String username, final String ip, final int failCount) {
    ...
    String content =
        buildAlertHtml(
            ...
            "登录失败告警",
            String.format(
                "检测到用户 <strong>%s</strong> 在短时间内登录失败 <strong>%d</strong> 次，可能存在暴力破解行为。",
                username, failCount),
            new String[][] {
              {"用户名", username},
              {"来源IP", ip},
   ...
   * @param reason 锁定原因
   */
  public void sendAccountLockedAlert(final String username, final String ip, final String reason) {
   ...
   * @param location 登录位置
   */
  public void sendNewLocationLoginAlert(
      final String username, final String ip, final String location) {
   ...
   * @param location 登录位置
   */
  public void sendNewLocationLoginSuccessAlert(
      final String username, final String ip, final String location) {
   ...
   * @param details 详细信息
   */
  public void sendSuspiciousIpAlert(final String ip, final int requestCount, final String details) {
   ...
   */
  public void sendSystemErrorAlert(
      final String errorType, final String errorMessage, final String stackTrace) {
   ...
   * @param message 告警信息
   */
  public void sendDatabaseAlert(final String message) {
   ...
   *
   * @param path 路径
   * @param usedPercent 使用百分比
   * @param freeSpaceMB 剩余空间(MB)
   */
  public void sendDiskSpaceAlert(
      final String path, final long usedPercent, final long freeSpaceMB) {
    AlertLevel level =
        usedPercent > DISK_CRITICAL_THRESHOLD ? AlertLevel.CRITICAL : AlertLevel.WARNING;
   ...
   * @param errorMessage 错误信息
   */
  public void sendBackupFailureAlert(final String backupType, final String errorMessage) {
   ...
   * @param message 维护信息
   */
  public void sendMaintenanceModeNotification(final boolean enabled, final String message) {
  ...
  private String buildAlertHtml(
    ...
    html.append("<span style='font-size:12px;opacity:0.9;'>级别: ")
    ...
    if (details != null && details.length > 0) {
      ...
      for (String[] row : details) {
        html.append("<tr>");
        html.append(
                "<td style='padding:8px;border-bottom:1px solid #f0f0f0;color:#666;width:120px;'>")
            .append(row[0])
            .append("</td>");
    ...
    if (additionalInfo != null && !additionalInfo.isEmpty()) {
      html.append(
          "<div style='background:#f9f9f9;padding:10px;border-radius:5px;margin-top:15px;'>");
