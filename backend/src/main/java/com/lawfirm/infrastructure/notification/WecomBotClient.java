 ...
 * 企业微信机器人客户端
 *
 * <p>文档：https://developer.work.weixin.qq.com/document/path/91770
 *
 * <p>支持消息类型： - 文本消息（支持@指定人） - Markdown消息（更丰富的格式） - 图文消息（带图片和链接） - 模板卡片消息（可交互）
 */
@Slf4j
@Component
...
public class WecomBotClient {
  ...
  public boolean sendText(
      ...
      final List<String> mentionedList,
      final List<String> mentionedMobileList) {
    try {
   ...
   * @return 是否发送成功
   */
  public boolean sendText(final String webhookUrl, final String content) {
   ...
   * 发送Markdown消息.
   *
   * <p>支持的格式： - 标题：# 一级标题 ~ ###### 六级标题 - 加粗：**text** - 链接：[text](url) - 引用：> text - 字体颜色：<font
   * color="info">绿色</font>、<font color="warning">橙色</font>、<font color="comment">灰色</font>
   *
   * @param webhookUrl Webhook地址
   * @param content Markdown内容（最长4096字节）
   * @return 是否发送成功
   */
  public boolean sendMarkdown(final String webhookUrl, final String content) {
    try {
      Map<String, Object> body = new HashMap<>();
   ...
   * @return 是否发送成功
   */
  public boolean sendKeyValueCard(
      final String webhookUrl,
      final String title,
   ...
   * @return 是否发送成功
   */
  public boolean sendTextMentionAll(final String webhookUrl, final String content) {
   ...
   * @return 是否连接成功
   */
  public boolean testConnection(final String webhookUrl) {
    try {
   ...
   * @return 是否发送成功
   */
  private boolean sendRequest(final String webhookUrl, final Map<String, Object> body) {
    try {
      ...
      if (response.getStatusCode() == HttpStatus.OK) {
        ...
        if (errcode == 0) {
          ...
          log.warn("企业微信消息发送失败: errcode={}, errmsg={}", errcode, result.get("errmsg"));
        }
      } else {
