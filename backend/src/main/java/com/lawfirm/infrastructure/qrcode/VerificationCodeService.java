...
public class VerificationCodeService {
     ...
     * @return Base64编码的验证码
     */
    public String generateCode(String businessType, Long businessId, String businessNo, LocalDateTime timestamp) {
        try {
            // 构建待签名字符串：业务类型|业务ID|业务编号|时间戳
            String dataToSign = String.format("%s|%d|%s|%s", 
                businessType,
                businessId,
                businessNo,
                timestamp.toString());
            ...
            log.error("生成验证码失败: businessType={}, businessId={}, businessNo={}, error={}", 
                businessType, businessId, businessNo, e.getMessage(), e);
     ...
     * @return 是否有效
     */
    public boolean verifyCode(String businessType, Long businessId, String businessNo, 
                             LocalDateTime timestamp, String verificationCode) {
        try {
            // 重新生成验证码进行比对
            String expectedCode = generateCode(businessType, businessId, businessNo, timestamp);
            return expectedCode.equals(verificationCode);
        } catch (Exception e) {
            log.error("验证验证码失败: businessType={}, businessId={}, businessNo={}, error={}", 
                businessType, businessId, businessNo, e.getMessage(), e);
            return false;
        }
    }
     ...
     * @return 完整的验证URL
     */
    public String generateVerificationUrl(String publicVerifyUrl, String businessType, 
                                          String businessNo, String verificationCode) {
        return String.format("%s/verify/%s?no=%s&code=%s", 
            publicVerifyUrl,
