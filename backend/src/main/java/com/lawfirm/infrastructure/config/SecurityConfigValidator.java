 ...
 * 生产环境安全配置验证器
 *
 * <p>在系统启动时检查关键安全配置，防止使用不安全的默认值部署到生产环境。
 *
 * <p>检查项目： 1. 数据库密码 - 不能使用默认开发密码 2. JWT密钥 - 必须设置强随机密钥 3. MinIO密钥 - 不能使用默认的 minioadmin
 */
@Slf4j
@Component
...
public class SecurityConfigValidator implements ApplicationRunner {
  ...
  /** 不安全的默认数据库密码列表. */
  private static final List<String> UNSAFE_DB_PASSWORDS =
      List.of("dev_password_123", "password", "123456", "root", "admin");

  /** 不安全的默认JWT密钥列表. */
  private static final List<String> UNSAFE_JWT_SECRETS =
      List.of(
  ...
  public void run(final ApplicationArguments args) {
    ...
    } else if (UNSAFE_JWT_SECRETS.stream()
        .anyMatch(s -> jwtSecret.toLowerCase().contains(s.toLowerCase()))) {
    ...
    // 3. 检查MinIO密钥
    if ("minioadmin".equalsIgnoreCase(minioAccessKey)
        || "minioadmin".equalsIgnoreCase(minioSecretKey)) {
