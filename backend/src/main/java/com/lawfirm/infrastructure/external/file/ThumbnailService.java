...
public class ThumbnailService {
    ...
    /**
     * 为上传的文件生成缩略图（支持图片和PDF）
     * 问题247修复：确保InputStream正确关闭
     * @param file 原始文件
    ...
    /**
     * 从输入流生成图片缩略图
     * ⚠️ 注意：此方法不会关闭 inputStream，调用者需要负责关闭
     */
    public String generateThumbnailFromStream(InputStream inputStream, String originalFileName) {
        try {
            ...
            // 上传缩略图到MinIO
            try (ByteArrayInputStream thumbnailStream = new ByteArrayInputStream(outputStream.toByteArray())) {
     ...
     * @return 缩略图URL
     */
    public String generatePdfThumbnail(InputStream inputStream, String originalFileName) {
        try {
