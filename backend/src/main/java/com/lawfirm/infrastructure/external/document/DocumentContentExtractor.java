...
/** 文档内容提取服务 支持提取Word、Excel等文档的文本内容 */
@Slf4j
@Service
public class DocumentContentExtractor {
   ...
   */
  private String extractFromDocx(final byte[] fileBytes) throws Exception {
  ...
  /**
   * 提取DOC文档内容 注意：需要poi-scratchpad依赖支持
   *
   * @param fileBytes 文件字节数组
   * @return 提取的文本内容
   * @throws Exception 提取失败时抛出异常
   */
  private String extractFromDoc(final byte[] fileBytes) throws Exception {
   ...
   * @throws Exception 提取失败时抛出异常
   */
  private String extractFromXlsx(final byte[] fileBytes) throws Exception {
   ...
   * @throws Exception 提取失败时抛出异常
   */
  private String extractFromXls(final byte[] fileBytes) throws Exception {
    StringBuilder text = new StringBuilder();
    try (InputStream is = new ByteArrayInputStream(fileBytes);
