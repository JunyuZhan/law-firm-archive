 ...
 * 节假日服务
 *
 * <p>提供工作日计算、节假日判断等功能 优先使用本地缓存，缓存未命中时调用API
 */
@Slf4j
@Service
...
public class HolidayService {
   ...
   */
  public String getDayTypeName(final LocalDate date) {
    HolidayCache cache = holidayCacheMapper.selectByDate(date);
    if (cache != null) {
   ...
   * @return 目标日期
   */
  public LocalDate addWorkdays(final LocalDate startDate, final int workdays) {
    // 先尝试使用API计算（更准确）
   ...
   * @return 工作日数量
   */
  public int countWorkdays(final LocalDate startDate, final LocalDate endDate) {
   ...
   * 计算诉讼期限截止日期
   *
   * <p>根据《民事诉讼法》规定，期间以日计算的，从期间开始的次日起算 期间届满的最后一日是节假日的，以节假日后的第一日为期间届满的日期
   *
  ...
  public LocalDate calculateDeadline(
      final LocalDate startDate, final int days, final boolean workdaysOnly) {
    // 从次日起算
   ...
   * @return 提醒日期
   */
  public LocalDate calculateReminderDate(final LocalDate targetDate, final int workdaysBefore) {
   ...
   * @return 休息日列表
   */
  public List<LocalDate> getOffDaysInRange(final LocalDate startDate, final LocalDate endDate) {
   ...
   * @return 同步的记录数
   */
  public int syncYearHolidays(final int year) {
   ...
   * @return true=已有数据
   */
  public boolean isYearSynced(final int year) {
   ...
   * @return 目标日期
   */
  private LocalDate calculateWorkdaysLocal(final LocalDate startDate, final int workdays) {
    LocalDate current = startDate;
   ...
   * @param info 节假日信息
   */
  private void saveToCache(final HolidayInfo info) {
    try {
   ...
   * @return 缓存实体
   */
  private HolidayCache convertToCache(final HolidayInfo info) {
