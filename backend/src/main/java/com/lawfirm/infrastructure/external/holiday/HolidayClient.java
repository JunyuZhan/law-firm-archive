 ...
 * 节假日API客户端
 *
 * <p>使用 timor.tech 免费节假日API 文档：https://timor.tech/api/holiday
 */
@Slf4j
@Component
...
public class HolidayClient {
   ...
   */
  public HolidayInfo getHolidayInfo(final LocalDate date) {
    ...
    try {
      ...
      return HolidayInfo.builder()
          ...
          .holidayName(
              holiday.isNull() || holiday.isMissingNode() ? null : holiday.path("name").asText())
   ...
   * @return 节假日列表
   */
  public List<HolidayInfo> getYearHolidays(final int year) {
   ...
   */
  public LocalDate calculateWorkday(final LocalDate startDate, final int workdays) {
   ...
   * @return 节假日列表
   */
  public List<HolidayInfo> getMonthHolidays(final int year, final int month) {
