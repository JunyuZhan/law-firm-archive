...
/**
 * 卷宗封面生成器 生成标准律师事务所业务档案卷宗封面PDF（牛皮纸风格）
 *
 * <p>支持两种模板： - 民事/行政/仲裁案件模板：包含当事人（原告、被告、第三人） - 刑事案件模板：包含被告人
 */
@Slf4j
@Component
...
public class DossierCoverGenerator {
  ...
  /** 页边距：右边距 */
  private static final float PAGE_MARGIN_RIGHT = 50f;

  /** 页边距：下边距 */
  private static final float PAGE_MARGIN_BOTTOM = 40f;

  /** 页边距：左边距 */
  private static final float PAGE_MARGIN_LEFT = 50f;

  /** 段落下边距：15 */
  private static final float PARAGRAPH_MARGIN_BOTTOM_15 = 15f;

  /** 段落下边距：20 */
  private static final float PARAGRAPH_MARGIN_BOTTOM_20 = 20f;

  /** 段落下边距：25 */
  private static final float PARAGRAPH_MARGIN_BOTTOM_25 = 25f;

  /** 表格列宽比例：25% */
  private static final float TABLE_COLUMN_WIDTH_25 = 25f;

  /** 备注字段最大长度：15 */
  private static final int REMARK_MAX_LENGTH = 15;
   ...
   * @return PDF字节数组
   */
  public byte[] generateCover(final Matter matter, final String archiveNo) {
  ...
  public byte[] generateCover(
    ...
    try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
      ...
      if ("CRIMINAL".equals(caseType)) {
        generateCriminalCover(
            document, matter, archiveNo, volumeCount, pageCount, retentionPeriod, font);
      } else {
        generateCivilCover(
            document, matter, archiveNo, volumeCount, pageCount, retentionPeriod, font);
      }
   ...
   * @param font 字体对象
   */
  private void generateCivilCover(
    ...
    if (firmName != null && !firmName.isEmpty()) {
      Paragraph firmPara =
          new Paragraph(firmName)
              .setFont(font)
              .setFontSize(FONT_SIZE_SUBTITLE)
              .setFontColor(DARK_BROWN)
              .setTextAlignment(TextAlignment.CENTER)
              .setMarginBottom(PARAGRAPH_MARGIN_BOTTOM_15);
      document.add(firmPara);
    }

    // 主标题：业务档案卷宗
    Paragraph title =
        new Paragraph("业 务 档 案 卷 宗")
            .setFont(font)
            .setFontSize(FONT_SIZE_TITLE)
            .setBold()
            .setFontColor(DARK_BROWN)
            .setTextAlignment(TextAlignment.CENTER)
            .setMarginBottom(PARAGRAPH_MARGIN_BOTTOM_20);
    document.add(title);

    // 年度字号行
    String yearNo = buildYearNo(matter, archiveNo);
    Paragraph yearPara =
        new Paragraph(yearNo)
            .setFont(font)
            .setFontSize(FONT_SIZE_SUBTITLE)
            .setFontColor(DARK_BROWN)
            .setTextAlignment(TextAlignment.CENTER)
            .setMarginBottom(PARAGRAPH_MARGIN_BOTTOM_25);
    ...
    addTwoColumnRow(
        mainTable,
        "委 托 人",
        getClientName(matter.getClientId()),
        "承 办 人",
        getLawyerName(matter.getLeadLawyerId()),
        font);
    ...
    String filingDateStr =
        matter.getFilingDate() != null ? matter.getFilingDate().format(formatter) : "    年  月  日";
    String closingDateStr =
        matter.getActualClosingDate() != null
            ? matter.getActualClosingDate().format(formatter)
            : "    年  月  日";
    ...
    addTwoColumnRow(
        ...
        "",
        "备    注",
        matter.getRemark() != null ? truncate(matter.getRemark(), REMARK_MAX_LENGTH) : "",
        font);
   ...
   * @param font 字体对象
   */
  private void generateCriminalCover(
      final Document document,
      final Matter matter,
      final String archiveNo,
      final Integer volumeCount,
    ...
    // 类别 | 年度
    String yearStr =
        matter.getCreatedAt() != null ? String.valueOf(matter.getCreatedAt().getYear()) : "____";
    ...
    String filingDateStr =
        matter.getFilingDate() != null ? matter.getFilingDate().format(formatter) : "    年  月  日";
    addTwoColumnRow(
        mainTable, "收案日期", filingDateStr, "委 托 人", getClientName(matter.getClientId()), font);
    ...
    addTwoColumnRow(
        mainTable,
        "审理结果",
        matter.getOutcome() != null ? truncate(matter.getOutcome(), 10) : "",
        "审    级",
   ...
   * @param font 字体对象
   */
  private void addPartySection(final Table table, final Matter matter, final PdfFont font) {
    // 当事人标签单元格（跨3行）- 使用构造函数指定rowspan
    Cell labelCell =
        new Cell(3, 1) // rowspan=3, colspan=1
            .add(
                new Paragraph("当\n事\n人")
                    .setFont(font)
    ...
    Cell plaintiffValue =
        new Cell(1, 2) // colspan=2
            .add(
                new Paragraph(getClientName(matter.getClientId()))
                    .setFont(font)
                    .setFontSize(FONT_SIZE_NORMAL)
                    .setFontColor(DARK_BROWN))
            .setBorder(new SolidBorder(BORDER_COLOR, BORDER_WIDTH))
    ...
    Cell defendantValue =
        new Cell(1, 2) // colspan=2
            .add(
                new Paragraph(matter.getOpposingParty() != null ? matter.getOpposingParty() : "")
                    .setFont(font)
                    .setFontSize(FONT_SIZE_NORMAL)
                    .setFontColor(DARK_BROWN))
            .setBorder(new SolidBorder(BORDER_COLOR, BORDER_WIDTH))
   ...
   * @param font 字体对象
   */
  private void addResultSection(final Table table, final Matter matter, final PdfFont font) {
    // 审/仲裁/结果标签（跨2行）
    Cell labelCell =
    ...
    Cell firstValue =
        createValueCell(matter.getOutcome() != null ? truncate(matter.getOutcome(), 10) : "", font);
  ...
  private void addFullWidthRow(
      final Table table, final String label, final String value, final PdfFont font) {
    Cell labelCell = createLabelCell(label, font);
    Cell valueCell =
        new Cell(1, 3) // colspan=3
            .add(
   ...
   *
   * @param table 表格对象
   * @param label1 第一个标签文本
   * @param value1 第一个值文本
   * @param label2 第二个标签文本
   * @param value2 第二个值文本
   * @param font 字体对象
   */
   ...
   * @return 年度字号字符串
   */
  private String buildYearNo(final Matter matter, final String archiveNo) {
    int year =
        matter.getCreatedAt() != null ? matter.getCreatedAt().getYear() : LocalDate.now().getYear();
   ...
   * @return 中文年份字符串
   */
  private String convertToChineseYear(final int year) {
   ...
   * @return 律师姓名
   */
  private String getLawyerName(final Long lawyerId) {
    if (lawyerId == null) {
      return "";
    }
    try {
   ...
   * @return 客户名称
   */
  private String getClientName(final Long clientId) {
    if (clientId == null) {
      return "";
    }
    try {
   ...
   * 获取保管期限名称.
   *
   * @param retentionPeriod 保管期限代码
   * @return 保管期限名称
   */
  private String getRetentionPeriodName(final String retentionPeriod) {
   ...
   * @return 案由名称
   */
  private String getCauseOfActionName(final Matter matter) {
