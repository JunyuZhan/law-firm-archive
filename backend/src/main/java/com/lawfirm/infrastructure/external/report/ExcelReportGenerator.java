...
public class ExcelReportGenerator {
    ...
    public ByteArrayInputStream generateMatterReport(List<Map<String, Object>> data) throws IOException {
        ...
        for (Map<String, Object> row : data) {
            Row dataRow = sheet.createRow(rowNum++);
            int colNum = 0;
            dataRow.createCell(colNum++).setCellValue(row.get("matterNo") != null ? row.get("matterNo").toString() : "");
            dataRow.createCell(colNum++).setCellValue(row.get("matterName") != null ? row.get("matterName").toString() : "");
            dataRow.createCell(colNum++).setCellValue(row.get("clientName") != null ? row.get("clientName").toString() : "");
            dataRow.createCell(colNum++).setCellValue(row.get("matterType") != null ? row.get("matterType").toString() : "");
            dataRow.createCell(colNum++).setCellValue(row.get("status") != null ? row.get("status").toString() : "");
            dataRow.createCell(colNum++).setCellValue(row.get("createdAt") != null ? row.get("createdAt").toString() : "");
        }
    ...
    public ByteArrayInputStream generateClientReport(List<Map<String, Object>> data) throws IOException {
        ...
        for (Map<String, Object> row : data) {
            Row dataRow = sheet.createRow(rowNum++);
            int colNum = 0;
            dataRow.createCell(colNum++).setCellValue(row.get("clientNo") != null ? row.get("clientNo").toString() : "");
            dataRow.createCell(colNum++).setCellValue(row.get("clientName") != null ? row.get("clientName").toString() : "");
            dataRow.createCell(colNum++).setCellValue(row.get("clientType") != null ? row.get("clientType").toString() : "");
            dataRow.createCell(colNum++).setCellValue(row.get("contactPerson") != null ? row.get("contactPerson").toString() : "");
            dataRow.createCell(colNum++).setCellValue(row.get("contactPhone") != null ? row.get("contactPhone").toString() : "");
            dataRow.createCell(colNum++).setCellValue(row.get("status") != null ? row.get("status").toString() : "");
        }
    ...
    public ByteArrayInputStream generateLawyerPerformanceReport(List<Map<String, Object>> data) throws IOException {
        ...
        for (Map<String, Object> row : data) {
            Row dataRow = sheet.createRow(rowNum++);
            int colNum = 0;
            dataRow.createCell(colNum++).setCellValue(row.get("lawyer_name") != null ? row.get("lawyer_name").toString() : "");
    ...
    public ByteArrayInputStream generateMatterProgressReport(List<Map<String, Object>> data) throws IOException {
        ...
        for (Map<String, Object> row : data) {
            ...
            matterNameCell.setCellStyle(dataStyle);
            ...
            businessTypeCell.setCellStyle(dataStyle);
            ...
            leadLawyerCell.setCellStyle(dataStyle);
            ...
            totalTasksCell.setCellStyle(dataStyle);
            ...
            completedTasksCell.setCellStyle(dataStyle);
            ...
            inProgressTasksCell.setCellStyle(dataStyle);
            ...
            if (row.get("task_completion_rate") != null) {
                if (row.get("task_completion_rate") instanceof Number) {
                    ...
                    completionRateCell.setCellStyle(percentStyle);
                } else {
                    completionRateCell.setCellValue(row.get("task_completion_rate").toString());
                    completionRateCell.setCellStyle(dataStyle);
                }
            }
            ...
            approvedHoursCell.setCellStyle(numberStyle);
            ...
            expectedEndDateCell.setCellStyle(dataStyle);
            ...
            actualEndDateCell.setCellStyle(dataStyle);
            ...
            estimatedFeeCell.setCellStyle(numberStyle);
        }
    ...
    public ByteArrayInputStream generateMatterTaskReport(List<Map<String, Object>> data) throws IOException {
        ...
        rowNum++;
        Row headerRow = sheet.createRow(rowNum++);
        String[] headers = {"项目编号", "项目名称", "客户名称", "任务编号", "任务标题", "优先级", "状态", "进度(%)", "截止日期", "完成时间", "执行人"};
        for (int i = 0; i < headers.length; i++) {
            Cell cell = headerRow.createCell(i);
            cell.setCellValue(headers[i]);
            cell.setCellStyle(headerStyle);
    ...
    public ByteArrayInputStream generateMatterStageReport(List<Map<String, Object>> data) throws IOException {
        ...
        rowNum++;
        Row headerRow = sheet.createRow(rowNum++);
        String[] headers = {"项目编号", "项目名称", "客户名称", "业务类型", "状态", "主办律师", "部门", "创建日期", "预计完成", "实际完成", "总任务", "已完成", "完成率(%)"};
        for (int i = 0; i < headers.length; i++) {
            Cell cell = headerRow.createCell(i);
            cell.setCellValue(headers[i]);
            cell.setCellStyle(headerStyle);
    ...
    public ByteArrayInputStream generateCostAnalysisReport(List<Map<String, Object>> data) throws IOException {
        ...
        rowNum++;
        Row headerRow = sheet.createRow(rowNum++);
        String[] headers = {"项目编号", "项目名称", "客户名称", "主办律师", "归集成本", "分摊成本", "总成本", "合同金额", "已收款", "利润", "利润率(%)"};
        for (int i = 0; i < headers.length; i++) {
            Cell cell = headerRow.createCell(i);
            cell.setCellValue(headers[i]);
            cell.setCellStyle(headerStyle);
    ...
    public ByteArrayInputStream generateAgingAnalysisReport(List<Map<String, Object>> data) throws IOException {
        ...
        rowNum++;
        Row headerRow = sheet.createRow(rowNum++);
        String[] headers = {"客户名称", "项目名称", "项目编号", "合同编号", "合同金额", "已收款", "应收金额", "账龄天数", "账龄区间", "签署日期"};
        for (int i = 0; i < headers.length; i++) {
            Cell cell = headerRow.createCell(i);
            cell.setCellValue(headers[i]);
            cell.setCellStyle(headerStyle);
    ...
    public ByteArrayInputStream generateProfitAnalysisReport(List<Map<String, Object>> data) throws IOException {
        ...
        rowNum++;
        Row headerRow = sheet.createRow(rowNum++);
        String[] headers = {"项目编号", "项目名称", "客户名称", "主办律师", "业务类型", "状态", 
                           "合同金额", "已收款", "应收金额", "归集成本", "分摊成本", "总成本", 
                           "利润", "利润率(已收款)", "利润率(合同)"};
        for (int i = 0; i < headers.length; i++) {
            Cell cell = headerRow.createCell(i);
            cell.setCellValue(headers[i]);
