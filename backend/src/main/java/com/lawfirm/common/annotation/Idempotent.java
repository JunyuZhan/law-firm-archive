 ...
 * 
 * 与 @RepeatSubmit 的区别：
 * - @RepeatSubmit：防止短时间内重复提交（基于时间窗口）
...
public @interface Idempotent {
    ...
    /**
     * 幂等键（SpEL表达式）
     * 当 keySource 为 SPEL 时使用
     * 支持引用方法参数，如：#command.orderNo, #userId
     */
    String key() default "";
    ...
    /**
     * 是否在执行成功后才标记为已处理
     * true: 方法执行成功后才标记（适用于需要确保执行成功的场景）
     * false: 方法执行前就标记（适用于需要严格防重的场景）
     */
    boolean markAfterSuccess() default false;

    /**
     * 是否返回上次执行的结果
     * 仅当 markAfterSuccess = true 时有效
     */
