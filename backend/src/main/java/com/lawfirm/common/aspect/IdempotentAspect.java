...
public class IdempotentAspect {
     ...
     * 执行前标记（严格防重模式）
     */
    private Object executeWithMarkBefore(ProceedingJoinPoint point, String redisKey, 
                                         long expireSeconds, String message) throws Throwable {
        // 尝试设置幂等键
     ...
     * 执行成功后标记（允许失败重试模式）
     */
    private Object executeWithMarkAfterSuccess(ProceedingJoinPoint point, String redisKey, 
                                               long expireSeconds, String message) throws Throwable {
        // 先检查是否已处理
     ...
     * 解析SpEL表达式
     */
    private String parseSpelKey(ProceedingJoinPoint point, String spelExpression) {
     ...
     * 从请求头获取幂等键
     */
    private String getHeaderKey(String headerName) {
        try {
