...
/** 角色类型枚举 ✅ 修复问题615: 将角色常量硬编码改为枚举 ✅ 修复问题616: 统一角色判断逻辑 */
public enum RoleType {
  ...
  RoleType(
      String code,
      String description,
      boolean canAccessAllContracts,
      boolean canModifyFinance,
      boolean canManageTemplates,
      boolean isSeniorManagement) {
    this.code = code;
   ...
   * @return 是否包含高级管理角色
   */
  public static boolean hasSeniorManagement(final Set<String> roleCodes) {
   ...
   * @return 是否可以访问所有合同
   */
  public static boolean canAccessAllContracts(final Set<String> roleCodes) {
    return roleCodes.stream()
        .map(RoleType::fromCode)
        .filter(r -> r != null)
   ...
   * @return 是否可以修改财务信息
   */
  public static boolean canModifyFinance(final Set<String> roleCodes) {
    return roleCodes.stream()
        .map(RoleType::fromCode)
        .filter(r -> r != null)
   ...
   * @return 是否可以管理模板
   */
  public static boolean canManageTemplates(final Set<String> roleCodes) {
    return roleCodes.stream()
        .map(RoleType::fromCode)
        .filter(r -> r != null)
  ...
  public static Set<String> getSeniorManagementCodes() {
    return SENIOR_MANAGEMENT_ROLES.stream().map(RoleType::getCode).collect(Collectors.toSet());
  }
  ...
  public static Set<String> getTemplateManageCodes() {
    return TEMPLATE_MANAGE_ROLES.stream().map(RoleType::getCode).collect(Collectors.toSet());
  }
}
