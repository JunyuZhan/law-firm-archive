...
public final class MoneyUtils {
  ...
  /**
   * 将金额转换为中文大写 例如：10000.50 -> 壹万元伍角整 123456.78 -> 壹拾贰万叁仟肆佰伍拾陆元柒角捌分
   *
   * @param amount 金额（支持 BigDecimal、Double、Long、Integer、String）
   * @return 中文大写金额
   */
  public static String toChinese(final Object amount) {
    if (amount == null) {
      return "";
    }
    ...
    int decimalPart =
        money
            .subtract(BigDecimal.valueOf(integerPart))
   ...
   * @return 中文大写
   */
  private static String convertIntegerPart(final long number) {
    if (number == 0) {
    ...
    while (remainingNumber > 0) {
      int group = (int) (remainingNumber % TEN_THOUSAND);
      remainingNumber /= TEN_THOUSAND;
   ...
   *
   * @param number 数字
   * @param prefixZero 是否前缀零
   * @return 中文大写
   */
  private static String convertGroup(final int number, final boolean prefixZero) {
    if (number == 0) {
      return "";
    }
    ...
    for (int i = 3; i >= 0; i--) {
      if (digits[i] != 0) {
        ...
        hasValue = true;
          ...
          && i > 0
          && (digits[i - 1] != 0 || digits[i] == 0 && needsZero(digits, i))) {
        // 中间的零需要读出
   ...
   * @return 是否需要添加零
   */
  private static boolean needsZero(final int[] digits, final int currentIndex) {
  ...
  /**
   * 格式化金额显示（带千分位） 例如：1234567.89 -> 1,234,567.89
   *
   * @param amount 金额
   * @return 格式化后的字符串
   */
  public static String formatWithComma(final Object amount) {
    if (amount == null) {
      return "0.00";
    }
  ...
  /**
   * 简化的金额大写（不带角分） 例如：10000 -> 壹万元整
   *
   * @param amount 金额
   * @return 中文大写（精确到元）
   */
  public static String toChineseYuan(final Object amount) {
    if (amount == null) {
      return "";
    }
