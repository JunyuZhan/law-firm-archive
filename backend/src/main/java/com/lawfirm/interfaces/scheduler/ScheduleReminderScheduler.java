 ...
 * 
 * 功能：
 * 1. 每5分钟检查需要提醒的日程，根据用户设置的提前提醒时间发送通知
 */
@Slf4j
@Component
    ...
    public void sendScheduleReminders() {
        ...
        for (Schedule schedule : schedules) {
            try {
                ...
                String message = String.format("【日程提醒】您有一个%s【%s】即将开始%s",
                        typeLabel, schedule.getTitle(), location);
                ...
                sentCount++;
                log.debug("发送日程提醒: scheduleId={}, userId={}, title={}", 
                        schedule.getId(), schedule.getUserId(), schedule.getTitle());
