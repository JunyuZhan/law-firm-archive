...
/**
 * 合同到期提醒定时调度器 问题255实现：合同到期提醒功能
 *
 * <p>功能： 1. 每天上午9点检查即将到期的合同（30天内），发送提醒通知给承办律师 2. 每天上午10点检查已逾期的合同，发送逾期警告
 */
@Slf4j
@Component
...
public class ContractExpiryReminderScheduler {
  ...
  public void sendUpcomingContractReminders() {
    ...
    // 1. 30天内到期的合同（首次提醒）
    List<Contract> contracts30Days =
        contractRepository.findExpiringContracts(today, today.plusDays(REMINDER_DAYS_30));
   ...
   * @return 发送成功的通知数量
   */
  private int sendContractReminder(
      final Contract contract, final long days, final boolean isOverdue) {
    int sentCount = 0;
    ...
    try {
      ...
      if (isOverdue) {
        ...
        message =
            String.format(
                "【逾期警告】合同【%s】（编号：%s）已逾期%d天（到期日期：%s），请尽快处理续签或归档！",
        ...
        message =
            String.format(
                "%s合同【%s】（编号：%s）将于%d天后到期（%s），请及时处理续签事宜！",
                urgency,
                contract.getName(),
      ...
      notificationAppService.sendSystemNotification(
          lead.getUserId(), title, message, "CONTRACT", contract.getId());

      sentCount++;
      log.debug(
          "发送合同{}提醒: contractId={}, userId={}, days={}",
          isOverdue ? "逾期" : "到期",
          contract.getId(),
          lead.getUserId(),
          days);
      ...
      log.error(
          "发送合同提醒失败: contractId={}, contractNo={}", contract.getId(), contract.getContractNo(), e);
    }
