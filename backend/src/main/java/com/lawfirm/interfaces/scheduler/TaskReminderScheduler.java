 ...
 * 
 * 功能：
 * 1. 每天上午9点检查即将到期的任务（3天内），发送提醒通知
 * 2. 每天上午10点检查已逾期的任务，发送逾期警告
 * 3. 检查设置了提醒时间的任务，在指定时间发送提醒
 */
@Slf4j
@Component
    ...
    public void sendUpcomingTaskReminders() {
        ...
        for (Task task : upcomingTasks) {
            try {
                ...
                String message = String.format("%s任务【%s】将于%d天后到期（%s），请及时处理！",
                ...
                sentCount++;
                log.debug("发送任务到期提醒: taskId={}, assigneeId={}, daysRemaining={}", 
                        task.getId(), task.getAssigneeId(), daysRemaining);
    ...
    public void sendOverdueTaskWarnings() {
        ...
        for (Task task : overdueTasks) {
            try {
                ...
                String message = String.format("【逾期警告】任务【%s】已逾期%d天（截止日期：%s），请尽快处理！",
                ...
                sentCount++;
                log.debug("发送逾期任务警告: taskId={}, assigneeId={}, daysOverdue={}", 
                        task.getId(), task.getAssigneeId(), daysOverdue);
    ...
    public void sendScheduledReminders() {
        ...
        for (Task task : tasksNeedReminder) {
            try {
                ...
                String message = String.format("任务提醒：【%s】%s",
                        task.getTitle(),
                        task.getDueDate() != null ? "，截止日期：" + task.getDueDate() : "");
