...
public class RegularizationAppService {
  ...
  /**
   * 分页查询转正申请 ✅ 优化：使用批量加载避免N+1查询.
   *
   * @param query 查询条件
   * @return 分页结果
   */
  public PageResult<RegularizationDTO> listRegularizations(final RegularizationQueryDTO query) {
    IPage<Regularization> page =
        regularizationMapper.selectRegularizationPage(
   ...
   * 批量加载员工信息.
   *
   * @param regularizations 转正申请列表
   * @return 员工映射表
   */
  private Map<Long, Employee> batchLoadEmployees(final List<Regularization> regularizations) {
  ...
  private Map<Long, User> batchLoadUsers(
      final List<Regularization> regularizations, final Map<Long, Employee> employeeMap) {
   ...
   * @return 转正申请DTO
   */
  public RegularizationDTO getRegularizationById(final Long id) {
   ...
   * @return 转正申请DTO列表
   */
  public List<RegularizationDTO> getRegularizationsByEmployeeId(final Long employeeId) {
   ...
   */
  @Transactional
  public RegularizationDTO createRegularization(final CreateRegularizationCommand command) {
    // 验证员工存在
    Employee employee = employeeRepository.getByIdOrThrow(command.getEmployeeId(), "员工不存在");

    // 检查是否已有待审批的转正申请
    List<Regularization> pendingApplications =
        regularizationRepository.findByEmployeeId(command.getEmployeeId()).stream()
   ...
   */
  @Transactional
  public RegularizationDTO approveRegularization(
      final Long id, final ApproveRegularizationCommand command) {
    Regularization regularization = regularizationRepository.getByIdOrThrow(id, "转正申请不存在");
    ...
    if (command.getApproved()) {
      Employee employee =
          employeeRepository.getByIdOrThrow(regularization.getEmployeeId(), "员工不存在");
      employee.setRegularDate(
          regularization.getExpectedRegularDate() != null
              ? regularization.getExpectedRegularDate()
              : LocalDate.now());
   ...
   */
  @Transactional
  public void deleteRegularization(final Long id) {
    Regularization regularization = regularizationRepository.getByIdOrThrow(id, "转正申请不存在");
    if (!"PENDING".equals(regularization.getStatus())) {
   ...
   *
   * @param regularization 转正申请实体
   * @return 转正申请DTO
   */
  private RegularizationDTO toDTO(final Regularization regularization) {
    // 单条查询时构建临时Map
   ...
   * @return 转正申请DTO
   */
  private RegularizationDTO toDTO(
