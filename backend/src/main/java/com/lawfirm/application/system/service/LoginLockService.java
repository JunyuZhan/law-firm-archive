 ...
 * 登录锁定服务
 * 
 * 实现登录失败后的账户锁定机制，防止暴力破解
 * 
 * 策略：
 * - 连续失败 3 次：需要图形验证码
 * - 连续失败 5 次：锁定账户 15 分钟
 * - 连续失败 10 次：锁定账户 1 小时
 * - 连续失败 20 次：锁定账户 24 小时
 * 
 * @author system
 */
@Slf4j
...
public class LoginLockService {
    ...
    public LockStatus checkLockStatus(String username) {
        ...
        if (ttl != null && ttl > 0) {
            int remainingMinutes = (int) Math.ceil(ttl / 60.0);
     ...
     * @return 处理结果
     */
    public FailResult recordFailure(String username) {
     ...
     * @return 处理结果
     */
    public FailResult recordFailure(String username, String ip) {
        String failCountKey = FAIL_COUNT_PREFIX + username;
     ...
     * 检查告警是否启用
     */
    private boolean isAlertEnabled(String configKey) {
     ...
     * 锁定账户
     */
    private void lockAccount(String username, int durationMinutes, String ip, String reason) {
        String lockKey = LOCK_PREFIX + username;
        redisTemplate.opsForValue().set(lockKey, "1", durationMinutes, TimeUnit.MINUTES);
        log.warn("账户已锁定: username={}, durationMinutes={}, reason={}", username, durationMinutes, reason);
        
        // 发送账户锁定告警邮件
        if (isAlertEnabled("notification.alert.account.locked")) {
            try {
                alertService.sendAccountLockedAlert(username, ip != null ? ip : "未知", 
                        reason + "，锁定" + durationMinutes + "分钟");
            } catch (Exception e) {
    ...
    public static class LockStatus {
        ...
        public static LockStatus locked(int remainingMinutes) {
    ...
    public static class FailResult {
        ...
        public static FailResult locked(int durationMinutes, int failCount) {
        ...
        public static FailResult captchaRequired(int failCount) {
            return FailResult.builder()
                    .locked(false)
                    .captchaRequired(true)
                    .failCount(failCount)
        ...
        public static FailResult normal(int failCount, int remainingBeforeCaptcha) {
            return FailResult.builder()
                    .locked(false)
