...
public class LoginLocationService {
     ...
     * @return 检查结果
     */
    public LocationCheckResult checkLocation(Long userId, String ip) {
        // 检查是否启用异地登录检测
        ...
        if (trustedLocations.isEmpty()) {
            log.info("用户首次登录，记录位置: userId={}, location={}", userId, currentLocation.getShortDescription());
            return LocationCheckResult.firstLogin(currentLocation);
        }
        
        // 检查是否为常用位置
        String currentLocationKey = isCityLevel ? currentLocation.getCity() : currentLocation.getProvince();
        if (currentLocationKey != null && trustedLocations.contains(currentLocationKey)) {
            return LocationCheckResult.normal(currentLocation);
        }
        
        // 异地登录
        log.warn("检测到异地登录: userId={}, currentLocation={}, trustedLocations={}, level={}", 
                userId, currentLocation.getShortDescription(), trustedLocations, isCityLevel ? "city" : "province");
    ...
    private Set<String> getTrustedLocations(Long userId, boolean isCityLevel) {
     ...
     */
    public void recordLogin(Long userId, String ip, boolean success) {
        if (!success) {
            return;
        }
        ...
        log.debug("记录登录位置: userId={}, ip={}, location={}, level={}", 
                userId, ip, location.getShortDescription(), isCityLevel ? "city" : "province");
    }

    /**
     * 更新登录位置历史
     */
    private void updateLocationHistory(Long userId, String ip, IpLocation location) {
     ...
     */
    private void updateTrustedLocation(Long userId, String locationKey, boolean isCityLevel) {
        String suffix = isCityLevel ? ":city" : "";
     ...
     */
    public String requestPermitCode(Long userId, String username, String ip, IpLocation location) {
        // 生成请求ID
        ...
        if ("fixed".equalsIgnoreCase(mode)) {
            ...
            // 随机码模式：生成6位随机数字
            permitCode = String.format("%06d", ThreadLocalRandom.current().nextInt(1000000));
        ...
        log.info("生成异地登录许可码请求: requestId={}, username={}, location={}, mode={}", 
                requestId, username, location.getShortDescription(), mode);
     ...
     * 发送许可码邮件给管理员
     */
    private void sendPermitCodeToAdmin(String username, String ip, IpLocation location, String permitCode) {
        ...
        String content = String.format("""
            ...
            <body style="font-family: Arial, sans-serif; line-height: 1.6;">
                ...
                <table style="border-collapse: collapse; margin: 20px 0;">
                    <tr><td style="padding: 8px; border: 1px solid #ddd; background: #f5f5f5;"><strong>用户名</strong></td><td style="padding: 8px; border: 1px solid #ddd;">%s</td></tr>
                    <tr><td style="padding: 8px; border: 1px solid #ddd; background: #f5f5f5;"><strong>登录IP</strong></td><td style="padding: 8px; border: 1px solid #ddd;">%s</td></tr>
                    <tr><td style="padding: 8px; border: 1px solid #ddd; background: #f5f5f5;"><strong>登录位置</strong></td><td style="padding: 8px; border: 1px solid #ddd;">%s</td></tr>
                    <tr><td style="padding: 8px; border: 1px solid #ddd; background: #f5f5f5;"><strong>请求时间</strong></td><td style="padding: 8px; border: 1px solid #ddd;">%s</td></tr>
                </table>
                <p style="font-size: 24px; color: #3498db; background: #ecf0f1; padding: 15px; text-align: center; letter-spacing: 8px;">
                    许可码：<strong>%s</strong>
                </p>
            ...
            """, username, ip, location.getShortDescription(), time, permitCode, PERMIT_CODE_EXPIRE_MINUTES);
     ...
     */
    @SuppressWarnings("unchecked")
    public PermitCodeVerifyResult verifyPermitCode(String requestId, String inputCode, Long userId) {
     ...
     */
    public void trustCurrentLocation(Long userId, String username, String ip) {
        IpLocation location = ipLocationService.getLocation(ip);
        String province = location.getProvince();
        String city = location.getCity();
        ...
        if (locationKey != null) {
            ...
            log.info("用户添加新的常用位置: userId={}, location={}, level={}", 
                    userId, locationKey, isCityLevel ? "city" : "province");
    ...
    public static class PermitCodeVerifyResult {
        ...
        public static PermitCodeVerifyResult success(String ip) {
        ...
        public static PermitCodeVerifyResult fail(String message) {
            PermitCodeVerifyResult result = new PermitCodeVerifyResult();
     ...
     */
    @SuppressWarnings("unchecked")
    public List<LoginRecord> getLoginHistory(Long userId, int limit) {
        String key = LOGIN_LOCATION_PREFIX + userId;
    ...
    public static class LocationCheckResult {
        ...
        public static LocationCheckResult firstLogin(IpLocation location) {
            LocationCheckResult result = new LocationCheckResult();
            result.setNewLocation(false);
        ...
        public static LocationCheckResult newLocation(IpLocation location, Set<String> trustedProvinces) {
            LocationCheckResult result = new LocationCheckResult();
            result.setNewLocation(true);
            result.setFirstLogin(false);
            result.setCurrentLocation(location);
            result.setTrustedProvinces(trustedProvinces);
            result.setMessage(String.format("检测到您从新的位置（%s）登录，请完成安全验证", 
