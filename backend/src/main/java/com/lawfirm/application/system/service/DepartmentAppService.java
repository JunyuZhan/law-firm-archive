...
public class DepartmentAppService {
  ...
  /**
   * 获取部门列表（平铺） 问题449修复：使用批量加载避免N+1查询
   *
   * @return 所有部门列表
   */
   ...
   * @param command 创建命令
   * @return 创建后的部门DTO
   * @throws BusinessException 如果父部门不存在
   */
  @Transactional
  public DepartmentDTO createDepartment(final CreateDepartmentCommand command) {
    // 验证父部门
  ...
  /**
   * 更新部门 问题461修复：检查循环引用（不只是直接循环）
   *
   * @param command 更新命令
   * @return 更新后的部门DTO
   * @throws BusinessException 如果部门不存在或形成循环引用
   */
  @Transactional
  public DepartmentDTO updateDepartment(final UpdateDepartmentCommand command) {
   ...
   * @return 是否形成循环
   */
  private boolean willFormCycle(final Long deptId, final Long newParentId) {
  ...
  /**
   * 删除部门 问题456修复：递归检查所有子孙部门
   *
   * @param id 部门ID
   * @throws BusinessException 如果部门不存在或存在子部门/用户
   */
  @Transactional
   ...
   * @return 是否有子孙部门
   */
  private boolean hasDescendants(final Long deptId) {
   ...
   * @throws BusinessException 如果部门或负责人不存在
   */
  @Transactional
  public void setLeader(final Long deptId, final Long leaderId) {
   ...
   * 构建树形结构
   *
   * @param dtoList 部门DTO列表
   * @return 树形结构的部门DTO列表
   */
  private List<DepartmentDTO> buildTree(final List<DepartmentDTO> dtoList) {
    Map<Long, DepartmentDTO> dtoMap =
        dtoList.stream().collect(Collectors.toMap(DepartmentDTO::getId, d -> d));
   ...
   * @return 部门DTO列表
   */
  private List<DepartmentDTO> convertToDTOs(final List<Department> depts) {
    if (depts.isEmpty()) {
      return Collections.emptyList();
    }
    ...
    // 转换DTO（从Map获取）
    return depts.stream().map(d -> toDTO(d, leaderMap)).collect(Collectors.toList());
  }
   ...
   * @return 部门DTO
   */
  private DepartmentDTO toDTO(final Department dept) {
    // 单个转换时仍查询关联数据
   ...
   *
   * @param dept 部门实体
   * @param leaderMap 负责人Map
   * @return 部门DTO
   */
  private DepartmentDTO toDTO(final Department dept, final Map<Long, User> leaderMap) {
    DepartmentDTO dto =
    ...
    if (dept.getLeaderId() != null && leaderMap.containsKey(dept.getLeaderId())) {
      dto.setLeaderName(leaderMap.get(dept.getLeaderId()).getRealName());
    }
