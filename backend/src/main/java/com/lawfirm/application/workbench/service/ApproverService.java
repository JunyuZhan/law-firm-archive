...
public class ApproverService {
     ...
     * 1. 当前部门负责人
     * 2. 上级部门负责人（沿部门层级向上）
     * 3. 团队负责人（TEAM_LEADER角色）
     * 4. 主任（DIRECTOR角色）
     * 
     * @param applicantId 申请人ID，如果为null则使用当前登录用户
     * @return 审批人ID
     */
    public Long findSealApplicationApprover(Long applicantId) {
        if (applicantId == null) {
            applicantId = SecurityUtils.getUserId();
        }
        
        User applicant = userRepository.getById(applicantId);
        if (applicant == null) {
    ...
    /**
     * 获取用印申请可选审批人列表（申请人架构垂直线上的领导）
     * 包括：当前部门负责人、上级部门负责人、团队负责人、主任
     * 
     * @param applicantId 申请人ID，如果为null则使用当前登录用户
     * @return 可选审批人列表
     */
    public List<Map<String, Object>> getSealApplicationAvailableApprovers(Long applicantId) {
        if (applicantId == null) {
            applicantId = SecurityUtils.getUserId();
        }
     ...
     * 查找数据交接审批人
     * 规则：
     * - RESIGNATION（离职交接）：由主任审批（人员变动需最高级别审批）
     * - PROJECT（项目移交）：由团队负责人或主任审批
     * - CLIENT（客户移交）：由团队负责人或主任审批
     * - LEAD（案源移交）：由团队负责人或主任审批
     */
    public Long findDataHandoverApprover(String handoverType) {
