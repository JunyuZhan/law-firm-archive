...
public class StatisticsAppService {
  ...
  /** 项目参与人仓储 */
  private final com.lawfirm.domain.matter.repository.MatterParticipantRepository
      matterParticipantRepository;

  /** 函件申请仓储 */
  private final com.lawfirm.domain.admin.repository.LetterApplicationRepository
      letterApplicationRepository;

  /** 用印申请仓储 */
  private final com.lawfirm.domain.document.repository.SealApplicationRepository
      sealApplicationRepository;

  /** 请假申请仓储 */
  private final com.lawfirm.domain.admin.repository.LeaveApplicationRepository
  ...
  /**
   * 获取部门及所有下级部门ID列表 使用递归CTE一次性查询，并添加缓存
   *
   * @param deptId 部门ID
   * @return 部门ID列表
   */
  ...
  public StatisticsDTO.RevenueStats getRevenueStats() {
    ...
    log.info(
        "获取收入统计: total={}, monthly={}, yearly={}, dataScope={}",
        totalRevenue,
        monthlyRevenue,
        yearlyRevenue,
        dataScope);
  ...
  public StatisticsDTO.MatterStats getMatterStats() {
    ...
    try {
      ...
      List<Map<String, Object>> statusCountList =
          statisticsMapper.countMattersByStatus(accessibleMatterIds);
      ...
      // 各类型案件数
      List<Map<String, Object>> typeCountList =
          statisticsMapper.countMattersByType(accessibleMatterIds);
  ...
  public StatisticsDTO.ClientStats getClientStats() {
    ...
    // 各类型客户数
    List<Map<String, Object>> typeCountList =
        statisticsMapper.countClientsByType(accessibleClientIds);
    Map<String, Long> typeCount = new HashMap<>();
    if (typeCountList != null) {
      for (Map<String, Object> item : typeCountList) {
        if (item == null) {
    ...
    log.info(
        "获取客户统计: total={}, formal={}, potential={}, newThisMonth={}, dataScope={}",
        totalClients,
        formalClients,
        potentialClients,
        newClientsThisMonth,
  ...
  /**
   * 获取律师业绩排行（根据权限过滤） ✅ 修复问题553: 使用批量查询避免N+1查询
   *
   * @param limit 限制数量
   * @return 律师业绩排行列表
   */
  public List<StatisticsDTO.LawyerPerformance> getLawyerPerformanceRanking(final Integer limit) {
    // 根据用户权限过滤数据
    String dataScope = com.lawfirm.common.util.SecurityUtils.getDataScope();
    Long currentUserId = com.lawfirm.common.util.SecurityUtils.getUserId();
    Long deptId = com.lawfirm.common.util.SecurityUtils.getDepartmentId();
    ...
    List<Map<String, Object>> rankings =
        statisticsMapper.getLawyerPerformanceRanking(limit, accessibleMatterIds);
    ...
    for (Map<String, Object> item : rankings) {
      ...
      performance.setMatterCount(
          matterCountObj != null ? ((Number) matterCountObj).longValue() : 0L);
      Object revenueObj = item.get("total_revenue");
      performance.setRevenue(
          revenueObj != null ? new BigDecimal(revenueObj.toString()) : BigDecimal.ZERO);
  ...
  /**
   * 获取工作台统计数据（用于仪表盘） 根据用户角色返回不同的统计数据： - 律师/团队负责人/主任：项目数、客户数、工时、任务数 - 财务：待确认收款、待开票、待审批报销、本月已收金额 -
   ...
   * 加载律师相关统计数据
   *
   * @param stats 工作台统计数据DTO
   * @param userId 用户ID
   */
  private void loadLawyerStats(final WorkbenchStatsDTO stats, final Long userId) {
    // 我的项目数（我参与的项目）
    var participantList =
        matterParticipantRepository
            .lambdaQuery()
    ...
    BigDecimal monthlyHours =
        timesheetRepository.sumHoursByUserAndMonth(userId, now.getYear(), now.getMonthValue());
   ...
   * @param stats 工作台统计数据DTO
   */
  private void loadFinanceStats(final WorkbenchStatsDTO stats) {
    try {
      // 待确认收款数
   ...
   * @param stats 工作台统计数据DTO
   */
  private void loadAdminStats(final WorkbenchStatsDTO stats) {
    try {
      ...
      long pendingLetterCount =
          letterApplicationRepository
              .lambdaQuery()
              .in(
                  com.lawfirm.domain.admin.entity.LetterApplication::getStatus,
                  "PENDING",
                  "APPROVED")
      ...
      // 待处理用印数
      long pendingSealCount =
          sealApplicationRepository
              .lambdaQuery()
              .in(
                  com.lawfirm.domain.document.entity.SealApplication::getStatus,
                  "PENDING",
                  "APPROVED")
  ...
  /** 工作台统计数据DTO 根据角色类型返回不同的统计数据 */
  @lombok.Data
  public static class WorkbenchStatsDTO {
    /** 待办任务数 */
    private Long taskCount;

    /** 角色类型：LAWYER/FINANCE/ADMIN_STAFF */
    ...
    /** 本月已收金额 */
    private java.math.BigDecimal monthlyReceivedAmount;

    /** 待处理出函数 */
    private Long pendingLetterCount;

    /** 待处理用印数 */
    private Long pendingSealCount;
   ...
   * 计算增长率（与上月对比）
   *
   * @param currentMonth 本月金额
   * @return 增长率
   */
  private BigDecimal calculateGrowthRate(final BigDecimal currentMonth) {
  ...
  /**
   * 获取可访问的项目ID列表（根据数据权限） ✅ 修复问题554: 使用ThreadLocal缓存避免同一请求内重复查询
   *
   ...
   * @return 可访问的项目ID列表
   */
  private List<Long> calculateAccessibleMatterIds(
      final String dataScope, final Long currentUserId, final Long deptId) {
  ...
  /**
   * 获取可访问的客户ID列表（根据数据权限） ✅ 修复问题558: 使用ThreadLocal缓存避免同一请求内重复查询
   *
   * @param dataScope 数据权限范围
   * @param currentUserId 当前用户ID
   * @param deptId 部门ID
   ...
   */
  private List<Long> getAccessibleClientIds(
      final String dataScope, final Long currentUserId, final Long deptId) {
    if ("ALL".equals(dataScope)) {
   ...
   * @return 可访问的客户ID列表
   */
  private List<Long> calculateAccessibleClientIds(
      final String dataScope, final Long currentUserId, final Long deptId) {
