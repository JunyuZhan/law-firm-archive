...
/**
 * 合同数据权限服务 ✅ 修复问题603-604: 添加ThreadLocal缓存减少数据库查询
 *
 * <p>Requirements: 8.1, 8.2, 8.3, 8.5
 */
@Slf4j
@Service
...
public class ContractDataPermissionService {
  ...
  /** ThreadLocal缓存：在同一请求中缓存可访问的合同ID 避免同一请求中多次查询数据库 */
   ...
   * <p>Requirements: 8.1
   *
   * @return 如果可以访问所有合同返回true，否则返回false
   */
   ...
   * <p>Requirements: 8.2
   *
   * @return 如果是财务角色返回true，否则返回false
   */
   ...
   * <p>Requirements: 8.3
   *
   * @return 如果是行政角色返回true，否则返回false
   */
   ...
   * 检查当前用户是否是律师角色
   *
   * @return 如果是律师角色返回true，否则返回false
   */
   ...
   *
   * @return 如果是高级管理角色返回true，否则返回false
   */
  ...
  /**
   * 获取当前用户可访问的合同ID列表 如果用户可以访问所有合同，返回null表示不需要过滤 ✅ 修复问题603: 使用ThreadLocal缓存减少重复查询
   *
   * <p>Requirements: 8.1
   *
   * @return 可访问的合同ID列表，如果可以访问所有合同则返回null
   */
  ...
  /**
   * 检查当前用户是否可以访问指定合同 ✅ 修复问题604: 使用ThreadLocal缓存减少数据库查询
   *
   * <p>Requirements: 8.1
   *
   ...
   */
  public boolean canAccessContract(final Long contractId) {
    if (canAccessAllContracts()) {
      return true;
    }
  ...
  /** 清除ThreadLocal缓存（在请求结束时调用） 建议在Filter或Interceptor中调用 */
   ...
   *
   * @param contractId 合同ID
   * @return 如果可以修改财务信息返回true，否则返回false
   */
  public boolean canModifyFinanceInfo(final Long contractId) {
    Set<String> roles = SecurityUtils.getRoles();
   ...
   * <p>Requirements: 8.3
   *
   * @return 如果只能只读访问返回true，否则返回false
   */
