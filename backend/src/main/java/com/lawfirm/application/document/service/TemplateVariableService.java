...
public class TemplateVariableService {
   ...
   * @return 包含所有变量的Map集合
   */
  public Map<String, Object> collectVariables(final Long matterId) {
    ...
    if (matterClients != null && !matterClients.isEmpty()) {
      ...
      MatterClient primaryClient =
          matterClients.stream()
  ...
  public Map<String, Object> collectVariablesWithContractAndApproval(
      final Long matterId, final Long contractId, final Approval approval) {
   ...
   * @param matter 案件信息
   */
  private void collectMatterVariables(final Map<String, Object> variables, final Matter matter) {
    ...
    variables.put(
        "matter.causeOfActionName",
        getCauseOfActionName(matter.getCauseOfAction(), matter.getCaseType()));
    variables.put("matter.litigationStage", nullToEmpty(matter.getLitigationStage()));
    variables.put(
        "matter.litigationStageName", getLitigationStageName(matter.getLitigationStage()));
    ...
    if (matter.getExpectedClosingDate() != null) {
      variables.put(
          "matter.expectedClosingDate", matter.getExpectedClosingDate().format(DATE_FORMATTER));
    } else {
   ...
   * @param client 客户信息
   */
  private void collectClientVariables(final Map<String, Object> variables, final Client client) {
   ...
   * @param lawyer 律师信息
   */
  private void collectLawyerVariables(final Map<String, Object> variables, final User lawyer) {
  ...
  /**
   * 收集多个委托人变量（用于模板自动填充多个委托人）。 格式：每个委托人按模板格式格式化，多个委托人用换行分隔。
   *
   * @param variables 变量集合
   * @param matterClients 案件委托人列表
   */
  private void collectMultipleClientsVariables(
      final Map<String, Object> variables, final List<MatterClient> matterClients) {
    if (matterClients == null || matterClients.isEmpty()) {
        ...
        matterClients.stream()
            .map(
                mc -> {
                  ...
                  String idLabel =
                      "ENTERPRISE".equals(client.getClientType()) ? "统一社会信用代码" : "身份证号";
                  String idNumber =
                      "ENTERPRISE".equals(client.getClientType())
                          ? nullToEmpty(client.getCreditCode())
                          : nullToEmpty(client.getIdCard());
  ...
  /**
   * 收集多个受托人（律师）变量（用于模板自动填充多个受托人）。 格式：律所信息 + 所有律师信息。
   *
   * @param variables 变量集合
   * @param participants 案件参与人列表
   */
  private void collectMultipleLawyersVariables(
      final Map<String, Object> variables, final List<MatterParticipant> participants) {
    if (participants == null || participants.isEmpty()) {
   ...
   * @param contract 合同信息
   */
  private void collectContractVariables(
      final Map<String, Object> variables, final Contract contract) {
    ...
    String authType =
        "LITIGATION".equals(contract.getContractType())
                || "CIVIL".equals(contract.getCaseType())
                || "CRIMINAL".equals(contract.getCaseType())
            ? "SPECIAL"
            : "GENERAL";
   ...
   * @param approval 审批信息
   */
  private void collectApprovalVariables(
      final Map<String, Object> variables, final Approval approval) {
   ...
   * @param variables 变量集合
   */
  private void collectDateVariables(final Map<String, Object> variables) {
   ...
   * @param variables 变量集合
   */
  private void collectFirmVariables(final Map<String, Object> variables) {
    // 从系统配置中获取律所信息
    String firmName = getConfigValue("firm.name", "律师事务所");
    String firmAddress = getConfigValue("firm.address", "");
  ...
  /**
   * 替换模板内容中的变量。 支持 ${variable.name} 格式的变量。
   *
   ...
   * @return 案件类型名称
   */
  private String getCaseTypeName(final String caseType) {
    if (caseType == null) {
      return "";
    }
   ...
   * 获取项目类型名称。
   *
   * @param matterType 项目类型代码
   * @return 项目类型名称
   */
  private String getMatterTypeName(final String matterType) {
    if (matterType == null) {
      return "";
    }
   ...
   * 获取客户类型名称。
   *
   * @param clientType 客户类型代码
   * @return 客户类型名称
   */
  private String getClientTypeName(final String clientType) {
    if (clientType == null) {
      return "";
    }
   ...
   * 获取收费方式名称。
   *
   * @param feeType 收费方式代码
   * @return 收费方式名称
   */
  private String getFeeTypeName(final String feeType) {
    if (feeType == null) {
      return "";
    }
   ...
   * @return 审批状态名称
   */
  private String getApprovalStatusName(final String status) {
    if (status == null) {
      return "";
    }
   ...
   * @return 代理权限范围描述
   */
  private String getAuthorizationScope(final String authType) {
   ...
   * 获取审理阶段名称。
   *
   * @param trialStage 审理阶段代码
   * @return 审理阶段名称
   */
  private String getTrialStageName(final String trialStage) {
    if (trialStage == null) {
      return "一审";
    }
   ...
   * @return 代理阶段名称
   */
  private String getLitigationStageName(final String litigationStage) {
   ...
   * @return 中文大写金额
   */
  private String convertToChineseAmount(final BigDecimal amount) {
    ...
    while (integerPart > 0) {
      int group = (int) (integerPart % TEN_THOUSAND);
      if (group > 0) {
   ...
   * @return 案由名称
   */
  private String getCauseOfActionName(final String causeOfAction, final String caseType) {
