...
public class FileAccessService {
     ...
     */
    public Map<String, String> uploadFile(MultipartFile file, String fileType, Long matterId, String folder) {
        // 1. 验证文件
        ...
        try {
            ...
            log.info("文件上传成功: fileName={}, storagePath={}, physicalName={}, hash={}", 
    ...
    /**
     * 从MinIO URL解析存储信息
     * 用于向后兼容：当只提供了fileUrl时，尝试解析并设置新字段
     * 
     * @param fileUrl MinIO文件URL
     * @return 包含存储信息的Map：bucketName, storagePath, physicalName（如果无法解析则返回null）
     */
    public Map<String, String> parseStorageInfoFromUrl(String fileUrl) {
    ...
    /**
     * 获取文档访问URL
     * 优先使用新字段（bucketName, storagePath, physicalName），回退到filePath
     * 
     * @param document 文档实体
     * @param accessType 访问类型（DOWNLOAD或VIEW，字符串）
     * @return 文件访问URL
     */
    public String getDocumentAccessUrl(com.lawfirm.domain.document.entity.Document document, String accessType) {
        if (document == null) {
            return null;
        }
        ...
        if (document.getStoragePath() != null && document.getPhysicalName() != null) {
            objectName = MinioPathGenerator.buildObjectName(document.getStoragePath(), document.getPhysicalName());
