...
public class StateCompensationAppService {
   ...
   * @return 国家赔偿信息DTO
   */
  public StateCompensationDTO getByMatterId(final Long matterId) {
    // 验证案件存在
   ...
   */
  @Transactional
  public StateCompensationDTO create(final CreateStateCompensationCommand command) {
    // 验证案件存在
    ...
    if ("STATE_COMP_CRIMINAL".equals(caseType)
        && !Boolean.TRUE.equals(command.getCriminalCaseTerminated())) {
   ...
   */
  @Transactional
  public StateCompensationDTO update(final UpdateStateCompensationCommand command) {
    MatterStateCompensation entity =
        stateCompensationRepository.getByIdOrThrow(command.getId(), "国家赔偿信息不存在");
   ...
   * @return DTO
   */
  private StateCompensationDTO toDTO(final MatterStateCompensation entity, final Matter matter) {
   ...
   * @return 名称
   */
  private String getObligorOrgTypeName(final String type) {
    if (type == null) {
      return null;
    }
    return switch (type) {
   ...
   * @return 名称
   */
  private String getCaseSourceName(final String source) {
    if (source == null) {
      return null;
    }
   ...
   * @return 名称
   */
  private String getDecisionResultName(final String result) {
    if (result == null) {
      return null;
    }
