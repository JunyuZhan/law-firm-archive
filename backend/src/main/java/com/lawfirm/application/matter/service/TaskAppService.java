...
public class TaskAppService {
  ...
  /** 接近完成的进度值（用于退回时） */
  private static final int ALMOST_COMPLETE_PROGRESS = 95;
   ...
   * @return 分页结果
   */
  public PageResult<TaskDTO> listTasks(final TaskQueryDTO query) {
    ...
    IPage<Task> page =
        taskRepository.page(new Page<>(query.getPageNum(), query.getPageSize()), wrapper);
   ...
   */
  @Transactional
  public TaskDTO createTask(final CreateTaskCommand command) {
   ...
   * @param task 任务实体
   */
  private void sendTaskAssignNotification(final Task task) {
    try {
      ...
      notificationAppService.sendSystemNotification(
          task.getAssigneeId(), title, content, "TASK", task.getId());
   ...
   * @return 描述
   */
  private String getPriorityText(final String priority) {
    if (priority == null) {
      return "普通";
    }
  ...
  public void deleteTask(final Long id) {
    ...
    // 检查是否有子任务
    long subTaskCount =
        taskRepository.count(new LambdaQueryWrapper<Task>().eq(Task::getParentId, id));
    if (subTaskCount > 0) {
   ...
   */
  @Transactional
  public TaskDTO updateStatus(final Long id, final String status) {
    Task task = taskRepository.getByIdOrThrow(id, "任务不存在");
   ...
   */
  @Transactional
  public TaskDTO approveTask(final Long id) {
    Task task = taskRepository.getByIdOrThrow(id, "任务不存在");
   ...
   */
  @Transactional
  public TaskDTO rejectTask(final Long id, final String comment) {
    Task task = taskRepository.getByIdOrThrow(id, "任务不存在");
    ...
    if (task.getProgress() != null && task.getProgress() == MAX_PROGRESS) {
      task.setProgress(ALMOST_COMPLETE_PROGRESS);
    }
    // 其他情况保持原进度，由负责人自行调整

    taskRepository.updateById(task);
    log.info(
        "任务验收退回: taskId={}, reviewedBy={}, comment={}, progress={}%",
   ...
   *
   * @param task 任务实体
   * @param reviewResult 验收结果
   * @param comment 验收意见
   */
  private void sendTaskReviewNotification(
      final Task task, final String reviewResult, final String comment) {
    try {
      if (task.getAssigneeId() == null) {
        return;
      }
      ...
      notificationAppService.sendSystemNotification(
          task.getAssigneeId(), title, content, "TASK", task.getId());
      log.info(
          "任务验收通知已发送: taskId={}, assigneeId={}, result={}",
          task.getId(),
          task.getAssigneeId(),
          reviewResult);
    } catch (Exception e) {
   ...
   * @param newStatus 新状态
   */
  private void sendTaskStatusNotification(
      final Task task, final String oldStatus, final String newStatus) {
    try {
      Long currentUserId = SecurityUtils.getUserId();
      String currentUserName = SecurityUtils.getRealName();
      ...
      String content =
          String.format("%s 将任务【%s】状态修改为：%s", currentUserName, task.getTitle(), statusName);
  ...
  /**
   * 更新任务进度 修复：添加状态验证，防止已取消/已完成的任务被更新
   *
   * @param id 任务ID
   * @param progress 进度
   * @return 更新后的任务DTO
   */
  @Transactional
  public TaskDTO updateProgress(final Long id, final Integer progress) {
    Task task = taskRepository.getByIdOrThrow(id, "任务不存在");
    ...
    if (!TaskStatus.PENDING.equals(currentStatus)
        && !TaskStatus.IN_PROGRESS.equals(currentStatus)) {
    ...
    taskRepository.updateById(task);
    log.info(
        "任务进度更新: taskId={}, title={}, progress={}%, status={}",
   ...
   * @return 任务DTO列表
   */
  public List<TaskDTO> getUpcomingTasks(final int days) {
   ...
   * @return 统计数组 [总数, 完成数]
   */
  public int[] getMatterTaskStats(final Long matterId) {
  ...
  /**
   * 验证任务操作权限 只有以下用户可以操作任务： 1. 系统管理员 2. 任务创建者 3. 任务负责人（被分配人）
   *
   * @param task 任务实体
   */
  private void validateTaskOperationPermission(final Task task) {
    // 管理员拥有所有权限
  ...
  /**
   * 应用数据范围过滤 ALL: 可看全部任务 DEPT_AND_CHILD: 可看本部门及下级部门的项目任务，或自己创建/负责的任务 DEPT: 可看本部门的项目任务，或自己创建/负责的任务
   * SELF: 只能看自己创建或分配给自己的任务
   *
   * @param wrapper 查询包装器
   */
  private void applyDataScopeFilter(final LambdaQueryWrapper<Task> wrapper) {
