...
public class ArchiveLocationAppService {
  ...
  public List<ArchiveLocationDTO> listLocations() {
    List<ArchiveLocation> locations =
   ...
   * @return 库位详情
   */
  public ArchiveLocationDTO getLocationById(final Long id) {
   ...
   */
  @Transactional
  public ArchiveLocationDTO createLocation(final ArchiveLocationDTO dto) {
    // 检查编码唯一性
    if (locationRepository.count(
            new LambdaQueryWrapper<ArchiveLocation>()
                .eq(ArchiveLocation::getLocationCode, dto.getLocationCode()))
        > 0) {
   ...
   */
  @Transactional
  public ArchiveLocationDTO updateLocation(final Long id, final ArchiveLocationDTO dto) {
    ArchiveLocation location = locationRepository.getByIdOrThrow(id, "库位不存在");
   ...
   * @return DTO对象
   */
  private ArchiveLocationDTO toDTO(final ArchiveLocation location) {
    ...
    dto.setAvailableCapacity(
        (location.getTotalCapacity() == null ? 0 : location.getTotalCapacity())
            - (location.getUsedCapacity() == null ? 0 : location.getUsedCapacity()));
              ...
              if (location.getTotalCapacity() != null && location.getTotalCapacity() > 0) {
                double usageRate =
                    (location.getUsedCapacity() != null
                            ? location.getUsedCapacity().doubleValue()
                            : 0.0)
