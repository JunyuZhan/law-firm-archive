...
public class ClientAccessLogService {
  ...
  public void saveAccessLog(final AccessLogCallbackRequest request) {
    ...
    // 3. 保存访问日志到数据库
    ClientAccessLog accessLog =
        ClientAccessLog.builder()
    ...
    log.info(
        "保存客户访问日志成功: id={}, matterId={}, clientId={}, accessTime={}",
        accessLog.getId(),
        request.getMatterId(),
        request.getClientId(),
  ...
  public PageResult<ClientAccessLogDTO> getAccessLogs(
    ...
    queryWrapper.eq(ClientAccessLog::getMatterId, matterId).eq(ClientAccessLog::getDeleted, false);
    ...
    IPage<ClientAccessLog> page =
        clientAccessLogMapper.selectPage(new Page<>(pageNum, pageSize), queryWrapper);

    List<ClientAccessLogDTO> dtoList =
