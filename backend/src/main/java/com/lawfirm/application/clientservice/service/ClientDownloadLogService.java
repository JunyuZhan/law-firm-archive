...
public class ClientDownloadLogService {
  ...
  public void saveDownloadLog(final DownloadLogCallbackRequest request) {
    ...
    // 3. 保存下载日志到数据库
    ClientDownloadLog downloadLog =
        ClientDownloadLog.builder()
    ...
    log.info(
        "保存客户下载日志成功: id={}, matterId={}, clientId={}, fileId={}, fileName={}, downloadTime={}",
        downloadLog.getId(),
        request.getMatterId(),
        request.getClientId(),
        request.getFileId(),
  ...
  public PageResult<ClientDownloadLogDTO> getDownloadLogs(
    ...
    queryWrapper
        .eq(ClientDownloadLog::getMatterId, matterId)
    ...
    IPage<ClientDownloadLog> page =
        clientDownloadLogMapper.selectPage(new Page<>(pageNum, pageSize), queryWrapper);

    List<ClientDownloadLogDTO> dtoList =
