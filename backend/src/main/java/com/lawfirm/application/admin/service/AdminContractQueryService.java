...
/**
 * 行政模块合同查询服务（只读） 用于司法局报备、介绍信等
 *
 * <p>Requirements: 5.1, 5.2, 5.3, 5.4, 5.5, 8.1, 8.3
 */
@Slf4j
@Service
...
public class AdminContractQueryService {
  ...
  /** CauseOfAction Service. */
  private final CauseOfActionService causeOfActionService;

  /** 导出时的最大分页大小（不分页） */
  private static final int EXPORT_MAX_PAGE_SIZE = 10000;

  /**
   * 查询已审批合同列表（只读）
   *
   * <p>Requirements: 5.1, 5.2, 5.3, 5.4, 8.1
   *
   * @param query 查询条件
   * @return 分页结果
   */
  public PageResult<AdminContractViewDTO> listApprovedContracts(final AdminContractQueryDTO query) {
    ...
    if (query.getLeadLawyerId() != null) {
      ...
      List<Long> contractIds =
          participantRepository.findContractIdsByUserIdAndRole(query.getLeadLawyerId(), "LEAD");
    ...
    List<AdminContractViewDTO> records =
        page.getRecords().stream().map(this::toAdminViewDTO).collect(Collectors.toList());
   ...
   * 获取合同详情（只读）
   *
   * <p>Requirements: 5.5, 8.1
   *
   * @param id 合同ID
   * @return 合同详情视图DTO
   */
  public AdminContractViewDTO getContractById(final Long id) {
   ...
   * @return 合同列表
   */
  public List<AdminContractViewDTO> listContractsByMonth(final int year, final int month) {
    ...
    query.setPageNum(1);
    query.setPageSize(EXPORT_MAX_PAGE_SIZE); // 导出不分页
   ...
   * @return 行政视图DTO
   */
  private AdminContractViewDTO toAdminViewDTO(final Contract contract) {
   ...
   * 获取审理阶段名称（支持多选，逗号分隔）
   *
   * @param trialStage 审理阶段（支持逗号分隔的多个值）
   * @return 审理阶段名称
   */
  private String getTrialStageName(final String trialStage) {
    if (trialStage == null || trialStage.isEmpty()) {
      return null;
    }
    // 支持多选：逗号分隔的多个值
   ...
   * 获取案由名称
   *
   * <p>数据库存储的是案由代码（如 "14", "76.2"），需要转换为中文名称用于导出。 根据案件类型选择查询民事/刑事/行政案由。
   *
