...
public class AssetAppService {
   ...
   * @return 分页结果
   */
  public PageResult<AssetDTO> listAssets(
    ...
    IPage<Asset> result =
        assetRepository.findPage(page, keyword, category, status, departmentId, null);
    ...
    List<AssetDTO> items = assets.stream().map(a -> toDTO(a, userMap)).collect(Collectors.toList());
   ...
   * @return 资产DTO
   */
  public AssetDTO getAssetById(final Long id) {
   ...
   */
  @Transactional
  public AssetDTO createAsset(final CreateAssetCommand command) {
    // 生成资产编号
   ...
   */
  @Transactional
  public AssetDTO updateAsset(final Long id, final CreateAssetCommand command) {
    Asset asset = assetRepository.getById(id);
    if (asset == null) {
      throw new BusinessException("资产不存在");
    }
  ...
  /**
   * 资产领用 权限控制： - 普通用户只能为自己领用 - 管理员/资产管理员可以代他人领用.
   *
   * @param command 领用命令
   */
  @Transactional
  public void receiveAsset(final AssetReceiveCommand command) {
    ...
    if (targetUserId != null && !targetUserId.equals(currentUserId)) {
      ...
      log.warn(
          "管理员代领用资产: operator={}, targetUser={}, assetNo={}",
          currentUserId,
          targetUserId,
          asset.getAssetNo());
    } else {
  ...
  /**
   * 资产归还 问题291修复：验证归还人权限，只能归还自己的资产或管理员代归还.
   *
   * @param assetId 资产ID
   * @param remarks 备注
   */
  @Transactional
  public void returnAsset(final Long assetId, final String remarks) {
    ...
    if (asset.getCurrentUserId() != null && !asset.getCurrentUserId().equals(currentUserId)) {
      ...
      log.warn(
          "管理员代归还资产: operator={}, currentUser={}, assetNo={}",
          currentUserId,
          asset.getCurrentUserId(),
          asset.getAssetNo());
    }
    ...
    log.info(
        "资产归还: assetNo={}, operator={}, fromUser={}",
        asset.getAssetNo(),
        currentUserId,
        asset.getCurrentUserId());
  }
  ...
  /**
   * 资产报废申请 问题300修复：使用中间状态PENDING_SCRAP，审批通过后才变为SCRAPPED.
   *
   * @param assetId 资产ID
   * @param reason 报废原因
   */
  @Transactional
  public void scrapAsset(final Long assetId, final String reason) {
    Asset asset = assetRepository.getById(assetId);
    if (asset == null) {
      throw new BusinessException("资产不存在");
    }
   ...
   */
  @Transactional
  public void approveScrap(final Long assetId, final boolean approved, final String comment) {
    Asset asset = assetRepository.getById(assetId);
    if (asset == null) {
      throw new BusinessException("资产不存在");
    }
  ...
  /**
   * 获取资产操作记录 问题293修复：优化N+1查询，只查询一次资产信息.
   *
   * @param assetId 资产ID
   * @return 操作记录列表
   */
  public List<AssetRecordDTO> getAssetRecords(final Long assetId) {
    ...
    return records.stream().map(r -> toRecordDTO(r, asset)).collect(Collectors.toList());
  }
  ...
  public List<AssetDTO> getIdleAssets() {
    return assetRepository.findIdleAssets().stream().map(this::toDTO).collect(Collectors.toList());
  }
  ...
  /**
   * 生成资产编号 问题292修复：使用日期+序列号+随机数避免并发冲突
   *
   * @param category 资产类别
   * @return 资产编号
   */
  private String generateAssetNo(final String category) {
    String prefix =
        switch (category) {
          case "OFFICE" -> "OF";
          case "IT" -> "IT";
   ...
   * @return 资产DTO
   */
  private AssetDTO toDTO(final Asset asset) {
    return toDTO(asset, null);
  }

  /**
   * 转换为DTO（批量优化版本，从Map获取用户信息） 问题290修复：支持批量加载
   *
   * @param asset 资产实体
   * @param userMap 用户Map
   * @return 资产DTO
   */
  private AssetDTO toDTO(final Asset asset, final Map<Long, User> userMap) {
    AssetDTO dto =
        AssetDTO.builder()
            .id(asset.getId())
    ...
    if (asset.getCurrentUserId() != null) {
      User user =
          (userMap != null)
              ? userMap.get(asset.getCurrentUserId())
  ...
  /** 转换为记录DTO（优化版本，直接使用传入的Asset对象） 问题293修复：避免N+1查询 */
  /**
   * 转换为资产记录DTO
   *
   * @param record 资产记录实体
   ...
   * @return 资产记录DTO
   */
  private AssetRecordDTO toRecordDTO(final AssetRecord record, final Asset asset) {
    AssetRecordDTO dto =
  ...
  private String getRecordTypeName(final String recordType) {
