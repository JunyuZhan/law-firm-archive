...
import com.lawfirm.application.openapi.dto.LetterVerificationPushDTO;
...
public class LetterVerificationService {
    ...
    /**
     * 推送验证数据到客服系统并生成二维码
     * 数据隔离方案：验证数据完全存储在客服系统，律所系统不对外暴露验证接口
     * 
     * @param application 函件申请
     * @param size 二维码尺寸（像素），默认200
     ...
     */
    @Transactional
    public String generateQrCodeWithPush(LetterApplication application, Integer size) {
        // 1. 生成验证码
        String verificationCode = generateVerificationCode(application);
        ...
        if (integration != null && Boolean.TRUE.equals(integration.getEnabled())) {
            // 3. 推送验证数据到客服系统
            try {
                verificationUrl = pushVerificationToClientService(integration, application, verificationCode);
                log.info("函件验证数据已推送到客服系统: applicationNo={}, url={}", 
                    application.getApplicationNo(), verificationUrl);
            } catch (Exception e) {
                log.error("推送验证数据到客服系统失败，使用本地URL: applicationNo={}", 
                    application.getApplicationNo(), e);
                // 推送失败，使用本地URL作为兜底（但本地接口已删除，扫码将无法验证）
                verificationUrl = verificationCodeService.generateVerificationUrl(
                    getPublicVerifyUrl(),
                    VerificationCodeService.BUSINESS_TYPE_LETTER,
                    application.getApplicationNo(),
                    verificationCode
     ...
     * @return 客服系统返回的验证URL
     */
    private String pushVerificationToClientService(ExternalIntegration integration, 
        ...
        // 构建推送数据
        LetterVerificationPushDTO pushData = LetterVerificationPushDTO.builder()
                ...
                .printedAt(application.getPrintedAt() != null ? application.getPrintedAt() : LocalDateTime.now())
