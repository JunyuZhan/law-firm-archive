...
public class MeetingNoticeAppService {
  ...
  /**
   * 发送会议通知 问题414修复：添加权限验证
   *
   * @param bookingId 预约ID
   */
  @Transactional
  public void sendMeetingNotice(final Long bookingId) {
    ...
    log.info(
        "会议通知已发送: bookingNo={}, title={}, sentBy={}",
  ...
  /**
   * 批量发送即将开始的会议通知 问题408修复：使用批量更新替代循环更新
   *
   * @param minutesBefore 提前多少分钟
   * @return 发送数量
   */
  @Transactional
  public int sendUpcomingMeetingNotices(final int minutesBefore) {
    ...
    if (!toUpdate.isEmpty()) {
      ...
      toUpdate.forEach(
          b -> log.info("发送会议通知: bookingNo={}, title={}", b.getBookingNo(), b.getTitle()));
    }
