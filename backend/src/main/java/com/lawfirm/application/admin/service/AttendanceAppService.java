...
public class AttendanceAppService {
   ...
   * @return 分页结果
   */
  public PageResult<AttendanceDTO> listAttendance(final AttendanceQueryDTO query) {
    IPage<Attendance> page =
        attendanceMapper.selectAttendancePage(
  ...
  /**
   * 签到 ✅ 修复：使用数据库唯一约束+重试机制处理并发 需要在数据库添加唯一索引: UNIQUE(user_id, attendance_date).
   *
   * @param command 签到命令
   * @return 考勤DTO
   */
  @Transactional
  public AttendanceDTO checkIn(final CheckInCommand command) {
   ...
   */
  @Transactional
  public AttendanceDTO checkOut(final CheckInCommand command) {
    Long userId = SecurityUtils.getUserId();
    LocalDate today = LocalDate.now();
    LocalDateTime now = LocalDateTime.now();
    ...
    if (now.isAfter(workEndDateTime)) {
      ...
      BigDecimal overtimeHours =
          BigDecimal.valueOf(overtime.toMinutes())
              .divide(BigDecimal.valueOf(MINUTES_PER_HOUR), 2, RoundingMode.HALF_UP);
   ...
   * @return 月度统计DTO
   */
  public MonthlyAttendanceStatisticsDTO getMonthlyStatistics(
   ...
   * @return 考勤DTO
   */
  private AttendanceDTO toDTO(final Attendance attendance) {
