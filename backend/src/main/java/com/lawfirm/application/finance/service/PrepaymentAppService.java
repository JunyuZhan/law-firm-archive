...
public class PrepaymentAppService {
   ...
   * @return 预收款分页结果
   */
  public PageResult<PrepaymentDTO> listPrepayments(final PrepaymentQueryDTO query) {
    IPage<Prepayment> page =
        prepaymentMapper.selectPrepaymentPage(
   ...
   * 批量转换预收款DTO（优化N+1查询）
   *
   * @param prepayments 预收款列表
   * @return DTO列表
   */
  private List<PrepaymentDTO> batchConvertToDTO(final List<Prepayment> prepayments) {
    ...
    for (Prepayment p : prepayments) {
      ...
      if (p.getMatterId() != null) {
        matterIds.add(p.getMatterId());
      }
      if (p.getConfirmerId() != null) {
        confirmerIds.add(p.getConfirmerId());
      }
    }
   ...
   * @return 预收款DTO
   */
  public PrepaymentDTO getPrepaymentById(final Long id) {
   ...
   * 创建预收款
   *
   * @param command 创建预收款命令
   * @return 预收款DTO
   */
  @Transactional
  public PrepaymentDTO createPrepayment(final CreatePrepaymentCommand command) {
    // 验证客户存在
   ...
   */
  @Transactional
  public PrepaymentDTO confirmPrepayment(final Long id) {
    Prepayment prepayment = prepaymentRepository.getByIdOrThrow(id, "预收款记录不存在");
   ...
   * 使用预收款（核销到收费记录）
   *
   * @param command 使用预收款命令
   * @return 预收款使用记录DTO
   */
  @Transactional
  public PrepaymentUsageDTO usePrepayment(final UsePrepaymentCommand command) {
    Prepayment prepayment =
        prepaymentRepository.getByIdOrThrow(command.getPrepaymentId(), "预收款记录不存在");
    ...
    // 创建核销记录
    PrepaymentUsage usage =
        PrepaymentUsage.builder()
    ...
    log.info(
        "预收款核销成功: prepaymentNo={}, feeNo={}, amount={}",
        prepayment.getPrepaymentNo(),
        fee.getFeeNo(),
        command.getAmount());
   ...
   * @return 可用预收款列表
   */
  public List<PrepaymentDTO> getAvailablePrepayments(final Long clientId) {
   ...
   */
  @Transactional
  public PrepaymentDTO refundPrepayment(final Long id, final String remark) {
    Prepayment prepayment = prepaymentRepository.getByIdOrThrow(id, "预收款记录不存在");
    ...
    prepayment.setRemark(
        (prepayment.getRemark() != null ? prepayment.getRemark() + "\n" : "") + "退款原因: " + remark);
    prepaymentRepository.updateById(prepayment);
   ...
   *
   * @param prepayment 预收款实体
   * @return 预收款DTO
   */
  private PrepaymentDTO toDTO(final Prepayment prepayment) {
   ...
   *
   * @param prepayment 预收款实体
   * @param clientMap 客户映射
   * @param contractMap 合同映射
   * @param matterMap 项目映射
   * @param userMap 用户映射
   * @return 预收款DTO
   */
  private PrepaymentDTO toDTOWithMaps(
    ...
    if (prepayment.getClientId() != null) {
      Client client =
          (clientMap != null)
              ? clientMap.get(prepayment.getClientId())
    ...
    if (prepayment.getContractId() != null) {
      Contract contract =
          (contractMap != null)
              ? contractMap.get(prepayment.getContractId())
    ...
    if (prepayment.getMatterId() != null) {
      Matter matter =
          (matterMap != null)
              ? matterMap.get(prepayment.getMatterId())
    ...
    if (prepayment.getConfirmerId() != null) {
      User user =
          (userMap != null)
              ? userMap.get(prepayment.getConfirmerId())
   ...
   *
   * @param usage 预收款使用记录实体
   * @return 预收款使用记录DTO
   */
  private PrepaymentUsageDTO toUsageDTO(final PrepaymentUsage usage) {
