...
public class MyFinanceService {
  ...
  public List<Map<String, Object>> getMyContractPayments() {
    try {
      ...
      // 查询当前用户参与的合同
      List<ContractParticipant> myParticipations =
      ...
      for (ContractParticipant participation : myParticipations) {
        try {
          ...
          if (contract == null
              || (contract.getDeleted() != null && contract.getDeleted())
          ...
          if (contract.getClientId() != null) {
            try {
              ...
              item.put(
                  "clientName",
                  client != null && (client.getDeleted() == null || !client.getDeleted())
                      ? client.getName()
                      : "-");
            } catch (Exception e) {
          ...
          log.error(
              "处理合同参与记录失败, participationId: {}",
              participation != null ? participation.getId() : "null",
              e);
          // 继续处理下一条记录
        }
      }
   ...
   * @return 提成记录列表
   */
  public List<Map<String, Object>> getMyCommissions(final String status) {
    try {
      Long currentUserId = SecurityUtils.getUserId();
      if (currentUserId == null) {
        log.warn("无法获取当前用户ID");
      ...
      for (Commission commission : commissions) {
        try {
          ...
          if (commission.getContractId() != null) {
            try {
              ...
              if (contract != null && (contract.getDeleted() == null || !contract.getDeleted())) {
                item.put(
                    "contractNo",
                    contract.getContractNo() != null ? contract.getContractNo() : "-");
                item.put("contractName", contract.getName() != null ? contract.getName() : "-");
          ...
          item.put(
              "paymentAmount",
              commission.getGrossAmount() != null ? commission.getGrossAmount() : BigDecimal.ZERO);
          item.put(
              "commissionRate",
              commission.getCommissionRate() != null
          ...
          log.error(
              "处理提成记录失败, commissionId: {}", commission != null ? commission.getId() : "null", e);
          // 继续处理下一条记录
        }
      }
   ...
   * @return 状态名称
   */
  private String getCommissionStatusName(final String status) {
    if (status == null) {
      return "-";
    }
   ...
   * 获取费用报销状态名称
   *
   * @param status 费用报销状态代码
   * @return 状态名称
   */
  private String getExpenseStatusName(final String status) {
    if (status == null) {
      return "-";
    }
    return switch (status) {
