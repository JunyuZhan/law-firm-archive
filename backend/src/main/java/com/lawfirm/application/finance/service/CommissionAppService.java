...
public class CommissionAppService {
   ...
   *
   * @param command 创建提成规则命令
   * @return 提成规则DTO
   */
  @Transactional(rollbackFor = Exception.class)
  public CommissionRuleDTO createCommissionRule(final CreateCommissionRuleCommand command) {
    ...
    CommissionRule rule =
        CommissionRule.builder()
            ...
            .leadLawyerRate(
                command.getLeadLawyerRate() != null ? command.getLeadLawyerRate() : BigDecimal.ZERO)
            .assistLawyerRate(
                command.getAssistLawyerRate() != null
                    ? command.getAssistLawyerRate()
                    : BigDecimal.ZERO)
            .supportStaffRate(
                command.getSupportStaffRate() != null
                    ? command.getSupportStaffRate()
                    : BigDecimal.ZERO)
            .originatorRate(
                command.getOriginatorRate() != null ? command.getOriginatorRate() : BigDecimal.ZERO)
   ...
   *
   * @param id 规则ID
   * @param command 更新提成规则命令
   * @return 提成规则DTO
   */
  @Transactional(rollbackFor = Exception.class)
  public CommissionRuleDTO updateCommissionRule(
      final Long id, final UpdateCommissionRuleCommand command) {
    checkRulePermission();
   ...
   */
  @Transactional(rollbackFor = Exception.class)
  public void deleteCommissionRule(final Long id) {
    checkRulePermission();
   ...
   */
  @Transactional(rollbackFor = Exception.class)
  public void setDefaultRule(final Long id) {
    checkRulePermission();
   ...
   */
  @Transactional(rollbackFor = Exception.class)
  public void toggleRule(final Long id) {
    checkRulePermission();
   ...
   * @return 提成规则DTO
   */
  public CommissionRuleDTO getCommissionRule(final Long id) {
    CommissionRule rule = commissionRuleRepository.findById(id);
    if (rule == null) {
      throw new BusinessException("提成规则不存在");
    }
  ...
  /**
   * 手动计算提成（财务用户手动计算） 根据合同参与人的提成比例，财务可以手动修改提成金额
   *
   * @param command 手动计算提成命令
   * @return 提成记录列表
   */
  @Transactional(rollbackFor = Exception.class)
  public List<CommissionDTO> manualCalculateCommission(
      final ManualCalculateCommissionCommand command) {
    // 获取收款记录
    ...
    for (ManualCalculateCommissionCommand.ParticipantCommission pc : command.getParticipants()) {
      ...
      ContractParticipant participant =
          contractParticipantRepository.getByIdOrThrow(pc.getParticipantId(), "参与人不存在");
      ...
      String userName =
          participant.getUserId() != null
              ? userRepository.findById(participant.getUserId()) != null
                  ? userRepository.findById(participant.getUserId()).getRealName()
                  : "未知"
              : "未知";

      CommissionDetail detail =
          CommissionDetail.builder()
      ...
      log.info(
          "手动计算提成成功: paymentId={}, participantId={}, userId={}, commissionAmount={}",
          payment.getId(),
          pc.getParticipantId(),
  ...
  /**
   * 查询提成记录列表 数据权限：ADMIN/DIRECTOR/TEAM_LEADER/FINANCE 可看全部，其他人只能看自己的
   *
   * @param query 查询条件
   * @return 提成记录分页结果
   */
  public PageResult<CommissionDTO> listCommissions(final CommissionQueryDTO query) {
    ...
    if (query.getUserId() != null) {
      ...
      List<Commission> commissions =
          commissionRepository.findByUserId(query.getUserId(), offset, query.getPageSize());
              ...
              .filter(
                  c -> {
                    if (query.getPaymentId() != null
                        && !c.getPaymentId().equals(query.getPaymentId())) {
                      return false;
                    }
                    if (query.getMatterId() != null
                        && !c.getMatterId().equals(query.getMatterId())) {
                      return false;
                    }
      ...
      List<CommissionDTO> dtos =
          filtered.stream().map(this::toCommissionDTO).collect(Collectors.toList());
    ...
    List<CommissionDTO> dtos =
        result.getRecords().stream().map(this::toCommissionDTO).collect(Collectors.toList());
   ...
   * @return 提成记录DTO
   */
  public CommissionDTO getCommission(final Long id) {
    ...
    if (!roleCodes.contains("ADMIN")
        ...
        && !roleCodes.contains("TEAM_LEADER")) {
      // 检查 commission_detail 表中是否有该用户的记录
      int count =
          commissionRepository.getBaseMapper().countByCommissionIdAndUserId(id, currentUserId);
      if (count == 0) {
   ...
   * @return 提成总额
   */
  public BigDecimal getUserTotalCommission(final Long userId) {
   ...
   * @return 提成汇总数据
   */
  public Map<String, Object> getCommissionSummary(final String startDate, final String endDate) {
    // 权限检查：只有管理层可以查看汇总
    ...
    BigDecimal approvedCommission =
        commissionRepository.sumCommissionByStatus(CommissionStatus.APPROVED, startDate, endDate);
    BigDecimal paidCommission =
        commissionRepository.sumCommissionByStatus(CommissionStatus.PAID, startDate, endDate);
    ...
    List<Map<String, Object>> userSummary =
        commissionRepository.sumCommissionByUser(startDate, endDate);
   ...
   */
  @Transactional(rollbackFor = Exception.class)
  public CommissionDTO approveCommission(
    ...
    if (Boolean.TRUE.equals(approved)) {
      ...
      if (comment != null) {
        commission.setRemark(
            (commission.getRemark() != null ? commission.getRemark() + "\n" : "")
                + "审批通过: "
                + comment);
      }
    } else {
   ...
   */
  @Transactional(rollbackFor = Exception.class)
  public void batchApproveCommissions(
      final List<Long> ids, final Boolean approved, final String comment) {
    checkDirectorPermission();
   ...
   */
  @Transactional(rollbackFor = Exception.class)
  public CommissionDTO issueCommission(final Long id) {
    // 权限检查：只有财务人员可以确认发放
   ...
   */
  @Transactional(rollbackFor = Exception.class)
  public void batchIssueCommissions(final List<Long> ids) {
    checkFinancePermission();
   ...
   * @return 提成报表数据列表
   */
  public List<Map<String, Object>> getCommissionReportData(
  ...
  /** 检查是否有管理层权限（ADMIN/DIRECTOR/TEAM_LEADER/FINANCE） 财务人员需要查看提成汇总以制作财务报表. */
  ...
  private boolean canViewAllCommissions() {
    ...
    Long userId = SecurityUtils.getUserId();
    List<String> roleCodes = userRepository.findRoleCodesByUserId(userId);
    return roleCodes.contains("DIRECTOR")
        || roleCodes.contains("TEAM_LEADER")
        || roleCodes.contains("FINANCE");
  }
  ...
  private CommissionRuleDTO toDTO(final CommissionRule rule) {
  ...
  /**
   * 获取待计算提成的收款记录列表 返回所有已确认但还没有生成提成记录的收款记录
   *
   * @return 待计算提成的收款记录列表
   */
  public List<PaymentDTO> getPendingCommissionPayments() {
    ...
    paymentWrapper
        .eq(Payment::getStatus, PaymentStatus.CONFIRMED)
    ...
    commissionWrapper.eq(Commission::getDeleted, false).select(Commission::getPaymentId);
    List<Long> paymentIdsWithCommission =
        commissionRepository.list(commissionWrapper).stream()
    ...
    if (!feeIds.isEmpty()) {
      ...
      // 从费用记录中提取额外的项目ID
      Set<Long> additionalMatterIds =
          feeMap.values().stream()
  ...
  private CommissionDTO toCommissionDTO(final Commission commission) {
