...
public class DocumentTemplateRepository
    extends AbstractRepository<DocumentTemplateMapper, DocumentTemplate> {
   ...
   * @return 模板列表
   */
  public List<DocumentTemplate> findByTemplateType(final String templateType) {
   ...
   * @return 模板，未找到返回null
   */
  public DocumentTemplate findFirstByTemplateType(final String templateType) {
   ...
   * 根据模板类型和案件类型查找模板。
   *
   * <p>优先匹配特定案件类型，如果没有则回退到通用模板(caseType=ALL)
   *
   * @param templateType 模板类型
   * @param caseType 案件类型（如 CIVIL, CRIMINAL, ADMINISTRATIVE 等）
   * @return 匹配的模板，未找到返回null
   */
  public DocumentTemplate findByTemplateTypeAndCaseType(
      final String templateType, final String caseType) {
    // 1. 优先查找特定案件类型的模板
   ...
   * @return 模板
   */
  public DocumentTemplate findByTemplateNo(final String templateNo) {
    LambdaQueryWrapper<DocumentTemplate> query = new LambdaQueryWrapper<>();
    query.eq(DocumentTemplate::getTemplateNo, templateNo).eq(DocumentTemplate::getStatus, "ACTIVE");
