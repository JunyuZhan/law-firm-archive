...
```bash
cd /opt/law-firm
docker compose --env-file .env -f docker/docker-compose.prod.yml up -d --build frontend
```
...
# 查看特定服务日志
docker compose --env-file .env -f docker/docker-compose.prod.yml logs -f backend
docker compose --env-file .env -f docker/docker-compose.prod.yml logs -f frontend
```
...
# 查看详细日志
docker compose --env-file .env -f docker/docker-compose.prod.yml logs service-name
...
1. 查看日志：`docker compose --env-file .env -f docker/docker-compose.prod.yml logs`（从项目根目录执行）
