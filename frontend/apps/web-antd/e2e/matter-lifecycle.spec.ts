...
test.describe('Matter Lifecycle Flow', () => {
    ...
    test('should display matter list and search bar', async ({ page }) => {
        // 检查页面标题或主要内容区
        await expect(page.locator('.ant-page-header-heading-title')).toContainText(/案件/i).catch(() => {
    ...
    test('should have "Create Matter" button visible for admin', async ({ page }) => {
        // 检查创建按钮，通常带有特定的图标或文本
        const createBtn = page.getByRole('button', { name: /新增/i });
    ...
    test('should navigate to matter details', async ({ page }) => {
        // 点击表格中的第一行详情链接/按钮
        // 假设有名为 "查看" 的操作按钮
        const detailLink = page.getByText(/查看/i).first();
