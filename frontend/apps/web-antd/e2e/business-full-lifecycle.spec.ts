...
test.describe('Business Full Lifecycle Audit', () => {
  test('should complete the loop from template to project and billing', async ({
    page,
  }) => {
    // 1. 登录 (假设已全局处理)
    ...
    const createMatterBtn = page.getByRole('button', { name: /新增/ });
    ...
    const dossierTab = page.getByRole('tab', { name: /项目卷宗/ });
    ...
    // 5. 跨模块财务检查
    const financeTab = page.getByRole('tab', { name: /费用信息/ });
