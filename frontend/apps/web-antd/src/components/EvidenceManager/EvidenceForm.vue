...
import { createEvidence, updateEvidence } from '#/api/evidence';
...
interface FormData {
  ...
  provePurpose: string;
  description: string;
  isOriginal: 'copy' | 'original';
  originalCount: number;
  copyCount: number;
  pageStart?: number;
  pageEnd?: number;
  fileInfo: null | UploadResult;
  // 卷宗引用相关
  fileSource: 'dossier' | 'upload';
  documentId?: number;
  documentInfo?: null | {
    fileName?: string;
    fileSize?: number;
  ...
  try {
    ...
    if (result.success && result.rawText) {
      ...
      if (!formData.value.description) {
        formData.value.description = result.rawText.slice(0, 300);
      }
      // 如果证明目的为空，尝试提取
      if (!formData.value.provePurpose && result.rawText.length > 50) {
        formData.value.provePurpose = result.rawText.slice(0, 100);
      }
  ...
  try {
    ...
    if (isEdit.value && props.evidence?.id) {
      await updateEvidence(
        props.evidence.id,
        baseData as UpdateEvidenceCommand,
      );
      ...
      // 根据文件来源设置不同的字段
      if (
        formData.value.fileSource === 'dossier' &&
        formData.value.documentId
      ) {
        // 从卷宗引用
        createData.documentId = formData.value.documentId;
      } else if (
        formData.value.fileSource === 'upload' &&
        formData.value.fileInfo
      ) {
        // 直接上传
        ...
        createData.thumbnailUrl =
          formData.value.fileInfo.thumbnailUrl || undefined;
      }
  ...
  >
    ...
    >
      ...
      <FormItem label="证明目的" name="provePurpose">
        <Textarea
          v-model:value="formData.provePurpose"
          :rows="2"
          placeholder="请输入证明目的"
        />
      </FormItem>
      ...
      <FormItem label="份数">
        <div style="display: flex; gap: 16px">
          <div>
            ...
            <InputNumber
              v-model:value="formData.originalCount"
              :min="0"
              :max="99"
              style="width: 80px"
            />
          ...
          <div>
            ...
            <InputNumber
              v-model:value="formData.copyCount"
              :min="0"
              :max="99"
              style="width: 80px"
            />
      ...
      <FormItem label="页码范围">
        <div style="display: flex; gap: 8px; align-items: center">
          <InputNumber
            v-model:value="formData.pageStart"
            :min="1"
            placeholder="起始"
            style="width: 100px"
          />
          <span>-</span>
          <InputNumber
            v-model:value="formData.pageEnd"
      ...
      <!-- 从卷宗选择 -->
      <FormItem
        v-if="!isEdit && formData.fileSource === 'dossier'"
        label="选择文件"
      >
        <div
          v-if="formData.documentInfo"
          style="display: flex; gap: 8px; align-items: center"
        >
          <Tag color="blue">
            {{ formData.documentInfo.title || formData.documentInfo.fileName }}
          </Tag>
          <span
            v-if="formData.documentInfo.fileSize"
            style="font-size: 12px; color: #999"
          >
            {{ formatFileSize(formData.documentInfo.fileSize) }}
          </span>
          <Button type="link" size="small" @click="clearDossierFile">
            移除
          </Button>
        </div>
      ...
      <!-- 直接上传 -->
      <FormItem
        v-if="!isEdit && formData.fileSource === 'upload'"
        label="上传文件"
      >
        <EvidenceUploader
          v-model="formData.fileInfo"
      ...
      <FormItem label="描述" name="description">
        <Textarea
          ...
          :rows="3"
          placeholder="请输入描述信息（可通过OCR自动提取）"
        />
      </FormItem>
    </Form>
