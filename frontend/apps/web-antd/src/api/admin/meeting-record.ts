...
/** 根据预约创建会议记录 */
export function createMeetingRecordFromBooking(
  bookingId: number,
  data: CreateMeetingRecordCommand,
) {
  return requestClient.post<MeetingRecordDTO>(
    `/admin/meeting-records/from-booking/${bookingId}`,
    data,
  );
}
...
export function getMeetingRecordsByRoom(roomId: number) {
  return requestClient.get<MeetingRecordDTO[]>(
    `/admin/meeting-records/room/${roomId}`,
  );
}
