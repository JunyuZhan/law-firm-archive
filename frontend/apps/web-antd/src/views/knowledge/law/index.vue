...
import { Card, Button, Space, Tag, Row, Col, Tree } from 'ant-design-vue';
...
import type { LawRegulationDTO, LawCategoryDTO } from '#/api/knowledge/types';
...
const gridColumns: VxeGridProps['gridOptions']['columns'] = [
  ...
  { title: '效力状态', field: 'status', width: 100, slots: { default: 'status' } },
...
const [Grid, gridApi] = useVbenVxeGrid({
  ...
  gridOptions: {
    columns: gridColumns,
    height: 'auto',
    proxyConfig: { ajax: { query: async ({ page, form }) => await loadData({ page: page.currentPage, pageSize: page.pageSize, ...form }) } },
...
async function loadCategories() {
  try { categories.value = await getLawCategoryTree(); } catch (e) { console.error(e); }
}

function onCategorySelect(selectedKeys: any[]) {
  selectedCategoryId.value = selectedKeys.length > 0 ? selectedKeys[0] : undefined;
  gridApi.reload();
}

function getStatusColor(status: string) {
  return { EFFECTIVE: 'green', AMENDED: 'orange', REPEALED: 'red' }[status] || 'default';
}

function handleAdd() { lawModalRef.value?.openCreate(); }
function handleEdit(row: LawRegulationDTO) { lawModalRef.value?.openEdit(row); }

function handleDelete(row: LawRegulationDTO) {
  Modal.confirm({
    title: '确认删除', content: `确定要删除法规 "${row.name}" 吗？`,
    onOk: async () => { await deleteLawRegulation(row.id); message.success('删除成功'); gridApi.reload(); },
  });
}

async function handleCollect(row: LawRegulationDTO) {
  if (row.collected) { await uncollectLawRegulation(row.id); message.success('已取消收藏'); }
  else { await collectLawRegulation(row.id); message.success('收藏成功'); }
  gridApi.reload();
}

function handleMarkRepealed(row: LawRegulationDTO) {
  Modal.confirm({
    title: '标注失效', content: '确定要将此法规标注为已废止吗？',
    onOk: async () => { await markLawRegulationRepealed(row.id); message.success('标注成功'); gridApi.reload(); },
  });
}

function handleModalSuccess() { gridApi.reload(); }
    ...
    <Row :gutter="16">
      <Col :span="5">
        <Card title="法规分类" style="height: calc(100vh - 200px); overflow-y: auto">
          <Tree :tree-data="categories" :field-names="{ children: 'children', title: 'name', key: 'id' }" :selected-keys="selectedCategoryId ? [selectedCategoryId] : []" @select="onCategorySelect" />
        </Card>
      </Col>
      <Col :span="19">
        <Grid>
          <template #toolbar-buttons><Button type="primary" @click="handleAdd">新增法规</Button></template>
          <template #status="{ row }"><Tag :color="getStatusColor(row.status)">{{ row.statusName }}</Tag></template>
          <template #action="{ row }">
            <Space>
