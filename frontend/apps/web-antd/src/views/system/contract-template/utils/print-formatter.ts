    ...
    if (trimmed.startsWith('"') && trimmed.endsWith('"')) {
      try {
        ...
        if (typeof unescaped === 'string') {
          ...
          if (innerParsed && typeof innerParsed === 'object' && innerParsed._structured === true) {
            return true;
          }
        }
      } catch {
...
export function formatPlainTextForPrint(content: string, variables: Record<string, string> = {}): string {
  ...
  if ((trimmed.startsWith('{') && trimmed.endsWith('}')) || 
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && parsed._structured === true && parsed.blocks) {
        console.warn('formatPlainTextForPrint: 检测到结构化内容但未被 isStructuredContent 识别，尝试重新格式化');
        // 使用原始内容或解析后的内容
      ...
      // 不是有效的JSON，继续处理
      console.debug('formatPlainTextForPrint: JSON 解析失败，作为纯文本处理', e);
    }
  }
  ...
  Object.entries(variables).forEach(([key, value]) => {
    // 转义特殊字符，避免在正则表达式中出错
    const escapedKey = key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    text = text.replace(new RegExp(`\\$\\{${escapedKey}\\}`, 'g'), value || `[${key}]`);
  });
  ...
  lines.forEach((line, index) => {
    ...
    // 检测合同编号行 - 右对齐
    if (trimmed.includes('合同编号') || (trimmed.includes('编号') && trimmed.includes('字第'))) {
