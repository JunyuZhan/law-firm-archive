...
import { message, Card, Table, Button, Space, Tag, DatePicker, Statistic, Row, Col, Modal, Descriptions, DescriptionsItem } from 'ant-design-vue';
...
const columns = [
  ...
  { title: '上班打卡', dataIndex: 'checkInTime', key: 'checkInTime', width: 100 },
  { title: '下班打卡', dataIndex: 'checkOutTime', key: 'checkOutTime', width: 100 },
...
async function loadData() {
  ...
  try {
    ...
    for (const record of dataSource.value) {
      ...
      else if (record.status === 'EARLY_LEAVE') stats.early++;
...
function handleView(record: AttendanceRecord) {
...
function formatWorkHours(record: AttendanceRecord) {
...
<template>
  <Page title="考勤管理" description="查看员工考勤记录">
    ...
    <Card style="margin-bottom: 16px">
      <Row :gutter="16" align="middle">
        ...
        <Col :span="12" style="text-align: right">
          <Space>
            <Button type="primary" :disabled="!!todayRecord?.checkInTime" @click="handleCheckIn">
              上班打卡
            </Button>
            <Button :disabled="!todayRecord?.checkInTime || !!todayRecord?.checkOutTime" @click="handleCheckOut">
              下班打卡
            </Button>
          </Space>
        </Col>
    ...
    <Row :gutter="16" style="margin-bottom: 16px">
      <Col :span="6">
        <Card><Statistic title="今日出勤" :value="statistics.normal" suffix="人" /></Card>
      </Col>
      <Col :span="6">
        <Card><Statistic title="迟到" :value="statistics.late" suffix="人" :valueStyle="{ color: '#faad14' }" /></Card>
      </Col>
      <Col :span="6">
        <Card><Statistic title="早退" :value="statistics.early" suffix="人" :valueStyle="{ color: '#faad14' }" /></Card>
      </Col>
      <Col :span="6">
        <Card><Statistic title="缺勤" :value="statistics.absent" suffix="人" :valueStyle="{ color: '#ff4d4f' }" /></Card>
      </Col>
    </Row>
    ...
    <Modal v-model:open="detailVisible" title="考勤详情" :footer="null">
      <Descriptions v-if="currentRecord" :column="2" bordered size="small">
        ...
        <DescriptionsItem label="日期">{{ formatDate(currentRecord.attendanceDate) }}</DescriptionsItem>
        <DescriptionsItem label="状态">
          <Tag :color="statusMap[currentRecord.status]?.color">
            {{ statusMap[currentRecord.status]?.text || currentRecord.status }}
          </Tag>
        </DescriptionsItem>
        <DescriptionsItem label="上班打卡">{{ formatTime(currentRecord.checkInTime) }}</DescriptionsItem>
        <DescriptionsItem label="下班打卡">{{ formatTime(currentRecord.checkOutTime) }}</DescriptionsItem>
        <DescriptionsItem label="工作时长">{{ formatWorkHours(currentRecord) }}</DescriptionsItem>
        <DescriptionsItem label="打卡地点">{{ currentRecord.location || '-' }}</DescriptionsItem>
        <DescriptionsItem v-if="currentRecord.remark" label="备注" :span="2">{{ currentRecord.remark }}</DescriptionsItem>
      </Descriptions>
    </Modal>
  </Page>
</template>
