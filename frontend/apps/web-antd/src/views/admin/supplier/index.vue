...
import { message, Tag, Space, Card, Button, Input, Select, Row, Col, Statistic, Modal } from 'ant-design-vue';
import { Plus } from '@vben/icons';
import { getSupplierList, deleteSupplier, changeSupplierStatus, getSupplierStatistics } from '#/api/admin/supplier';
...
const statusColorMap: Record<string, string> = { ACTIVE: 'green', INACTIVE: 'red' };
const ratingColorMap: Record<string, string> = { A: 'green', B: 'blue', C: 'orange', D: 'red' };
...
async function loadStatistics() {
  try {
    ...
    statistics.value = { total: stats.total || 0, active: stats.active || 0, inactive: stats.inactive || 0 };
  } catch (error) {
...
function handleReset() {
  queryParams.value = { keyword: undefined, supplierType: undefined, status: undefined, rating: undefined };
  gridApi.reload();
}
...
<template>
  <Page title="供应商管理" description="管理供应商信息">
    <Row :gutter="16" style="margin-bottom: 16px">
      <Col :span="8"><Card><Statistic title="供应商总数" :value="statistics.total" suffix="家" /></Card></Col>
      <Col :span="8"><Card><Statistic title="启用" :value="statistics.active" suffix="家" :value-style="{ color: '#3f8600' }" /></Card></Col>
      <Col :span="8"><Card><Statistic title="停用" :value="statistics.inactive" suffix="家" :value-style="{ color: '#cf1322' }" /></Card></Col>
    </Row>

    <Card>
      <div style="margin-bottom: 16px">
        <Space wrap>
          <Input v-model:value="queryParams.keyword" placeholder="搜索供应商名称/联系人/电话" style="width: 200px" allowClear @pressEnter="handleSearch" />
          <Select v-model:value="queryParams.supplierType" placeholder="供应商类型" style="width: 120px" allowClear :options="supplierTypeOptions" />
          <Select v-model:value="queryParams.status" placeholder="状态" style="width: 100px" allowClear :options="statusOptions" />
          <Select v-model:value="queryParams.rating" placeholder="评级" style="width: 100px" allowClear :options="ratingOptions" />
          <Button type="primary" @click="handleSearch">搜索</Button>
          <Button @click="handleReset">重置</Button>
          <Button type="primary" @click="handleAdd"><Plus class="size-4" />新增供应商</Button>
        </Space>
      </div>

      <Grid>
        <template #rating="{ row }">
          <Tag v-if="row.rating" :color="ratingColorMap[row.rating]">{{ row.ratingName || row.rating }}</Tag>
          <span v-else>-</span>
        </template>
        <template #status="{ row }">
          <Tag :color="statusColorMap[row.status || '']">{{ row.status === 'ACTIVE' ? '启用' : '停用' }}</Tag>
        </template>
        <template #action="{ row }">
          <Space>
