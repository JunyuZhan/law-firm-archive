...
import type {
  DevelopmentMilestoneDTO,
  DevelopmentPlanDTO,
  DevelopmentPlanQuery,
} from '#/api/hr/development-plan';
import type { CareerLevelDTO } from '#/api/hr/promotion';
...
async function loadOptions() {
  try {
    const levelRes = await getCareerLevelList({
      pageNum: 1,
      pageSize: 100,
      status: 'ACTIVE',
    });
...
// 添加目标
function addGoal(type: 'action' | 'career' | 'performance' | 'skill') {
...
// 删除目标
function removeGoal(
  type: 'action' | 'career' | 'performance' | 'skill',
  index: number,
) {
  const key =
    type === 'career'
    ...
    >
      <Form :label-col="{ span: 5 }" :wrapper-col="{ span: 18 }">
        ...
        <FormItem label="计划标题" required>
          <Input
            v-model:value="formData.planTitle"
            placeholder="如：2026年度职业发展计划"
          />
        </FormItem>
        <FormItem label="目标职级">
          <Select
            ...
            allow-clear
            :options="
              careerLevels.map((l) => ({ label: l.levelName, value: l.id }))
            "
          />
        </FormItem>
        <FormItem label="目标日期">
        ...
        <FormItem label="职业目标">
          <div>
            <Space
              v-for="(goal, i) in formData.careerGoals"
              :key="i"
          ...
          <Space>
            <Input
              v-model:value="tempGoal.career"
              placeholder="输入职业目标"
              style="width: 300px"
              @press-enter="addGoal('career')"
            />
        ...
        <FormItem label="技能目标">
          <div>
            <Space
              v-for="(goal, i) in formData.skillGoals"
              :key="i"
              style="margin-bottom: 4px"
            >
          ...
          <Space>
            <Input
              v-model:value="tempGoal.skill"
              placeholder="输入技能目标"
              style="width: 300px"
              @press-enter="addGoal('skill')"
            />
        ...
        <FormItem label="所需培训">
          <Textarea
            v-model:value="formData.requiredTraining"
            :rows="2"
            placeholder="需要参加的培训课程"
          />
        </FormItem>
        <FormItem label="里程碑">
          <div
            v-for="(m, i) in formData.milestones"
            :key="i"
            style="
              padding: 8px;
              margin-bottom: 8px;
          ...
          >
            <Space style="width: 100%">
              <Input
                v-model:value="m.milestoneName"
                placeholder="里程碑名称"
                style="width: 200px"
              />
              <DatePicker v-model:value="m.targetDate" placeholder="目标日期" />
              <Button size="small" danger @click="removeMilestone(i)">
                删除
              </Button>
            </Space>
          </div>
    ...
    >
      <Descriptions v-if="currentRecord" :column="2" bordered size="small">
        <DescriptionsItem label="计划编号">
          {{ currentRecord.planNo }}
        </DescriptionsItem>
        <DescriptionsItem label="员工姓名">
          {{ currentRecord.employeeName }}
        </DescriptionsItem>
        <DescriptionsItem label="计划年度">
          {{ currentRecord.planYear }}
        </DescriptionsItem>
        <DescriptionsItem label="计划标题">
        ...
        <DescriptionsItem label="目标日期">
          {{ formatDate(currentRecord.targetDate) }}
        </DescriptionsItem>
        <DescriptionsItem label="导师">
          {{ currentRecord.mentorName || '-' }}
        </DescriptionsItem>
        ...
        <DescriptionsItem
          v-if="currentRecord.careerGoals?.length"
          label="职业目标"
          :span="2"
        >
          <Tag
            v-for="(g, i) in currentRecord.careerGoals"
            :key="i"
            style="margin: 2px"
          >
            {{ g }}
        ...
        <DescriptionsItem
          v-if="currentRecord.skillGoals?.length"
          label="技能目标"
          :span="2"
        >
          <Tag
            v-for="(g, i) in currentRecord.skillGoals"
            :key="i"
            style="margin: 2px"
          >
            {{ g }}
        ...
        <DescriptionsItem
          v-if="currentRecord.requiredTraining"
          label="所需培训"
          :span="2"
        >
          ...
          >
            ...
            >
              <div>
                ...
                <p v-if="m.description" style="color: #666">
                  {{ m.description }}
                </p>
                <p style="font-size: 12px; color: #999">
                  目标日期: {{ formatDate(m.targetDate) }}
                </p>
              </div>
              <div>
                <Tag v-if="m.status === 'COMPLETED'" color="success">
                  已完成
                </Tag>
                <Button
