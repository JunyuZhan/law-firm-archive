        ...
        <Row :gutter="16">
          <Col :span="12">
            <FormItem label="规则编码" name="ruleCode">
              <Input v-model:value="formData.ruleCode" placeholder="请输入规则编码" />
            </FormItem>
          </Col>
          <Col :span="12">
...
const gridColumns: VxeGridProps['columns'] = [
  ...
  },
  {
    ...
    minWidth: 100,
    slots: {
      default: ({ row }: { row: CommissionRule }) => h('span', formatPercent(row.firmRate)),
    },
  },
  {
    title: '主办律师比例',
    ...
    minWidth: 120,
    slots: {
      default: ({ row }: { row: CommissionRule }) => h('span', formatPercent(row.leadLawyerRate)),
    },
  },
  {
    title: '案源人比例',
    ...
    minWidth: 100,
    slots: {
      default: ({ row }: { row: CommissionRule }) => h('span', formatPercent(row.originatorRate)),
    },
  },
  {
    title: '状态',
    ...
    minWidth: 80,
    slots: {
      default: ({ row }: { row: CommissionRule }) => h(
        Tag,
...
const rules = {
  ...
  firmRate: [
    { required: true, message: '请输入律所比例' },
  ],
  leadLawyerRate: [
    { required: true, message: '请输入主办律师比例' },
  ],
};
...
const handleSubmit = async () => {
  try {
    ...
    if (isEdit.value && id) {
      ...
      await commissionRuleApi.create(submitData as Omit<CommissionRule, 'id' | 'createdAt' | 'updatedAt'>);
