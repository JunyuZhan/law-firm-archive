...
import { message, Modal, Card, Descriptions, DescriptionsItem, Tag, Button, Space, Tabs, TabPane, Table, Empty, Spin, Form, FormItem, Select, Input, InputNumber, DatePicker, Textarea, Popconfirm, TimeRangePicker, Checkbox } from 'ant-design-vue';
...
const scheduleForm = ref<Partial<CreateScheduleCommand> & { date?: any; time?: [any, any] | null }>({
  title: '',
  description: '',
  location: '',
  scheduleType: 'COURT',
...
const letterForm = ref<Partial<CreateLetterApplicationCommand> & { expectedDate?: any; approverId?: number }>({
...
const canEditOperations = computed(() => {
  ...
  return !['PENDING_CLOSE', 'CLOSED', 'ARCHIVED'].includes(matter.value.status);
});
...
async function loadMatterDetail() {
  ...
  loading.value = true;
  try {
    const [matterRes, timelineRes, letterRes, deadlineRes, scheduleRes] = await Promise.all([
      getMatterDetail(id),
    ...
    if (matter.value.participants && matter.value.participants.length > 0) {
      matter.value.participants.forEach((participant) => {
        ...
        if (participant.userId && 
            ['LEAD', 'CO_COUNSEL'].includes(participant.role) &&
  ...
  try {
    const data: CreateLetterApplicationCommand = {
      ...
      expectedDate: letterForm.value.expectedDate ? 
        (typeof letterForm.value.expectedDate === 'string' ? letterForm.value.expectedDate : letterForm.value.expectedDate.format('YYYY-MM-DD')) 
        : undefined,
...
// 结案审批人列表
const closeApprovers = ref<Array<{
  id: number;
  realName: string;
  ...
  try {
    await applyCloseMatter(matter.value.id, {
      closingDate: closeForm.value.closingDate.format ? 
        closeForm.value.closingDate.format('YYYY-MM-DD') : closeForm.value.closingDate,
...
async function handleSubmitDeadline() {
  ...
  try {
    const data: CreateDeadlineCommand = {
      ...
      deadlineDate: typeof deadlineForm.value.deadlineDate === 'string' 
...
async function loadSchedules() {
  if (!matter.value?.id) return;
  try {
    const res = await getSchedules({ matterId: matter.value.id, pageNum: 1, pageSize: 100 });
    schedules.value = res || [];
  } catch (error) {
...
<template>
  <Page>
      ...
      <div v-if="matter" class="matter-detail">
        ...
        <div style="margin-bottom: 16px">
          ...
          <Space style="float: right">
            <Button 
              v-if="!['ARCHIVED', 'CLOSED', 'PENDING_CLOSE'].includes(matter.status)" 
              @click="handleEdit"
            >
              编辑
            </Button>
        ...
        <Card title="基本信息" style="margin-bottom: 16px">
          <Descriptions :column="2" bordered>
            <DescriptionsItem label="项目编号">{{ matter.matterNo }}</DescriptionsItem>
            <DescriptionsItem label="项目名称">{{ matter.name }}</DescriptionsItem>
            <DescriptionsItem label="项目类型">
              <Tag>{{ matterTypeMap[matter.matterType] || matter.matterType }}</Tag>
            </DescriptionsItem>
            <DescriptionsItem label="业务类型">{{ matter.businessType || '-' }}</DescriptionsItem>
            <DescriptionsItem label="客户">{{ matter.clientName || '-' }}</DescriptionsItem>
            <DescriptionsItem label="状态">
              <Tag :color="statusMap[matter.status]?.color || 'default'">
                {{ statusMap[matter.status]?.text || matter.status }}
              </Tag>
            </DescriptionsItem>
            <DescriptionsItem label="案源人">{{ matter.originatorName || '-' }}</DescriptionsItem>
            <DescriptionsItem label="主办律师">{{ matter.leadLawyerName || '-' }}</DescriptionsItem>
            <DescriptionsItem label="部门">{{ matter.departmentName || '-' }}</DescriptionsItem>
            <DescriptionsItem label="收费类型">{{ feeTypeMap[matter.feeType || ''] || '-' }}</DescriptionsItem>
            <DescriptionsItem label="预估费用">
              {{ matter.estimatedFee ? `¥${matter.estimatedFee.toLocaleString()}` : '-' }}
            </DescriptionsItem>
            <DescriptionsItem label="实际费用">
              {{ matter.actualFee ? `¥${matter.actualFee.toLocaleString()}` : '-' }}
            </DescriptionsItem>
            <DescriptionsItem label="立案日期">
              {{ matter.filingDate ? dayjs(matter.filingDate).format('YYYY-MM-DD') : '-' }}
            </DescriptionsItem>
            <DescriptionsItem label="预计结案日期">
              {{ matter.expectedClosingDate ? dayjs(matter.expectedClosingDate).format('YYYY-MM-DD') : '-' }}
            </DescriptionsItem>
            <DescriptionsItem label="实际结案日期">
              {{ matter.actualClosingDate ? dayjs(matter.actualClosingDate).format('YYYY-MM-DD') : '-' }}
            </DescriptionsItem>
            <DescriptionsItem label="标的金额">
              {{ matter.claimAmount ? `¥${matter.claimAmount.toLocaleString()}` : '-' }}
            </DescriptionsItem>
            <DescriptionsItem label="创建时间" :span="2">
              {{ matter.createdAt ? dayjs(matter.createdAt).format('YYYY-MM-DD HH:mm:ss') : '-' }}
            </DescriptionsItem>
            <DescriptionsItem label="项目描述" :span="2">{{ matter.description || '-' }}</DescriptionsItem>
            <DescriptionsItem label="备注" :span="2">{{ matter.remark || '-' }}</DescriptionsItem>
          </Descriptions>
        </Card>
        ...
        <Card style="margin-bottom: 16px">
          <Tabs v-model:activeKey="activeTab">
            ...
            <TabPane key="letters" tab="出函记录">
              <div v-if="matter.status === 'ACTIVE' || matter.status === 'DRAFT'" style="margin-bottom: 16px">
                <Button type="primary" @click="handleApplyLetter">申请出函</Button>
              </div>
              <Table
                ...
                :columns="[
                  ...
                  { title: '联系人', dataIndex: 'targetContact', key: 'targetContact', width: 100 },
                  { title: '联系电话', dataIndex: 'targetPhone', key: 'targetPhone', width: 120 },
                  { title: '出函律师', dataIndex: 'lawyerNames', key: 'lawyerNames', width: 120 },
                  { title: '份数', dataIndex: 'copies', key: 'copies', width: 60 },
                  { title: '期望日期', dataIndex: 'expectedDate', key: 'expectedDate', width: 100 },
              ...
              >
                <template #bodyCell="{ column, record }">
                  <template v-if="column.key === 'statusName'">
                    <Tag :color="getLetterStatusColor(record.status)">{{ record.statusName }}</Tag>
                  </template>
                  ...
                  <template v-else-if="column.key === 'action'">
                    <Button type="link" size="small" @click="handleViewLetterDetail(record as LetterApplicationDTO)">查看详情</Button>
                  </template>
                </template>
              </Table>
            ...
            <TabPane key="deadlines" tab="期限提醒">
              <div v-if="canEditOperations" style="margin-bottom: 16px">
                <Space>
                  <Button type="primary" @click="handleAddDeadline">添加期限</Button>
                  <Button @click="handleAutoCreateDeadlines">自动创建常用期限</Button>
                </Space>
              </div>
              <Table
                v-if="deadlines.length > 0"
                :columns="[
                  { title: '期限名称', dataIndex: 'deadlineName', key: 'deadlineName', width: 120 },
                  { title: '类型', dataIndex: 'deadlineTypeName', key: 'deadlineTypeName', width: 80 },
                  { title: '截止日期', dataIndex: 'deadlineDate', key: 'deadlineDate', width: 100 },
                  { title: '状态', key: 'status', width: 80 },
                  { title: '提醒', dataIndex: 'reminderDays', key: 'reminderDays', width: 60 },
              ...
              >
                <template #bodyCell="{ column, record }">
                  <template v-if="column.key === 'deadlineDate'">
                    {{ record.deadlineDate ? dayjs(record.deadlineDate).format('YYYY-MM-DD') : '-' }}
                  </template>
                  <template v-else-if="column.key === 'status'">
                    <Tag :color="getDeadlineStatusColor(record.status, record.daysRemaining)">
                      {{ getDeadlineStatusText(record.status, record.daysRemaining) }}
                    </Tag>
                  </template>
                  <template v-else-if="column.key === 'action'">
              ...
              <Empty v-else :description="canEditOperations ? '暂无期限提醒，点击上方按钮添加' : '暂无期限提醒'" />
            </TabPane>

            <!-- 日程安排 -->
            <TabPane key="schedules" tab="日程安排">
              <div v-if="canEditOperations" style="margin-bottom: 16px">
                <Button type="primary" @click="handleAddSchedule">添加日程</Button>
              </div>
              <Table
                v-if="schedules.length > 0"
                :columns="[
                  { title: '日程标题', dataIndex: 'title', key: 'title', width: 180, ellipsis: true },
              ...
              >
                <template #bodyCell="{ column, record }">
                  ...
                  <template v-else-if="column.key === 'startTime'">
                    {{ record.startTime ? dayjs(record.startTime).format('MM-DD HH:mm') : '-' }}
                  </template>
                  <template v-else-if="column.key === 'endTime'">
                    {{ record.endTime ? dayjs(record.endTime).format('MM-DD HH:mm') : '-' }}
                  </template>
                  <template v-else-if="column.key === 'status'">
              ...
              <Empty v-else :description="canEditOperations ? '暂无日程安排，点击上方按钮添加' : '暂无日程安排'" />
            </TabPane>
              ...
              <div v-if="timeline.length > 0" class="timeline-container">
                <div v-for="(item, index) in timeline" :key="index" class="timeline-item">
                  ...
                  <div class="timeline-content">
                    <div class="timeline-title">{{ item.title || item.eventTypeName || item.eventType }}</div>
                    <div class="timeline-description">{{ item.description }}</div>
                    <div class="timeline-time">{{ dayjs(item.eventTime).format('YYYY-MM-DD HH:mm:ss') }}</div>
                  </div>
                </div>
              </div>
            ...
            <TabPane key="participants" tab="团队成员">
              <Table
                ...
                :columns="[
                  ...
                  { title: '角色', dataIndex: 'role', key: 'role', width: 90, customRender: ({ text }) => roleMap[text] || text },
                  { title: '提成比例', dataIndex: 'commissionRate', key: 'commissionRate', width: 80, customRender: ({ text }) => text ? `${text}%` : '-' },
                  { title: '是否案源', dataIndex: 'isOriginator', key: 'isOriginator', width: 80, customRender: ({ text }) => text ? '是' : '否' },
                ]"
                :dataSource="matter.participants"
                :pagination="false"
                rowKey="id"
                size="small"
              />
        ...
        <Card v-if="matter.opposingParty" title="对方当事人信息">
          <Descriptions :column="2" bordered>
            <DescriptionsItem label="对方当事人">{{ matter.opposingParty }}</DescriptionsItem>
            <DescriptionsItem label="对方律师">{{ matter.opposingLawyerName || '-' }}</DescriptionsItem>
            <DescriptionsItem label="律师执业证号">{{ matter.opposingLawyerLicenseNo || '-' }}</DescriptionsItem>
            <DescriptionsItem label="律师事务所">{{ matter.opposingLawyerFirm || '-' }}</DescriptionsItem>
            <DescriptionsItem label="联系电话">{{ matter.opposingLawyerPhone || '-' }}</DescriptionsItem>
            <DescriptionsItem label="邮箱">{{ matter.opposingLawyerEmail || '-' }}</DescriptionsItem>
          </Descriptions>
        </Card>
      </div>
    </Spin>
    ...
    <!-- 添加期限弹窗 -->
    <Modal v-model:open="deadlineModalVisible" title="添加期限提醒" width="500px" @ok="handleSubmitDeadline">
      <Form :label-col="{ span: 5 }" :wrapper-col="{ span: 18 }">
        ...
        <FormItem label="期限名称" required>
          <Input v-model:value="deadlineForm.deadlineName" placeholder="如：一审举证期限" />
        </FormItem>
        <FormItem label="截止日期" required>
          <DatePicker v-model:value="deadlineForm.deadlineDate" style="width: 100%" format="YYYY-MM-DD" />
        </FormItem>
        <FormItem label="提前提醒">
        ...
        <FormItem label="说明">
          <Textarea v-model:value="deadlineForm.description" :rows="2" placeholder="备注说明" />
        </FormItem>
      </Form>
    </Modal>

    <!-- 申请出函弹窗 -->
    <Modal v-model:open="letterModalVisible" title="申请出函" width="600px" @ok="handleSubmitLetter">
      <Form :label-col="{ span: 5 }" :wrapper-col="{ span: 18 }">
        ...
        <FormItem label="接收单位" required>
          <Input v-model:value="letterForm.targetUnit" placeholder="如：XX市中级人民法院" />
        </FormItem>
        <FormItem label="联系人">
          <Input v-model:value="letterForm.targetContact" placeholder="接收单位联系人" />
        </FormItem>
        <FormItem label="联系电话">
          <Input v-model:value="letterForm.targetPhone" placeholder="联系电话" />
        </FormItem>
        <FormItem label="出函事由" required>
          <Textarea v-model:value="letterForm.purpose" :rows="2" placeholder="请输入出函事由" />
        </FormItem>
        <FormItem label="出函律师">
          <UserTreeSelect
        ...
        <FormItem label="份数">
          <InputNumber v-model:value="letterForm.copies" :min="1" :max="10" style="width: 100%" />
        </FormItem>
        <FormItem label="期望日期">
          <DatePicker v-model:value="letterForm.expectedDate" style="width: 100%" format="YYYY-MM-DD" />
        </FormItem>
        <FormItem label="备注">
          <Textarea v-model:value="letterForm.remark" :rows="2" placeholder="备注信息" />
        </FormItem>
      </Form>
    </Modal>

    <!-- 添加日程弹窗 -->
    <Modal v-model:open="scheduleModalVisible" title="添加日程" width="550px" @ok="handleSubmitSchedule">
      <Form :label-col="{ span: 5 }" :wrapper-col="{ span: 18 }">
        <FormItem label="日程标题" required>
          <Input v-model:value="scheduleForm.title" placeholder="如：XX案开庭" />
        </FormItem>
        ...
        <FormItem label="日期" required>
          <DatePicker v-model:value="scheduleForm.date" style="width: 100%" format="YYYY-MM-DD" />
        </FormItem>
        <FormItem label="全天">
        ...
        <FormItem label="地点">
          <Input v-model:value="scheduleForm.location" placeholder="如：XX法院第3法庭" />
        </FormItem>
        <FormItem label="提前提醒">
          <Select v-model:value="scheduleForm.reminderMinutes" :options="REMINDER_OPTIONS" />
        </FormItem>
        <FormItem label="备注">
          <Textarea v-model:value="scheduleForm.description" :rows="2" placeholder="备注信息" />
        </FormItem>
      </Form>
    </Modal>
    ...
    >
      <div v-if="letterDetailRecord">
        <Descriptions :column="2" bordered>
          <DescriptionsItem label="申请编号">{{ letterDetailRecord.applicationNo }}</DescriptionsItem>
          <DescriptionsItem label="函件类型">
            <Tag>{{ letterDetailRecord.letterTypeName }}</Tag>
          </DescriptionsItem>
          <DescriptionsItem label="模板名称">{{ letterDetailRecord.templateName || '-' }}</DescriptionsItem>
          <DescriptionsItem label="状态">
            <Tag :color="getLetterStatusColor(letterDetailRecord.status)">{{ letterDetailRecord.statusName }}</Tag>
          </DescriptionsItem>
          <DescriptionsItem label="接收单位" :span="2">{{ letterDetailRecord.targetUnit }}</DescriptionsItem>
          <DescriptionsItem label="联系人">{{ letterDetailRecord.targetContact || '-' }}</DescriptionsItem>
          <DescriptionsItem label="联系电话">{{ letterDetailRecord.targetPhone || '-' }}</DescriptionsItem>
          <DescriptionsItem label="接收地址" :span="2">{{ letterDetailRecord.targetAddress || '-' }}</DescriptionsItem>
          <DescriptionsItem label="出函律师">{{ letterDetailRecord.lawyerNames || '-' }}</DescriptionsItem>
          <DescriptionsItem label="份数">{{ letterDetailRecord.copies }}</DescriptionsItem>
          <DescriptionsItem label="期望日期">
            {{ letterDetailRecord.expectedDate ? dayjs(letterDetailRecord.expectedDate).format('YYYY-MM-DD') : '-' }}
          </DescriptionsItem>
          <DescriptionsItem label="申请人">{{ letterDetailRecord.applicantName }}</DescriptionsItem>
          <DescriptionsItem label="出函事由" :span="2">{{ letterDetailRecord.purpose }}</DescriptionsItem>
          <DescriptionsItem label="备注" :span="2">{{ letterDetailRecord.remark || '-' }}</DescriptionsItem>
          ...
          <DescriptionsItem v-if="letterDetailRecord.approvedAt" label="审批时间">
            {{ dayjs(letterDetailRecord.approvedAt).format('YYYY-MM-DD HH:mm:ss') }}
          </DescriptionsItem>
          <DescriptionsItem v-if="letterDetailRecord.approverName" label="审批人">
            {{ letterDetailRecord.approverName }}
          </DescriptionsItem>
          <DescriptionsItem v-if="letterDetailRecord.approvalComment" label="审批意见" :span="2">
            {{ letterDetailRecord.approvalComment }}
          </DescriptionsItem>
          <DescriptionsItem v-if="letterDetailRecord.printedAt" label="打印时间">
            {{ dayjs(letterDetailRecord.printedAt).format('YYYY-MM-DD HH:mm:ss') }}
          </DescriptionsItem>
          <DescriptionsItem v-if="letterDetailRecord.printerName" label="打印人">
            {{ letterDetailRecord.printerName }}
          </DescriptionsItem>
          <DescriptionsItem v-if="letterDetailRecord.receivedAt" label="领取时间">
            {{ dayjs(letterDetailRecord.receivedAt).format('YYYY-MM-DD HH:mm:ss') }}
          </DescriptionsItem>
          <DescriptionsItem v-if="letterDetailRecord.receiverName" label="领取人">
