...
import { DOCUMENT_TYPE_OPTIONS, TONE_OPTIONS, type AiStatus } from '../types';
...
const revisionMode = ref<'regenerate' | 'refine'>('refine'); // regenerate: 重新生成, refine: 在此基础上修改
...
const contextCollectorRef = ref<InstanceType<typeof ContextCollector>>();
...
function handleAddCustomTone() {
  if (customToneInput.value && !customTones.value.includes(customToneInput.value)) {
...
async function handleRevision() {
  ...
  try {
    ...
    aiGeneratedContent.value = result.content;
    revisionRequest.value = '';
    message.success(revisionMode.value === 'refine' ? '文书已根据您的要求修改！' : '文书已重新生成！');
  } catch (error: any) {
    stopProgress(false);
    message.error(error.message || 'AI 生成失败');
    // 恢复之前的内容
...
<template>
  <div class="ai-mode">
            ...
            <Row :gutter="16" align="middle">
              <Col :span="16">
                <Select
                  ...
                  showSearch
                >
                  <template #suffixIcon><IconifyIcon icon="carbon:machine-learning-model" /></template>
                </Select>
              </Col>
              <Col :span="8">
                <Space>
                  ...
                  <template #dropdownRender="{ menuNode }">
                    <div>
                      ...
                      <div v-if="customToneInput" class="custom-tone-add">
                        ...
                        <div class="add-custom-tone" @click="handleAddCustomTone">
                          <span>➕ 添加自定义：</span>
          ...
          <FormItem label="补充信息（可选）">
            <div class="ocr-upload">
              <Space>
                ...
                <span class="hint-text">支持上传图片，自动提取文字作为参考</span>
              </Space>
            </div>
            <Textarea
          ...
          <Col :span="12">
            ...
            <div class="doc-summary">
              ...
              <p class="requirement-preview">
                ...
                {{ aiFormData.requirement ? aiFormData.requirement.substring(0, 100) + (aiFormData.requirement.length > 100 ? '...' : '') : '未填写' }}
              </p>
            ...
            >
              <template #message>
                <Space>
                  <span>📂 保存位置：</span>
                  <strong>{{ matterSelectorRef?.selectedMatter?.name || '项目卷宗' }}</strong>
                </Space>
              </template>
            </Alert>
            <Alert 
          ...
          <Row :gutter="24">
            <Col :span="16">
              <div class="preview-header">
                <h4 style="margin: 0;">
                  <IconifyIcon icon="carbon:machine-learning-model" class="ai-icon" /> AI 生成内容
                ...
                <Space>
                  ...
                  <Button type="link" :loading="aiGenerating" @click="handleGenerate">
                    <IconifyIcon icon="carbon:renew" /> 重新生成
                  </Button>
                  <Button type="link" @click="showRevision" :disabled="aiGenerating">
              ...
              <Card v-if="showRevisionPanel" class="revision-panel">
                <template #title>
                  <Space>
                    ...
                    <span>修改要求</span>
                    <Tag v-if="revisionMode === 'refine'" color="blue">在此基础上修改</Tag>
                    <Tag v-else color="orange">完全重新生成</Tag>
                  </Space>
                </template>
                <template #extra>
                  <Button type="text" size="small" @click="cancelRevision">取消</Button>
                </template>
                ...
                <div class="revision-mode">
                  ...
                  <Space>
                    ...
                    >
                      <IconifyIcon icon="carbon:settings-adjust" /> 在此基础上修改
                    </Button>
                    <Button 
                      :type="revisionMode === 'regenerate' ? 'primary' : 'default'"
                      size="small"
                ...
                <Textarea 
                  ...
                  :placeholder="revisionMode === 'refine' 
                    ? '请输入修改要求，例如：\n- 第一段增加更多法律依据\n- 结尾部分语气更加正式\n- 补充关于时效的说明' 
                    : '请输入新的文书要求...'"
                  :rows="4"
                  show-count
                ...
                <div class="revision-actions">
                  <Space>
                    ...
                    >
                      <IconifyIcon :icon="revisionMode === 'refine' ? 'carbon:settings-adjust' : 'carbon:renew'" />
              ...
              <div v-if="aiGenerating" class="generate-progress">
                <Card class="progress-card">
                  <div class="progress-header">
                    <IconifyIcon icon="carbon:machine-learning-model" class="progress-icon" />
              ...
              <Card v-else class="preview-card">
                <pre class="preview-content">{{ displayedContent || '点击"生成预览"开始生成文书' }}</pre>
              </Card>
            </Col>
            <Col :span="8">
              ...
              <Card>
                ...
                <Divider />
                <p class="model-info">
                  <IconifyIcon icon="carbon:machine-learning-model" class="ai-icon" /> 使用 {{ selectedModel?.name || aiStatus.model }} 
              ...
              <!-- 脱敏映射详情 -->
              <Card v-if="maskingMapping?.mappings?.length" class="mapping-card" size="small">
                <template #title>
                  <span class="mapping-title">🔐 脱敏映射（{{ maskingMapping.mappings.length }} 项）</span>
                </template>
                <div class="mapping-content">
                  <div v-for="(m, i) in maskingMapping.mappings.slice(0, 10)" :key="i" class="mapping-item">
                  ...
                  <p v-if="maskingMapping.mappings.length > 10" class="more-hint">
    ...
    <div class="action-bar">
      <Space>
        <Button v-if="aiStep > 0 && !generateSuccess" @click="handlePrev">上一步</Button>
        <Button 
          v-if="aiStep < 2" 
          type="primary" 
