...
import { recognizeBusinessLicense, recognizeIdCard } from '#/api/ocr';
...
function handleApprove(row: ConflictCheckDTO) {
  ...
  Modal.confirm({
    ...
    onOk: async () => {
      try {
        await (isExemption
          ? approveExemption(row.id, '批准豁免')
          : approveConflictCheck(row.id));
...
function handleViewDetail(row: ConflictCheckDTO) {
  Modal.info({
    ...
    content: h('div', { style: 'padding: 16px 0' }, [
      ...
      row.reviewComment
        ? h('p', {}, [h('strong', {}, '审核意见: '), row.reviewComment])
        : null,
    ]),
  });
}
...
function handleApplyExemption(row: ConflictCheckDTO) {
  ...
  Modal.confirm({
    ...
    content: h('div', {}, [
      h(
        'p',
        { style: 'color: #ff4d4f; margin-bottom: 16px' },
  ...
  <Page title="利冲检查" description="管理利益冲突审查">
    <Card>
      ...
      <Grid>
        ...
        <!-- 结果列 -->
        <template #result="{ row }">
          <Tag v-if="row.result" :color="getResultColor(row.result)">
            {{ row.resultName }}
          </Tag>
          <span v-else>-</span>
        </template>
        ...
        <template #action="{ row }">
          <Space>
            ...
            <a
              ...
              @click="handleApplyExemption(row)"
              style="color: #722ed1"
              >申请豁免</a
            >
      ...
      <Spin :spinning="ocrLoading" tip="正在识别证件...">
        ...
        <Alert type="info" style="margin-bottom: 16px" show-icon>
          <template #message>
            ...
            <span class="ml-2 text-xs text-gray-500">上传证件自动填充</span>
          </template>
        ...
        >
          <FormItem
            label="审查类型"
            name="checkType"
            :rules="[{ required: true, message: '请选择审查类型' }]"
          >
            <Select
          ...
          >
            <Input
              ...
              placeholder="请输入对方当事人（可OCR识别）"
            />
          </FormItem>
          <FormItem label="项目名称" name="matterName">
            <Input
              v-model:value="formData.matterName"
