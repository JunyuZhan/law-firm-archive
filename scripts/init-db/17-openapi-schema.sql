...
CREATE TABLE IF NOT EXISTS public.openapi_push_config (
    ...
    -- 关联信息
    matter_id BIGINT NOT NULL,                    -- 项目ID（一个项目一条配置）
    client_id BIGINT NOT NULL,                    -- 客户ID
...
CREATE TABLE IF NOT EXISTS public.openapi_client_token (
    ...
    -- 令牌信息
    token VARCHAR(128) NOT NULL,                  -- 访问令牌（安全随机生成）
...
-- =====================================================
ALTER TABLE ONLY public.openapi_push_config
    DROP CONSTRAINT IF EXISTS openapi_push_config_matter_id_key,
    ADD CONSTRAINT uk_openapi_push_config_matter_id UNIQUE (matter_id);

ALTER TABLE ONLY public.openapi_client_token
    DROP CONSTRAINT IF EXISTS openapi_client_token_token_key,
    ADD CONSTRAINT uk_openapi_client_token_token UNIQUE (token);

ALTER TABLE ONLY public.openapi_matter_share_config
    DROP CONSTRAINT IF EXISTS openapi_matter_share_config_matter_id_key,
