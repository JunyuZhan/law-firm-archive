...
-- 5. 初始化权限菜单（使用正确的列名）
-- =====================================================
-- 系统管理下的开放接口管理菜单（parent_id=2 是系统管理）
INSERT INTO public.sys_menu (id, parent_id, name, path, component, icon, menu_type, permission, sort_order, visible, status, is_external, is_cache)
VALUES 
    (1810, 2, '开放接口管理', '/system/openapi', 'system/openapi/index', 'ApiOutlined', 'MENU', 'system:openapi:list', 18, true, 'ENABLED', false, true)
ON CONFLICT (id) DO NOTHING;

-- 开放接口管理的按钮权限
INSERT INTO public.sys_menu (id, parent_id, name, menu_type, permission, sort_order, visible, status, is_external, is_cache)
VALUES 
    (1811, 1810, '查看令牌', 'BUTTON', 'system:openapi:list', 1, true, 'ENABLED', false, false),
    (1812, 1810, '创建令牌', 'BUTTON', 'system:openapi:create', 2, true, 'ENABLED', false, false),
    (1813, 1810, '撤销令牌', 'BUTTON', 'system:openapi:revoke', 3, true, 'ENABLED', false, false),
    (1814, 1810, '查看日志', 'BUTTON', 'system:openapi:log', 4, true, 'ENABLED', false, false)
ON CONFLICT (id) DO NOTHING;
...
WHERE r.role_code = 'ADMIN' AND m.id IN (1810, 1811, 1812, 1813, 1814, 1815, 1816)
