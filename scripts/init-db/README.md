...
| `22-civil-cause-of-action.sql` | **民事案由数据**：2025年修订版，完整数据（1055项） | ✅ 必须 |
| `23-criminal-cause-of-action.sql` | **刑事罪名数据**：常用罪名 | ✅ 必须 |
| `24-admin-cause-of-action.sql` | **行政案由数据**：常用行政案由 | ✅ 必须 |
| `25-enhancement.sql` | **整合版**：version字段、权限细化、自动归档 | ✅ 必须 |
| `27-dict-init-data.sql` | 字典初始化数据（50种字典类型） | ✅ 必须 |

> **注意**: 21号脚本为冗余脚本，配置已包含在 20 中，无需执行。
