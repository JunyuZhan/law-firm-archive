...
main() {
    ...
    check_api "联系人" "$BASE_URL/client/contact"
    ...
    check_api "报销列表" "$BASE_URL/finance/expense/list"
    check_api "佣金列表" "$BASE_URL/finance/commission/list"
    check_api "工资单列表" "$BASE_URL/finance/payroll/list"
    ...
    check_api "质检列表" "$BASE_URL/knowledge/quality-check/list"
    check_api "风险提示" "$BASE_URL/knowledge/risk-warning/list"

    echo ""
    echo -e "${BLUE}=== 人事管理 (6项) ===${NC}"

    check_api "员工列表" "$BASE_URL/hr/employee?pageNum=1&pageSize=5"
    check_api "劳动合同" "$BASE_URL/hr/contract/list"
    check_api "请假列表" "$BASE_URL/hr/leave/list"
    check_api "考勤列表" "$BASE_URL/hr/attendance/list"
    check_api "晋升列表" "$BASE_URL/hr/promotion/list"
    check_api "培训列表" "$BASE_URL/hr/training/list"
    ...
    check_api "请假申请" "$BASE_URL/admin/leave/list"
    check_api "加班申请" "$BASE_URL/admin/overtime/list"
    check_api "外出申请" "$BASE_URL/admin/goout/list"

    echo ""
    echo -e "${BLUE}=== 工作台 (5项) ===${NC}"

    check_api "待办列表" "$BASE_URL/workbench/approval/list?pageNum=1&pageSize=5"
    check_api "我的待办" "$BASE_URL/workbench/approval/my"
    check_api "统计信息" "$BASE_URL/workbench/statistics"
    check_api "报告列表" "$BASE_URL/workbench/report/list"
    check_api "定时报告" "$BASE_URL/workbench/scheduled-report/list"
