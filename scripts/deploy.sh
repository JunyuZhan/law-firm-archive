...
setup_env() {
    ...
    if [ "$FIRST_TIME" = true ]; then
        ...
        # MinIO 使用默认密码 minioadmin/minioadmin（可共享）
        ...
        if [[ "$OSTYPE" == "darwin"* ]]; then
            ...
            # MinIO 使用默认密码 minioadmin/minioadmin，不需要在 .env 中配置
        ...
        # MinIO 使用默认密码 minioadmin/minioadmin（可共享，便于统一管理）
        # 不再检查 MinIO 默认密码，因为它是共享服务
