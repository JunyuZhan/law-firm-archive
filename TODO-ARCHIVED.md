...
> 📁 此文件为历史归档，包含所有任务的详细记录和代码示例。
> 
> 📋 当前待处理任务请查看 [TODO.md](./TODO.md)
...
| 优先级 | 问题 | 位置 | 修复方案 |
|--------|------|------|----------|
| ✅ | 全量加载员工到内存 | `PayrollAppService.java:194-206` | 已改为 SQL 条件过滤（按查询月末、离职状态过滤） |
| ✅ | 全量加载用户到内存 | `ApproverService.java:400-401` | 已改为 `LIMIT 1` 查询 |

**核实结果**（2026-02-12）：
- ✅ `PayrollAppService.java` 第194-206行：使用 `lambdaQuery()` 带条件过滤，不再全表加载
- ✅ `ApproverService.java` 第400-401行：改为 `lambdaQuery().last("LIMIT 1").one()`
...
**核实结果**（2026-02-12）：
- ✅ 已修复：使用 `try-catch` 捕获 `DuplicateKeyException` 并重新查询
- ✅ 代码位置：`DataPushService.java:316-340`
...
**核实结果**（2026-02-12）：
- ✅ 已修复：使用 `lambdaQuery()` 在 SQL 层面过滤
- ✅ 代码位置：`PayrollAppService.java:194-206`

**修复后代码**：
```java
// PayrollAppService.java:194-206 - 使用SQL条件过滤
...
**核实结果**（2026-02-12）：
- ✅ 整个方法使用 `@Transactional(rollbackFor = Exception.class)`，事务保证原子性
- ✅ 如果事件处理失败，整个事务会回滚，不会出现部分成功部分失败
- 🟢 当前实现可接受，风险较低
...
**核实结果**（2026-02-12）：
- ✅ **已有 `SecurityConfigValidator` 启动时强制检查**
- ✅ 生产环境（prod profile）如果使用默认密钥会抛异常阻止启动
- ✅ 检查项包括：数据库密码、JWT密钥、MinIO密钥
- ✅ 开发环境会跳过检查并提示

**位置**：`SecurityConfigValidator.java`
...
**核实结果**（2026-02-12）：
- ✅ `CauseOfActionService` 已使用 `@Cacheable` 缓存案由数据
- ✅ `CacheConfig` 已配置缓存基础设施
- 🟢 其他服务可按需添加缓存，属于性能优化建议
...
**核实结果**（2026-02-12）：
- ✅ 已正确配置 `addUrlPatterns("/*")` 拦截所有请求
- ✅ `setOrder(Integer.MAX_VALUE)` 确保最后执行清理
- ✅ 清理 StatisticsAppService、ContractDataPermissionService、ReportAppService 的 ThreadLocal 缓存
...
**核实结果**（2026-02-12）：
- ❌ 确认问题存在，`reorderDocuments` 方法循环内逐个 `updateById`
- 影响：排序操作产生 2N 次数据库操作（N次查询 + N次更新）
...
**核实结果**（2026-02-12）：
- ❌ 确认问题存在
- `getMatterById` 调用单条 `toDTO`，内部查询：客户、案由、合同、用户等
- `toParticipantDTO` 循环内查询用户信息
- `toMatterClientDTO` 循环内查询客户信息
- 但：**详情页为单条记录场景，N+1 影响有限**
...
**核实结果**（2026-02-12）：
- ❌ 确认问题存在
- `toCostAllocationDTO` 内部查询 expense、user、matter
- `toCostSplitDTO` 内部查询 expense、user、matter
- 列表场景会产生 3N 次额外查询
...
**核实结果**（2026-02-12）：
- ✅ **已有 `@OperationLog` 注解和 `OperationLogAspect` 切面**
- ✅ 大量 Controller 已使用 `@OperationLog` 记录操作日志
- ✅ 日志记录：模块、动作、请求参数、响应结果
- ✅ 有 `OperationLogAppService` 提供日志查询
- 🟢 建议按需扩展敏感操作的日志记录
...
**核实结果**（2026-02-12）：
- ✅ **Mapper.xml 文件中未发现 `${}` 使用** - 所有动态 SQL 均使用 `#{}` 参数化
- ✅ `SqlUtils.escapeLike()` 用于模糊查询转义
- ✅ 列名白名单校验已实现（PageUtils、DataScopeInterceptor、DataScopeHelper）
...
**核实结果**（2026-02-12）：
- ✅ **已有 `AsyncConfig` 实现 `AsyncConfigurer`**
- ✅ 已配置 `taskExecutor` 线程池（核心线程=CPU+1，最大=CPU*2）
- ✅ 已配置 `AsyncUncaughtExceptionHandler` 异常处理器
- ✅ `ReportAppService.generateReportAsync` 已使用 `@Async("taskExecutor")`
- ⚠️ 部分 `@Async` 方法未指定线程池，但会使用默认的 `taskExecutor`
- ⚠️ `DossierAutoArchiveService` 仍使用 `Thread.sleep` 等待事务提交，建议优化

**状态**：✅ 已完成（基础配置已有，个别优化点为低优先级）
...
**核实结果**（2026-02-12）：
- ✅ 密码使用 `@Value` 从配置文件注入
- ✅ **日志中未发现密码泄露** - 搜索 `log.*(info|debug|warn|error).*password` 无匹配结果
- ⚠️ 建议配置 Spring Boot Actuator 敏感信息过滤（`management.endpoint.env.keys-to-sanitize`）
...
**问题描述**：定时任务在集群环境下可能重复执行。

**核实结果**（2026-02-12）：
- ❌ **确认问题存在**：系统未使用 ShedLock 或任何分布式锁机制
- ❌ 共发现 14 个 `@Scheduled` 定时任务，均无防重保护

**定时任务清单**：
| 任务名 | 类名 | Cron |
|--------|------|------|
| 工资表自动确认 | PayrollAutoConfirmScheduler | 0 0 2 * * ? |
| 系统日报 | SystemReportService | 0 0 8 * * ? |
| 系统周报 | SystemReportService | 0 0 9 ? * MON |
| 系统健康检查 | SystemReportService | 0 0 * * * ? |
| 任务到期提醒 | TaskReminderScheduler | 0 0 9 * * ? |
| 逾期任务警告 | TaskReminderScheduler | 0 0 10 * * ? |
| 自定义提醒检查 | TaskReminderScheduler | 0 0 * * * ? |
| 定时报表执行 | ScheduledReportScheduler | 0 * * * * ? |
| 日程提醒 | ScheduleReminderScheduler | 0 */5 * * * ? |
| 节假日年度同步 | HolidaySyncScheduler | 0 0 1 1 1 ? |
| 节假日月度检查 | HolidaySyncScheduler | 0 0 2 1 * ? |
| 合同到期提醒 | ContractExpiryReminderScheduler | 0 0 9 * * ? |
| 合同逾期警告 | ContractExpiryReminderScheduler | 0 0 10 * * ? |
| AI账单生成 | AiBillingScheduler | 0 0 2 1 * ? |

**修复建议**：
1. 引入 ShedLock 依赖
2. 为每个定时任务添加 `@SchedulerLock` 注解
3. 或确保定时任务具有幂等性设计
...
**修复记录**（2026-02-12）：
- ✅ 添加 `batchConvertToDTO` 方法，批量加载案例和用户信息
- ✅ `getCaseNotes` 和 `getMyNotes` 方法使用批量加载版本
...
```java
// CaseStudyNoteAppService.java - 已修复
...
**修复记录**（2026-02-12）：
- ✅ 添加 `batchConvertToDTO` 方法，批量加载项目和检查人信息
- ✅ `getChecksByMatterId` 和 `getInProgressChecks` 方法使用批量加载版本
...
```java
// QualityCheckAppService.java - 已修复
...
**修复记录**（2026-02-12）：
- ✅ 添加 `batchLoadTemplates` 方法，批量加载模板信息
- ✅ `listScheduledReports` 方法使用批量加载版 `toScheduledReportDTO`
...
```java
// ScheduledReportAppService.java - 已修复
...
**修复记录**（2026-02-12）：
- ✅ 添加 `batchLoadSheetUsers` 方法，批量加载审批人信息
- ✅ 添加 `fillApprovalInfo` 方法，填充审批信息
- ✅ `listPayrollSheets` 方法使用批量加载版 `toSheetDTO`
...
1. ~~**任务65**: 全量数据加载性能风险~~ ✅ 已修复（PayrollAppService 使用SQL过滤, ApproverService 改为 LIMIT 1）
2. ~~**任务82**: SQL注入防护检查~~ ✅ 已完成（Mapper.xml 中未使用 `${}`）
3. ~~**任务84**: 数据库连接信息安全~~ ✅ 已完成（日志中无密码泄露）
4. ~~**任务92**: 通知已读权限校验缺失~~ ✅ 已修复
5. **任务86/94**: 定时任务无分布式锁 ❌ **确认问题存在**（14个定时任务均无 ShedLock 保护）

### 中优先级问题 - 已修复

- ~~**任务66**: getOrCreateConfig 竞态条件~~ ✅ 使用 DuplicateKeyException 处理
- ~~**任务67**: 证据文件路径解析数组越界风险~~ ✅ 已修复
- ~~**任务70**: PayrollAppService 员工过滤逻辑复杂度~~ ✅ 使用 SQL 条件过滤
- ~~**任务72**: 默认密钥安全风险~~ ✅ SecurityConfigValidator 启动时检查
- ~~**任务75**: ThreadLocalCleanupFilter 配置检查~~ ✅ 已正确配置 `/*`
- ~~**任务80**: 统一审计日志~~ ✅ 已有 @OperationLog 注解和切面
- ~~**任务83**: 异步任务配置~~ ✅ AsyncConfig 已配置线程池和异常处理
- ~~**任务87-91**: EmployeeAppService/CaseLibraryAppService/NotificationAppService~~ ✅ 已修复
- ~~**任务95/100**: TimesheetAppService/ContractAppService N+1~~ ✅ 已修复

### 中优先级问题 - 待修复

1. **任务76**: DocumentAppService 循环内逐个更新排序（性能优化）
2. **任务79**: ExpenseAppService 成本记录转换N+1查询（性能优化）

### 低优先级问题（可暂缓）

- **任务71**: ApprovalAppService 批量审批事件发布时机（已有事务保护）
- **任务74**: 缓存使用不足（性能优化建议）
- **任务78**: MatterAppService 详情页N+1查询（单条记录影响小）

### 中优先级问题 - 最近修复（2026-02-12）

- ~~**任务96**: CaseStudyNoteAppService.toDTO N+1~~ ✅ 已修复
- ~~**任务97**: QualityCheckAppService.toDTO N+1~~ ✅ 已修复
- ~~**任务98**: ScheduledReportAppService.toDTO N+1~~ ✅ 已修复
- ~~**任务99**: PayrollAppService.toSheetDTO N+1~~ ✅ 已修复
